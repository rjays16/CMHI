...
//added julius
	...
	{
		global $db;
		$res = $db->Execute("SELECT encounter_nr FROM care_encounter where pid=".$db->qstr($pid)." AND is_discharged='0'");
				while($row=$res->FetchRow())
				{
					return trim($row["encounter_nr"]);
				}
	}
	function insurancePHIC($roomtype,$insurance)
	{
		global $db;
		$res = $db->Execute("SELECT PHIC from seg_cost_center_price where new_ward_nr=".$db->qstr($roomtype)." AND insurance=".$db->qstr($insurance));
				while($row=$res->FetchRow())
				{
					return trim($row["PHIC"]);
				}
	}
	function insuranceNONPHIC($roomtype)
	{
		global $db;
		$res = $db->Execute("SELECT NONPHIC FROM seg_cost_center_price where new_ward_nr=".$db->qstr($roomtype));
				while($row=$res->FetchRow())
				{
					return trim($row["NONPHIC"]);
				}

	}
	function getRoomTypes($enc){
		global $db;

			$res = $db->Execute("SELECT current_ward_nr from care_encounter
						WHERE encounter_nr = ".$db->qstr($enc));
				while($row=$res->FetchRow())
				{
					return trim($row["current_ward_nr"]);
				}


	}
	function getinsrnceTyp($enc)
	{
		global $db;

			$res = $db->Execute("SELECT hcare_id FROM seg_encounter_insurance WHERE encounter_nr=".$db->qstr($enc));
				while($row=$res->FetchRow())
				{
					return trim($row["hcare_id"]);
				}
	}
	function getprice($service_codes)
	{
		global $db;
		$res = $db->Execute("SELECT price_cash FROM seg_lab_services WHERE service_code=".$db->qstr($service_codes));
		while($row=$res->FetchRow())
		{
			return trim($row["price_cash"]);
		}

	}
	function getpriceradio($service_codes)
	{
		global $db;
		$res = $db->Execute("SELECT price_cash FROM seg_radio_services WHERE service_code=".$db->qstr($service_codes));
		while($row=$res->FetchRow())
		{
			return trim($row["price_cash"]);
		}
...
//				$filterSQL .= ") (AND a.prod_class='".$filter."'";
	...
	function getPhsDiscounts($service_code) {
		global $db;

		$this->sql ="SELECT price 
						FROM seg_service_discounts 
						WHERE discountid = 'PHS' 
						AND service_area = 'PH' 
	...
	//added by EJ 08/26/2014
	function checkDependent($pid){
		global $db;

		$pid = $db->qstr($pid);
		$this->sql = "SELECT * FROM seg_dependents WHERE dependent_pid = $pid";
