...
//include_once($root_path.'modules/radiology/radio-upload-image.php');
...
#echo "class_radiology.php : encHasRadioRequest : this->sql = '".$this->sql."' <br> \n";
	...
	function getAllRadioInfoByBatch($batch_nr='',$limit=TRUE){
		...
		$this->sql="SELECT r_findings.r_img, r_findings.r_id, r_request.rad_tech, r_request.served_date, r_serv.pid, sri.rid, r_serv.encounter_nr, r_serv.refno, r_serv.is_urgent,
...
#		"status", "service_date"   ==> from table 'care_test_request_radio'
		...
		$index = "r_img, r_id, batch_nr, findings, radio_impression,	findings_date, doctor_in_charge, encoder, history, create_id, create_dt";
		#$values = "$batch_nr, '$findings', '$radio_impression', '$findings_date', '$doctor_in_charge', '$result_status', '".
		$values = "'$r_img', '$r_id', $batch_nr, '$findings', '$radio_impression', '$findings_date', '$doctor_in_charge', '".
...
#" result_status = '$result_status', ".
		$elems="r_id = '$r_id', r_img = '$r_img', batch_nr = '$batch_nr', findings = '$findings', radio_impression ='$radio_impression', ".
...
#echo "class_radiology.php : updateRadioFindingInfoFromArray : this->sql = '".$this->sql."' <br> \n";
		...
		*/
	function saveAFinding($batch_nr='',$finding_nr='-1', $findings='', $radio_impression='', $findings_date='', $r_id='',$r_img='', $doctor_id='',$result_status='',$mode='Add'){
		global $db,$HTTP_SESSION_VARS;
...
#echo "class_radiology.php : saveAFinding : radio_impression_array : "; print_r($radio_impression_array); echo " <br> \n";
			...
			$findingsInfo['r_id'] = $r_id; #added by mai 05-27-2014
			$findingsInfo['r_img'] = $r_img; #added by mai 05-27-2014
	...
	function _searchBasicInfoRadioPending($search_type='',$key,$sub_dept_nr='',$ORDER_BY_OPTION='',$limit=FALSE,$len=30,$so=0){
		...
		if(is_numeric($key)){
			...
			if ($key){
				...
				if($key != "null"){
					$WHERE_SQL = " AND p.name_last LIKE '".$key."%'";
				}
			...
			/*if ($key == 'null'){ edited by mai 10-16-2014
				#echo "here";
...
#			echo "class_radiology.php : _searchBasicInfoRadioPending : FALSE 02 <br>";
		...
		*/
		function getAllInfoUnifiedBatchRequestList($refno=''){
				...
				$this->sql = "SELECT r_serv.pid,r_request.`served_date`, r_findings.`create_dt`, r_findings.r_id, r_serv.refno, r_serv.encounter_nr, r_request.batch_nr AS id, r_serv.is_urgent,
												r_findings.findings, r_findings.radio_impression, r_findings.findings_date, r_findings.doctor_in_charge,
												r_findings.result_status, r_findings.encoder AS findings_encoder,
												IF((ISNULL(r_request.is_in_house) || r_request.is_in_house='0'),
														r_request.request_doctor,
		...
		#added by VAN 03-18-08
			...
			// }
				...
				if($key != "null"){
					...
					$WHERE_SQL=" AND (
							    sched.scheduled_dt = DATE(NOW()) 
							    OR (
							      ISNULL(sched.scheduled_dt) 
							      AND (
							        r.`is_served` <> 1 
							        OR (
							          r.`is_served` = 1 
							          AND (DATE(r.served_date) = DATE(NOW()))
							        )
							      )
							    )
							  ) ";
        ...
        if (!empty($sub_dept_nr)) {
            ...
            "END)";
				...
				$this->sql="SELECT SQL_CALC_FOUND_ROWS r.request_flag AS charge_name,
								CONCAT('DR. ', fn_get_personell_firstname_last(e.`current_att_dr_nr`)) AS attending_dr,
								GROUP_CONCAT(r_services.`name`) AS services,
		...
		if(is_numeric($suchwort)) {
			...
			if(sizeof($comp)>1){
				...
				if($suchwort=='%'||$suchwort=='%%'){
                    ...
                    $whereClause = "WHERE (
									    sk.scheduled_dt = DATE(NOW()) OR (
									      ISNULL(sk.scheduled_dt) 
									      AND (
									        rs.`is_served` <> 1 
									        OR (
									          rs.`is_served` = 1 
									          AND (DATE(rs.served_date) = DATE(NOW()))
									        )
									      )
									    )
									  ) " . //edited by mai 10-16-2014
                        "AND r.status NOT IN ($this->dead_stat) AND rs.status NOT IN ($this->dead_stat) ";
				}else{
					# Check if it is a complete DOB
					$DOB=@formatDate2STD($suchwort,$date_format);
...
#			echo "class_radiology.php : _searchBasicInfoRadioDone : FALSE 02 <br>";
	...
	function checkHoliday($date){
		...
		$date = $db->qstr($date);
		$this->sql = "SELECT * FROM seg_holidays WHERE dynamic_date = $date";
	...
	//added by EJ 08/19/2014
	function getPriceIncrease($code){
		global $db;
		...
		$this->sql = "SELECT 
					  ROUND(SUM((slb.`price_cash` * inc.`increase`) + slb.`price_cash`),2) AS cash_price,
					  ROUND(SUM((slb.`price_charge` * inc.`increase`) + slb.`price_charge`),2) AS charge_price,
					  ROUND(SUM((slb.`price_cash` * inc.`increase`) + slb.`price_charge`),2) AS net_price 
					FROM `seg_radio_services` slb
					LEFT JOIN `seg_increase` inc   
					ON $where";
