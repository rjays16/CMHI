...
#echo "class_personell.php : getPersonellInfo :: sql ='".$sql."'<br> \n";   #burn: November 14, 2007
		...
		*/
        ...
        global $db;
                ...
                $this->sql ="SELECT
                                                 IF (other_title,other_title,if(job_function_title='Doctor',other_title,if(job_function_title='Consulting doctor','MD,FPCR',if(job_function_title='Radiologic Technologist','MD,DPBR','')))) as drtitle, p.doctor_role as drRole, p.doctor_level,
...
# echo "LastInsertPKAddress : this->sql = '".$this->sql."' <br> \n";
		...
		function get_Personell_info($personell_nr=0){
				...
				$this->sql ="SELECT *, fn_get_person_name_first_mi_last(pid) AS name FROM care_personell where nr='".$personell_nr."'";
       ...
       function getDoctorAccreditation($doctor_nr) {
        ...
        function updatePersonnelNameUsers($personell_nr, $name_person){
         ...
         function getDoctorWithDept($dept_nr, $keyword){
         	global $db, $sql_LIKE;
         	...
         	$this->sql = "SELECT 
							...
							WHERE cpl.`short_id` LIKE 'D%' 
							  AND (
							    cp.name_last ".$sql_LIKE." ".$db->qstr($keyword."%")
							    ." OR cp.name_first ".$sql_LIKE." ".$db->qstr($keyword."%")
							  .") AND cpa.location_nr=".$db->qstr($dept_nr);
         ...
         function getPersonellNrWithLoginID(){
         	global $db;
         	$this->sql = "SELECT 
							  personell_nr 
							FROM
         ...
         //added by mai 09-13-2014
        function getDrEncounter($encounter_nr){
         	global $db;
         	$this->sql = " SELECT 
							  CONCAT(
							    'DR. ',
							    fn_get_personell_firstname_last (dr.`attending_dr_nr`)
							  ) AS dr_name 
							FROM
							  seg_encounter_dr_mgt dr 
        ...
        function getDeptUser($user_id){
        	global $db;
        	$this->sql = "SELECT 
						  cd.`name_formal` AS name_dept 
						FROM
						  care_users cu 
						  LEFT JOIN care_personell_assignment cpa 
						    ON cpa.personell_nr = cu.`personell_nr` 
						  LEFT JOIN care_department cd 
						    ON cd.nr = cpa.location_nr WHERE cu.`login_id` = ".$db->qstr($user_id);

			if($this->result = $db->Execute($this->sql)){
				if($this->row = $this->result->FetchRow()){
					return $this->row['name_dept'];
				}
			}
        ...
        function getPersonnelName($login_id){
        	global $db;
        	$this->sql = "SELECT name FROM care_users WHERE login_id = ".$db->qstr($login_id);
        	if($this->result = $db->Execute($this->sql)){
        		if($this->row = $this->result->FetchRow()){
        ...
        function getAllDoctors(){
        	...
        	$this->sql = "SELECT 
							  cp.nr AS dr_nr,
							  fn_get_person_lastname_first(cp.`pid`) AS dr_name
							FROM
							  care_personell cp 
							WHERE job_function_title = 'Doctor' 
							ORDER BY fn_get_person_lastname_first(cp.`pid`)";
        ...
        function countPatientsEncounterDate($dr_nr, $date, $ptype){
        	global $db;

        	$this->sql = " SELECT 
							   COUNT(
								    IF(
								      current_att_dr_nr = ".$db->qstr($dr_nr).",
								      encounter_nr,
								      NULL
								    )
								  ) total_patient,
								  COUNT(encounter_nr) AS sum_patient 
							FROM
							  care_encounter 
							WHERE encounter_type = ".$db->qstr($ptype)."
							  AND DATE(encounter_date) = ".$db->qstr($date).
							  " AND current_att_dr_nr <> 0";
        ...
        function countPatientsDischargeDate($dr_nr, $date, $ptype){
        	global $db;
        	...
        	$this->sql = " SELECT 
							...
							FROM
							  care_encounter 
							WHERE encounter_type = ".$db->qstr($ptype)."
							  AND (DATE(discharge_date) >= ".$db->qstr($date).
							  " OR is_discharged = 0) AND DATE(encounter_date) <=".$db->qstr($date).
							  " AND current_att_dr_nr <> 0";
        ...
        function getNameTitle($pid){
        	...
        	$this->sql = "SELECT 
						  CONCAT(
						   fn_get_person_name_first_mi_last(care_person.pid),
						    ', ',
        ...
        function getLincenseNr($pid){
        	global $db;
        	
        	$this->sql = "SELECT license_nr FROM care_personell WHERE pid = ".$db->qstr($pid);
