...
class Rest_Curl{
	...
	function getPackUnit($unit_id){
		global $db;

		$sql = "SELECT unit_name FROM seg_unit WHERE unit_id = ".$db->qstr($unit_id);
...
function inpatientLabItem($refno, $frombill=false, $bill_nr=null, $bill_date=null){
	...
	function comPayment($data){
		...
		$comp_info = $this->getCompany($comp_id);
		...
		$data = array(
				'refno' => $or,
				'oldrefno' => $or,
				'pid' => $pid, //strtoupper($comp_info['comp_name']),
				'trdate' => date('m/d/Y', strtotime($date)),
				'time' => date('H:i:s', strtotime($date)),
				'src' => 'COM',
				'accounts' => $items,
	...
	function postHmoPf($encounter_nr, $ref_no, $hcare_id, $dr_nr){
		...
		if($doc_data){
			foreach($doc_data AS $value){
				...
				$account_codes = $this->getAccountCodes($hcare_id, '', PF_PAYMENT/*PF_HMO*/);
	...
	*/
	...
	function cashTransactions($or_no){
		...
		foreach($items AS $key => $value){
			...
			/*else if($value['ref_source'] == 'PF'){
				$amount = $value['amount_due'];
				$account_codes = $this->getAccountCodes(PF_PAYMENT, '', PF_PAYMENT);
				
				foreach($account_codes AS $codes_key => $codes_value){
					$credit_pf[$codes_value['credit_id']] += $amount;
					$debit_pf[$codes_value['debit_id']] += $amount;
				}
			}*/
		...
		#-----PF payment saving to FIS-----
		/*if(!empty($debit_pf) && !empty($credit_pf)){
			foreach($debit_pf AS $key => $value){
				$debits_pf[] = array('debitid'=>$key, 'amount'=>$value);
			}

			foreach($credit_pf AS $key => $value){
				$credits_pf[] = array('creditid'=>$key, 'amount'=>$value);
			}

			$accounts_pf = array('debits'=>$debits_pf, 'credits'=>$credits_pf);
			$accounts_pf = base64_encode(json_encode($accounts_pf));
				
			$data = array('accounts' => $accounts_pf,
							'or' => $or,
							'pid' => $pid,
							'enc' => $enc,
							'date' => $date);

			$this->walkinPfItem($data);
		}*/
