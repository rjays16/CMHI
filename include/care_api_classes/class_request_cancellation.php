	...
	class SegRequestCancel extends Core {
		...
		function getLaboratoryRequests($filters, $sort, $offset, $maxrows)
		{
			...
			$this->sql = "SELECT SQL_CALC_FOUND_ROWS \n".
									"(SELECT IF(b.bill_nr,1,0) FROM seg_billing_encounter AS b WHERE b.encounter_nr=l.encounter_nr AND is_deleted IS NULL) AS `is_billed`, \n".
		...
		function getRadiologyRequests($filters, $sort, $offset, $maxrows)
		{
			...
			$this->sql = "SELECT SQL_CALC_FOUND_ROWS \n".
									"(SELECT IF(b.bill_nr,1,0) FROM seg_billing_encounter AS b WHERE b.encounter_nr=r.encounter_nr AND b.is_deleted IS NULL) AS `is_billed`, \n".
		...
		function getPharmacyRequests($filters, $sort, $offset, $maxrows)
		{
			...
			$this->sql = "SELECT SQL_CALC_FOUND_ROWS \n".
									"(SELECT IF(b.bill_nr,1,0) FROM seg_billing_encounter AS b WHERE b.encounter_nr=p.encounter_nr AND b.is_deleted IS NULL) AS `is_billed`, \n".
		...
		function getPfRequests($filters, $sort, $offset, $maxrows){
			global $db;
			$cond = array();
			if(is_array($filters)){
				foreach($filters as $i=>$v)
					...
					switch($i)
					{
						case 'NAME':
							if (strpos($v,',')!==false) {
								$split_name = explode(',', $v);
								$cond[] = "cp.name_last LIKE ".$db->qstr(trim($split_name[0])."%");
								$cond[] = "cp.name_first LIKE ".$db->qstr(trim($split_name[1])."%");
							}
							else {
								if ($v) {
			...
			$this->sql = "SELECT 
								  ...
								  WHERE b.encounter_nr = p.encounter_nr AND b.is_deleted IS NULL) AS `is_billed`,
								  p.chrge_dte AS `request_date`,
								  p.`pid` AS `pid`,
								  p.refno,
								  ph.dr_nr AS `item_code`,
								  p.encounter_nr,
								  fn_get_person_name (p.`pid`) AS `patient_name`,
								  CONCAT(
								    'DR. ',
								    fn_get_person_name (pd.`pid`)
								  ) `item_name`,
								  ph.request_flag,
								  'none' AS `request_status` 
								FROM
								  seg_pf_service AS p 
								  INNER JOIN seg_pf_service_details AS ph 
								    ON p.refno = ph.refno 
								  INNER JOIN care_personell AS pd 
								    ON pd.short_id = ph.dr_nr 
		...
		function getMiscellaneousRequests($filters, $sort, $offset, $maxrows)
		{
			...
			$this->sql = "SELECT SQL_CALC_FOUND_ROWS \n".
									"(SELECT IF(b.bill_nr,1,0) FROM seg_billing_encounter AS b WHERE b.encounter_nr=m.encounter_nr AND b.is_deleted IS NULL) AS `is_billed`, \n".
		...
		function cancelPharmaRequestStatus($refno, $item_code, $reason)
		{
			...
			$this->returnInventoryItems($refno, $item_code); //return items to inventory
		...
		*08-14-2015
		*/
		function returnInventoryItems($refno, $item_code){
			$status = $this->checkStatusPharma($refno, $item_code);

			if($status['serve_status']){
				$invHelper = new InventoryHelper();
				$invHelper->addStock($status['bestellnum'], $status['pharma_area'], $status['quantity'], '', $refno, $refno, CANCEL);
			}

		}

		/*Get pharmacy item information
		*08-14-2015
		*/
		function checkStatusPharma($refno='', $item_code=''){
			global $db;
			...
			$this->sql = "SELECT 
							...
							WHERE spoi.`refno` = ".$db->qstr($refno)."
							  AND spoi.`bestellnum` = ".$db->qstr($item_code);
		...
		//added by mai 11-20-2014
		function deletePfRequestItem($refno, $item_code){
			global $db;
			$this->sql = "DELETE FROM seg_pf_service_details \n".
									"WHERE refno=".$db->qstr($refno)." AND dr_nr=".$db->qstr($item_code);
			$this->result = $db->Execute($this->sql);
			if($this->result!==FALSE) {
				return $this->result;
			} else {
