...
#echo"class_ward.php : getRoomInfo :: this->sql ='".$this->sql."' <br> \n";
	...
	*/
		$this->sql="SELECT DISTINCT r.location_nr AS room_nr,
									...
									p.photo_filename,
									e.insurance_class_nr,
									e.is_maygohome, /*added by mai 07-04-2014*/
									(SELECT h.hcare_id FROM seg_encounter_insurance h WHERE h.encounter_nr = b.encounter_nr AND hcare_id = '18') AS hcare_id, /*added by mai 07-04-2014*/
									i.name AS insurance_name,
									i.LD_var AS \"insurance_LDvar\",
									n.nr AS ward_notes
							FROM $this->tb_location AS r
...
function RoomNrExists($wardNr=0, $room_nr=0, $bol=1){
	...
	//created by EJ 09/17/2014
	function getQuickInformerDetails() {
		global $db;

		$this->sql = "SELECT 
					  enc.`encounter_nr`,
					  enc.`is_maygohome` AS mgh,
					  (SELECT 
					    ins.`hcare_id` 
					  FROM
					    seg_encounter_insurance AS ins 
					  WHERE ins.`encounter_nr` = enc.`encounter_nr` 
					    AND hcare_id = '18') AS phic 
					FROM
					  care_encounter AS enc 
