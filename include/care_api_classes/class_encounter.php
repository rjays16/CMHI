...
//			$this->sql = "SELECT encounter_nr FROM $this->tb_enc WHERE encounter_nr BETWEEN ".$db->qstr($ic_nr)." AND ".$db->qstr($ref_nr)." AND encounter_type=".$db->qstr($enc_class_nr)." ORDER BY encounter_nr DESC";
	...
	**/
	...
	$this->sql ="SELECT cp.senior_ID, ps.job_function_title, ps.nr AS personnelID, enc_ins.hcare_id,ins.firm_id,enc.current_ward_nr, cp.pid, enc.encounter_nr, enc.is_chargelock,
...
function AllDispositionClassesObject(){
  ...
  /**
	...
	*/
	...
	function getFinalDiagnosis($encounter_nr=0){
		global $db;

		$this->getSQL = "SELECT 
				final_diagnosis 
				FROM seg_soa_diagnosis 
				WHERE encounter_nr = ".$db->qstr($encounter_nr);

		if ($this->result = $db->Execute($this->getSQL)) {
			if ($this->result->RecordCount()) {
				return $this->result->FetchRow();
			}else{
				return false;
	...
	*/
	...
	function updateDOA($enc_nr='', $DOA=0, $DOA_reason=''){
		...
		function MayGoHome($enc_nr='', $mgh_date='0000-00-00 00:00:00', $is_mgh=1){
				...
				$this->sql="UPDATE $this->tb_enc SET\n".
					"history=CONCAT(history, " . $db->qstr($logEntry) . "),\n".
...
function GetPaidBillDate($pid, $enc){
	...
	$this->sql = "SELECT or_date FROM seg_pay AS sp
					...
					WHERE sp.`cancelled_by` = '' AND sp.`pid` = ".$pid;
	...
	if($result['or_date'])
		...
		$this->sql2 = "SELECT bill_dte AS or_date FROM seg_billing_encounter
							WHERE request_flag = 'paid' AND doctor_flag = 'paid' 
								AND is_deleted IS NULL AND encounter_nr = ".$db->qstr($enc);
...
#added by mai 09-30-2014
function getManualCoverage($encounter_nr){
	global $db;
	$this->sql = "SELECT coverage FROM seg_encounter_insurance WHERE encounter_nr = ".$db->qstr($encounter_nr)." AND hcare_id = '18'";
	$result = $db->GetRow($this->sql);
...
function updateManualCoverage($encounter_nr, $coverage){
	global $db, $HTTP_SESSION_VARS;
	$this->sql = "UPDATE seg_encounter_insurance SET coverage=".$db->qstr($coverage).
				", modify_id = ".$db->qstr($HTTP_SESSION_VARS['sess_user_name']).", modify_dt = NOW() ".
				" WHERE encounter_nr = ".$db->qstr($encounter_nr)." AND hcare_id = 18";
...
function lockCharging($encounter_nr, $islock){
	global $db;
	$this->sql = "UPDATE care_encounter SET is_chargelock = ".$db->qstr($islock).
						" WHERE encounter_nr = ".$db->qstr($encounter_nr);
...
#end added by mai
 #added by mai 09-30-2014
    public function getPhicCosts($encounter_nr, $area='', $refno=0){
    	global $db;

    	switch($area){
    		case 'lab':
    			$whereLab = " AND labserv.refno <> ".$db->qstr($refno);
    		break;
    		case 'rad':
    			$whereRad = " AND radserv.refno <> ".$db->qstr($refno);
    		break;
    		case 'pha':
    			$wherePha = " AND phaserv.refno <>".$db->qstr($refno);
    		break;
    	}

    	$this->sql = "SELECT 
						  SUM(price_charge) AS sum_charge 
						FROM
						  (
						    (SELECT 
						      'lab' AS center,
						      ...
						      labdet.price_charge 
						    FROM
						      seg_lab_servdetails labdet 
						    WHERE refno IN 
						      (SELECT 
						        labserv.refno 
						      FROM
						        seg_lab_serv labserv 
						      WHERE encounter_nr = ".$db->qstr($encounter_nr).$whereLab.") 
						      AND request_flag = 'phic') 
						    UNION
						    (SELECT 
						      'rad' AS center,
						      raddet.batch_nr,
						      raddet.price_charge 
						    FROM
						      care_test_request_radio raddet 
						    WHERE refno IN 
						      (SELECT 
						        radserv.refno 
						      FROM
						        seg_radio_serv radserv 
						      WHERE encounter_nr = ".$db->qstr($encounter_nr).$whereRad."
						        AND grant_type = 'phic')) 
						    UNION
						    (SELECT 
						      'pha' AS center,
						      phadet.refno,
						      (phadet.pricecharge*quantity) AS price_charge 
						    FROM
						      seg_pharma_order_items phadet 
						    WHERE refno IN 
						      (SELECT 
						        phaserv.refno 
						      FROM
						        seg_pharma_orders phaserv 
						      WHERE encounter_nr = ".$db->qstr($encounter_nr).$wherePha."
						        AND charge_type = 'PHIC'))
						  ) t ";
		
			if ($this->result = $db->Execute($this->sql)) {
				if ($this->result->RecordCount()){
						$row = $this->result->FetchRow();
						return $row['sum_charge'];
				}
			}
    ...
    #end added by mai
    #added by mai 10-28-2014
    function getPrincipalDiagnosis($encounter_nr){
    	global $db;

  		$this->sql= "SELECT 
					  icd.`description` 
					FROM
					  care_encounter_diagnosis d 
					  LEFT JOIN care_icd10_en icd 
					    ON icd.diagnosis_code = d.`code` 
					WHERE d.`encounter_nr` = ".$db->qstr($encounter_nr); 		
