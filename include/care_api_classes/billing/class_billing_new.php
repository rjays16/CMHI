...
class Billing extends Core {
    ...
    function getAccomodationList()
    {
        ...
        $this->sql = "select cel.encounter_nr, location_nr, cr.type_nr, ce.current_room_nr AS room, ce.current_ward_nr AS ward, cw.accomodation_type, concat(ctr.name,' (',cw.name,')') as name, ".
            ...
            "   where (cel.encounter_nr = ".$db->qstr($this->encounter_nr)." ".$filter[0].") and is_deleted = 0".
      ...
      /*added by mai 07-09-2014*/
    function getDiscounts_Serv($refno, $encounter_nr){
        global $db;
        $discount_areas = array();
        $this->sql = "SELECT 
                      ...
                      SUM(t.company_discount) AS company_discount 
                    FROM
                      (SELECT 
                        ...
                        NULL AS company_discount 
                      FROM
                        seg_billing_other_discounts discounts 
                      WHERE discounts.refno = $db->qstr($refno) 
                        AND dr_nr = '0' 
                      UNION
                      SELECT 
                        insurance.bill_area,
                        ...
                        WHERE hcare_id = '18' 
                          AND ref_no = insurance.ref_no 
                          AND bill_area = insurance.bill_area) AS phic,
                        NULL AS ar_discount,
                        NULL AS sc_discount,
                        NULL AS company_discount,
                        (SELECT 
                      ...
                      FROM
                        seg_billingcoverage_adjustment insurance 
                      WHERE insurance.ref_no = $db->qstr($refno) 
                      UNION
                      SELECT 
                        company.bill_areas,
                        NULL AS other_insurance,
                        ...
                        NULL AS ar_discount,
                        NULL AS sc_discount,
                        company.amount AS company_discount 
                      FROM
                        seg_billing_company_areas company 
                      WHERE company.encounter_nr= $db->qstr($encounter_nr) 
                        AND dr_nr = '0') t 
        ...
        if($result=$db->Execute($this->sql)){
            if($result->RecordCount()){
                while($row=$result->FetchRow()){
                    array_push($discount_areas,array('bill_area'=>$row['bill_area'],
                                            'sc_discount'=>$row['sc_discount']+$row['ar_discount'],
                                            'ar_discount'=>/*$row['ar_discount']*/0,
                                            'phic'=>$row['phic'],
                                            'other_insurance'=>$row['other_insurance']+$row['company_discount']));
                }
            }
            return $discount_areas;
        }else{
     ...
     function getDiscounts_Pf($refno, $encounter_nr){
        global $db;
        $discount_areas = array();
        $this->sql = "SELECT 
                          t.bill_areas AS bill_area,
                        ...
                        FROM
                          ...
                          WHERE insurance.bill_nr = $db->qstr($refno) 
                          UNION
                          SELECT 
                            company.bill_areas,
                            company.dr_nr,
    ...
    function getXLOList()
    {
        ...
        /*if ($this->old_bill_nr != '' && $this->is_final) {
            ...
            $cond_lab = " AND serv_dt BETWEEN DATE(".$db->qstr($this->bill_frmdte).
                               ") AND DATE(".$db->qstr($this->charged_date).")" ;
            $cond_rad =  " AND rh.request_date BETWEEN DATE(".$db->qstr($this->bill_frmdte).
                               ") AND DATE(".$db->qstr($this->charged_date).")" ;
        ...
        $this->sql = "select lh.refno, serv_dt, serv_tm, ld.service_code, ls.name as service_desc, ls.group_code, " .
                    ...
                    "   group by  md.service_code, ms.name, service_desc order by source, group_desc, service_desc";
    ...
    function getProfFeesList() {
        ...
        $strSQL = "select attending_dr_nr as dr_nr, name_last, name_first, name_middle, 'Attending Doctor' as role, ".
                    ...
                    "UNION ALL SELECT 
                                  ...
                                  1 AS num_days,
                                  SUM(spsd.chrg_amount) AS daily_rate,
                                  '' AS opcodes,
                                  SUM(spsd.`chrg_amount`) AS dr_charge,
                                  0 AS is_excluded,
                                  cpa.role_nr,
                                  'D1' AS role_area,
        ...
        $strSQL = "select dr_nr, role_area, role_type_level, opcodes, sum(num_days) as totaldays, sum(rvu) as totalrvu, (sum(multiplier * rvu)/sum(rvu)) as avgmuliplier, sum(dr_charge) as totalcharge, ".
                    ...
                    "UNION
                        ...
                        SELECT 
                          ...
                          SUM(spsd.chrg_amount) AS totalcharge,
                          SUM(spsd.`chrg_amount`) AS chrg_for_coverage,
                          'examinations' as tbl_source  
                        FROM
                          seg_pf_service_details spsd 
                          LEFT JOIN seg_pf_service sps 
...
//                      $n_pf   += ($b_noexcluded) ? $objpf->getChrgForCoverage() : $objpf->getDrCharge();
    ...
    function deleteAccommodation($data, $room_nr){
        global $db;
        ...
        if($data['accom_type'] == 'BL'){
            ...
            $this->sql = "UPDATE care_encounter_location SET is_deleted = 1 
                            WHERE is_deleted = 0 AND encounter_nr = ".$db->qstr($data['encounter_nr'])." AND location_nr = ".$db->qstr($room_nr)."
                           ";
        }
...
function savebill($data, $bill_nr, $final) {
    ...
    */
    ...
    function getDaysCount(){
        ...
        /*added bymai 09-20-2014*/
        if($admit == '1970-01-01'){
            $admit = date('Y-m-d', strtotime($this->getCaseDate($this->encounter_nr)));
        }
        //end added by mai
