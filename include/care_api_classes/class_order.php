...
class SegOrder extends Core {
	...
	function getOrderInfo($refno) {
		...
		$refno = $db->qstr($refno);
		$this->sql="SELECT o.*,\n".
				" IF(p.sex = 'f', 'Female', 'Male') AS gender,
  					fn_get_age (DATE(NOW()), p.`date_birth`) AS age,".
...
//				"IFNULL(p.name_middle,'') AS name_middle,\n".
	...
	function getPersonInfoFromEncounter($nr) {
		...
		$nr = $db->qstr($nr);
		$this->sql= "
SELECT ps.nr AS personnelID, sri.rid, enc.is_chargelock, enc.encounter_nr, cp.senior_ID, cp.fromtemp, cp.pid,cp.name_last,cp.name_first,cp.date_birth,cp.addr_zip, cp.sex,cp.death_date,cp.status,cp.street_name,
...
ORDER BY name_last ASC";
	...
	function getOrderItemsFullInfo($refno, $discountID) {
		global $db;
		$refno = $db->qstr($refno);
		$this->sql = "SELECT  spi.prescription_id, spi.`item_code`, spi.`item_name`, spi.`quantity` AS quantity_prescription, spi.`dosage`,
  						spi.`period_count`, spi.period_interval, sp.`instructions`, o.quantity,o.pricecash AS `force_price`,o.is_consigned,a.*,\n".
				...
				"LEFT JOIN seg_prescription AS sp ON sp.pharma_refno = o.refno 
  				 LEFT JOIN seg_prescription_items AS spi ON (spi.`prescription_id` = sp.`id` AND spi.`item_code` = a.`bestellnum`) \n".
				"WHERE o.refno = $refno\n".
				"UNION
					SELECT 
					  ...
					  spi.`item_code`,
					  spi.`item_name`,
					  spi.`quantity` AS quantity_prescription,
					  spi.`dosage`,
					  spi.`period_count`,
					  spi.`period_interval`,
					  sp.`instructions`,
					...
					FROM
					  ...
					  LEFT JOIN seg_prescription sp 
					    ON sp.`id` = spi.`prescription_id` 
					WHERE sp.`pharma_refno` = $refno AND spi.`item_code` = ''";
	...
	function getOrderItemsForServe($refno) {
		global $db;
		$refno = $db->qstr($refno);
		$this->sql = "SELECT a.bestellnum,a.artikelname, a.generic,\n".
...
//	}
	...
	//				break;
		...
		if(count($filters)==1){ /*added by mai 08-15-2014*/
			$phHaving[] = "count_served_items=0";
		}
