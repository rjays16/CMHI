...
#echo "getOccupation : this->sql = '".$this->sql."' <br> \n";
		...
		*/
		function getNewPIDNr($ref_nr,$personell=""){
				...
				if($personell){
					$ref_nr = '900000';
					$this->sql="SELECT pid FROM $this->tb_person WHERE pid>='$ref_nr' AND pid<'999999' ORDER BY CAST(pid as UNSIGNED) DESC";
				}
				else
					$this->sql="SELECT pid FROM $this->tb_person WHERE pid>='$ref_nr' AND pid<'900000' ORDER BY CAST(pid as UNSIGNED) DESC";
...
#                        if(defined('SHOW_FIRSTNAME_CONTROLLER')&&SHOW_FIRSTNAME_CONTROLLER){
				...
				#edited by VAN 04-16-08
						...
						$this->sql= " SELECT SQL_CALC_FOUND_ROWS rd.rid, cp.photo_filename,dep.dependent_pid, ps.nr AS personnelID, cp.civil_status, cp.senior_ID, cp.fromtemp, cp.pid, $old_select cp.name_last,cp.name_first,cp.name_middle,cp.date_birth,cp.addr_zip, cp.sex,cp.death_date,cp.death_encounter_nr, cp.status,cp.street_name,
														sb.brgy_name, sm.zipcode, sm.mun_name, sp.prov_name, sr.region_name,IF(fn_calculate_age(NOW(),cp.date_birth),fn_get_age(NOW(),cp.date_birth),age) AS age, cp.sex, cp.suffix, is_temp_bdate, enc.is_chargelock,
														SUBSTRING(MAX(CONCAT(enc.encounter_date,enc.encounter_nr)),20) AS encounter_nr,
...
#                        if(defined('SHOW_FIRSTNAME_CONTROLLER')&&SHOW_FIRSTNAME_CONTROLLER){
		...
		function SearchSelectWithCurrentEncounter($searchkey='',$maxcount=100,$offset=0,$oitem='name_last',$odir='ASC',$fname=FALSE){
				...
				$this->sql="SELECT SQL_CALC_FOUND_ROWS
														rd.rid, tbperson.pid, tbenc.is_chargelock,
...
/*added by mai 09-02-2014*/
function ListBillingMgh($maxcount=100,$offset=0, $hasOnlyFinalBill=FALSE){
	global $db;
	...
	if($hasOnlyFinalBill){
		$where = ' AND sbe.is_final = 1 AND (sbe.is_deleted !=1 OR sbe.is_deleted IS NULL)';
	}else{
		$where = ' AND tbenc.`is_maygohome` <> 0 ';
	}

	$this->sql = "SELECT 
					  SQL_CALC_FOUND_ROWS tbperson.pid,
					  SUBSTRING(
					    MAX(
					      CONCAT(
					        tbenc.encounter_date,
					        tbenc.encounter_nr
					      )
					    ),
					    20
					  ...
					  tbperson.sex,
					  sbe.bill_dte,
					  MAX(tbenc.encounter_date) AS er_opd_datetime,
					  CONCAT(
					    MAX(
					      IF(
					        admission_dt IS NULL 
					        OR admission_dt = '',
					        DATE_FORMAT(
					          encounter_date,
					          '%b %e, %Y %l:%i%p'
					  ...
					  tbperson.death_encounter_nr,
					  tbperson.street_name,
					  sb.brgy_name,
					  sm.zipcode,
					  sm.mun_name,
					  sp.prov_name,
					  sr.region_name,
					  cif.name as insurance_name,
					  SUBSTRING(
					    MAX(
					      CONCAT(scg.grant_dte, scg.discountid)
					    ),
					  ...
					  SUBSTRING(
					    MAX(
					      CONCAT(
					        tbenc.encounter_date,
					        tbenc.encounter_type
					      )
					    ),
					    20
					  ) AS encounter_type,
					  spn.refno as promi_refno
					FROM
					  care_person AS tbperson 
					  INNER JOIN care_encounter AS tbenc 
					    ON tbenc.pid = tbperson.pid 
					    AND tbenc.encounter_type NOT IN (5, 6) 
					    AND tbenc.encounter_status <> 'cancelled' 
					    AND tbenc.status NOT IN (
					      'deleted',
					      'hidden',
					      'inactive',
					      'void'
					  ...
					  LEFT JOIN seg_charity_grants scg 
					    ON scg.encounter_nr = tbenc.encounter_nr 
					  LEFT JOIN seg_barangays AS sb 
					    ON sb.brgy_nr = tbperson.brgy_nr 
					  LEFT JOIN seg_municity AS sm 
					    ON sm.mun_nr = tbperson.mun_nr 
					  ...
					  LEFT JOIN seg_billing_encounter sbe
   						ON tbenc.encounter_nr = sbe.encounter_nr
   					  LEFT JOIN seg_company_allotment sca 
   					  	ON tbenc.encounter_nr = sca.encounter_nr
    				  ...
    				  LEFT JOIN seg_encounter_insurance sein 
    				  	ON (sein.encounter_nr=tbenc.encounter_nr AND hcare_id <> 18)
					  LEFT JOIN care_insurance_firm cif 
					  	ON sein.hcare_id=cif.hcare_id
					WHERE (
					    (
					      tbenc.is_discharged = 0 
					      AND death_date IN (NULL, '0000-00-00', '')
					    ) 
					    OR (
					      tbenc.is_discharged = 0 
					  ...
					  AND tbenc.encounter_type IN (1, 2, 3, 4, 12) 
					  AND tbenc.encounter_status <> 'cancelled' 
					  AND tbenc.status NOT IN (
					    'deleted',
					    'hidden',
					  ...
					  AND IF(
					    ...
					    FROM
					      care_encounter 
					    WHERE pid = tbenc.pid) = 1,
					    tbenc.encounter_type IN (1, 2, 3, 4, 12),
					    tbenc.encounter_type IN (1, 2, 3, 4)
					  ) 
					  ".$where."
					 GROUP BY tbperson.pid 
...
#----------- added by LST 06-26-2008 to allow search using encounter_nr ---- EOB ----
				...
				if(isset($oitem)&&!empty($oitem)) $sql3 .= "ORDER BY $oitem $odir";
					 ...
					 $this->sql="SELECT SQL_CALC_FOUND_ROWS tbperson.pid, sbe.bill_dte, fn_billing_compute_net_amount(sbe.bill_nr) - sbe.total_prevpayments as total_bill, spn.refno AS promi_refno, sc.comp_name, cifm.name as insurance_name,
                        ...
                        "(SELECT CONCAT(date_format((case when tbenc.admission_dt is null or tbenc.admission_dt = '' then tbenc.encounter_date else tbenc.admission_dt end), '%b %e, %Y %l:%i%p'), ' to ', (case when tbenc.discharge_date is null or tbenc.is_discharged = '0' then 'present' else date_format(str_to_date(concat(date_format(tbenc.discharge_date, '%Y-%m-%d'), ' ', date_format(tbenc.discharge_time, '%H:%i:%s')), '%Y-%m-%d %H:%i:%s'), '%b %e, %Y %l:%i%p') end)) as prd
...
//						"CONCAT(MAX(IF(admission_dt is null or admission_dt = '', date_format(encounter_date, '%b %e, %Y %l:%i%p'), date_format(admission_dt, '%b %e, %Y %l:%i%p'))), ' to present') as confine_period, ".
						...
						"LEFT JOIN seg_company_allotment sca on tbenc.encounter_nr = sca.encounter_nr\n".
						"LEFT JOIN seg_company sc on sc.comp_id = sca.comp_id\n".
						" LEFT JOIN seg_encounter_insurance sein ON (sein.encounter_nr=tbenc.encounter_nr AND hcare_id <> 18)\n".
					  	" LEFT JOIN care_insurance_firm cifm ON sein.hcare_id=cifm.hcare_id\n".
						"LEFT JOIN seg_promissory_note spn
    						ON (spn.encounter_nr = tbenc.encounter_nr AND spn.note_status <> 'deleted')\n".
						$sql2.' '.$sql4.' '.$sql3;
...
//        $searchkey=trim($searchkey);
				...
				$this->sql="SELECT SQL_CALC_FOUND_ROWS tbperson.pid, sri.rid, tbenc.encounter_nr, admission_dt, tbperson.name_last, tbperson.name_first, tbperson.name_middle, sc.comp_name, cifm.name as insurance_name,".
								...
								" LEFT JOIN seg_company_allotment AS sca ON sca.encounter_nr = tbenc.encounter_nr\n".
								" LEFT JOIN seg_company AS sc ON sca.comp_id = sc.comp_id\n".
								" LEFT JOIN seg_charity_grants_pid AS scg ON scg.pid=tbenc.pid\n".
								" LEFT JOIN seg_barangays AS sb ON sb.brgy_nr=tbperson.brgy_nr\n".
								" LEFT JOIN seg_municity AS sm ON sm.mun_nr=tbperson.mun_nr\n".
								" LEFT JOIN seg_provinces AS sp ON sp.prov_nr=sm.prov_nr\n".
...
#exit();
    ...
    public function saveInsuranceMembershipInfo($params)
    {
        ...
        //added by maimai 12-01-2014
        $params['member_fname'] = $this->convertEncoding($params['member_fname']);
        $params['member_lname'] = $this->convertEncoding($params['member_lname']);
        $params['member_mname'] = $this->convertEncoding($params['member_mname']);
        //end added by mai
        ...
        if ($ok) {
            ...
            $db->Execute('UPDATE seg_insurance_member_info SET employer_no = '.$db->qstr($params['employer_no']).'WHERE pid = '.$params['pid'].' AND hcare_id ='.$params['hcare_id']);

            $db->Execute('UPDATE seg_insurance_member_info SET insurance_nr = '.$db->qstr($params['insurance_nr']).'WHERE pid = '.$params['pid'].' AND hcare_id ='.$params['hcare_id']);
        ...
        if($ok){
        	...
        	$sql = "SELECT * FROM seg_insurance_member_info WHERE pid = $pid AND hcare_id = $hcare_id";
      		if ($result=$db->Execute($sql)) {
	            if ($result->RecordCount()) {
	                if ($row = $result->FetchRow()) {
	                	$row['encounter_nr'] = $encounter_nr;
	                   $this->saveForEclaimsMemberInfo($encounter_nr,$row);
	                }
	            }
        	}
        }
    ...
    #added by monmon
    function getInfoFromEclaims($encounter_nr){
    	global $db;
    	$sql = "SELECT encounter_nr FROM seg_encounter_insurance_memberinfo WHERE encounter_nr = '$encounter_nr' AND hcare_id = 18";
    	$enc_nr = $db->GetOne($sql);
    	return $enc_nr;
    }
    function saveForEclaimsMemberInfo($encounter_nr,$data){
    	global $db;
    	$action = 'insert';
    	unset($data['create_date']);

    	$enc_nr = $this->getInfoFromEclaims($encounter_nr);
    	if($enc_nr){
    		$action = 'update';
    	}

    	$enc_nr = $data['encounter_nr'];
    	$pid = $data['pid'];
    	$hcare_id = $data['hcare_id'];
    	$ins_nr = $data['insurance_nr'];
    	$lname = $data['member_lname'];
    	$fname = $data['member_fname'];
    	$mname = $data['member_mname'];
    	$suf = $data['suffix'];
    	$sex = $data['sex']; #not yet available : to be added
    	...
    	$modiftdt = date('Y-m-d H:i:s');
    	$create_date = date('Y-m-d H:i:s');
    	$modfyid = $_SESSION['sess_temp_userid'];
    	$history = $this->setHistory($action,$data);
    	$attributes = array(
	                $enc_nr,
	                $pid,
			    	$hcare_id,
	...
	if($hcare_id == '18'){
    	if($action == 'insert'){
    		$sql = "INSERT INTO seg_encounter_insurance_memberinfo (
	            encounter_nr,pid,hcare_id,insurance_nr,member_lname,member_fname,member_mname,suffix,sex,
	            birth_date,street_name,brgy_nr,mun_nr,relation,member_type,employer_no,employer_name,
	            create_dt,create_id,modify_dt,modify_id,history)
	            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		    $qry = $db->Prepare($sql);
		    $result = $db->Execute($qry,$attributes);
    	}
    	else{
    		$sql = "UPDATE seg_encounter_insurance_memberinfo set
	            encounter_nr = ?,pid=?,hcare_id=?,insurance_nr=?,member_lname=?,member_fname=?,member_mname=?,suffix=?,sex=?,
	            birth_date=?,street_name=?,brgy_nr=?,mun_nr=?,relation=?,member_type=?,employer_no=?,employer_name=?,modify_dt=?,modify_id=?,history=? WHERE encounter_nr = '".$enc_nr."'";

		    $qry = $db->Prepare($sql);
		    $result = $db->Execute($qry,array(
	                $enc_nr,
	                $pid,
			    	$hcare_id,
			    	$ins_nr,
    ...
    function setHistory($action,$attr){
    	global $db;
    	$history = "";
    	$attr['is_pbef'] = 0;
    	$attr['was_temp'] = 0;
    	$attr['parent_pid'] = null;

    	$_attr = $this->formatHistoryAttr($attr);

    	if($action == 'insert'){
    		$history =  "Created by " . $_SESSION['sess_temp_userid'] . " on " .date('Y-m-d H:i:s')."\n"
                    . $_attr . "\n\n";
    	}
    	else{

    		$sql = "SELECT history from seg_encounter_insurance_memberinfo WHERE encounter_nr = '".$attr['encounter_nr']."'";
    		$history = $db->GetOne($sql);
    		$history .=  "Updated by " . $_SESSION['sess_temp_userid'] . " on " .date('Y-m-d H:i:s')."\n"
                    . $_attr . "\n\n";
    	}
    	return $history;
    }
    function formatHistoryAttr($attr){
    	$strattr = '';
    	$ar = array();
    ...
    */
    function getPersonOutstandingBill($bill_nr)
    {
	   	global $db;
	   	...
	   	$this->sql = "SELECT SUM(a.amount) amount 
						  ...
						  FROM
						    seg_pay_request spr 
						  WHERE spr.service_code = a.bill_nr 
						    AND spr.or_no = a.description)";
    ...
    //added by maimai 12-02-2014
    function getCancelledPayments($bill_nr){
    	global $db;
    	...
    	$this->sql = "SELECT 
					...
					FROM
					  seg_pay_request spr 
					  LEFT JOIN `seg_person_ledger_d` a 
					    ON (
					      a.`entry_type` = 'debit' 
					      AND description = spr.or_no 
					      AND bill_nr = spr.service_code
					    ) 
					  LEFT JOIN seg_pay sp 
					    ON sp.or_no = spr.or_no 
					WHERE spr.ref_source = 'FB' 
					  AND sp.cancelled_by <> '' 
					  AND spr.service_code = ".$db->qstr($bill_nr);
    ...
    * @return boolean
    */
    function hasBillSaveInLedger($bill_nr)
    {
	   	global $db;
	   	$bill_nr = $db->qstr($bill_nr);
    ...
    */
    ...
    public function getFullAge($birthdate, $discharge_date)
    {
        $oDateNow = new DateTime($discharge_date);
