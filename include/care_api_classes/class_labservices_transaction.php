	...
	function SearchSelect($searchkey='',$maxcount=100,$offset=0,$oitem='name_last',$odir='ASC',$fname=FALSE,$mod,$done=0, $samplelist=0, $is_doctor=0, $encounter_nr='', $source='LB', $cond='', $count_sql=0, $group_code=FALSE, $isERIP=0) {
		global $db, $sql_LIKE, $root_path, $date_format;
						...
						if(sizeof($comp)>1){
								...
								if($suchwort=='%'||$suchwort=='%%'){
										...
										if ($done){
												...
												$sql2=" WHERE r.status NOT IN ($this->dead_stat)  AND d.status <> 'deleted'";
												//added by mai 10-27-2014
												$having = "HAVING (
														    total_serve <> total_req 
														    OR (
														      r.`serv_dt` = DATE(NOW())
														    )
														  ) ";
			 ...
			 if ($mod){
						...
						if(isset($oitem)&&!empty($oitem)) $sql3 =" $sql_erip $sql_source $cond $sql_sample_gr $having ORDER BY is_urgent DESC,r.serv_dt DESC,refno DESC ";

						$this->sql= "SELECT SQL_CALC_FOUND_ROWS r.refno,  SUM(d.is_served) AS total_serve,
  COUNT(d.is_served) AS total_req, GROUP_CONCAT(d.status) AS STATUS, p.name_last,p.name_first, p.name_middle, p.pid, p.sex,
					 ...
					 if(isset($oitem)&&!empty($oitem)) $sql3 =" $sql_erip $sql_source $cond ORDER BY is_urgent DESC,r.serv_dt DESC,refno DESC ";
...
INNER JOIN seg_lab_result_groupname AS gn ON gn.group_id = gp.group_id
                ...
                if(is_numeric($suchwort)) {
                        ...
                        if(sizeof($comp)>1){
                                ...
                                if($suchwort=='%'||$suchwort=='%%'){
                                		...
                                		if($done){
                                			$sql2=" WHERE r.serve_dt = DATE(NOW()) AND r.status NOT IN ($this->dead_stat) ";
                                		}else{
                                			$sql2=" WHERE r.status NOT IN ($this->dead_stat) ";
                                		}
                                }else{
    ...
    function getLabResultPid($searchkey,$maxcount=100,$offset=0,$encounter_nr, $pid, $ref_source='LB', $count_sql){
               ...
               $this->sql = /*"SELECT SQL_CALC_FOUND_ROWS o.refno, date_received AS request_date, 
                                ...
                                ORDER BY date_received DESC"*/
                                "SELECT s.`refno`, rd.`service_code`, serv.`name`, IFNULL(gp.`group_id`,'') AS group_id, rd.`service_date`, gn.`form_id`
									FROM seg_lab_serv AS s
...
function getLabRequestsEncounter($encounter_nr){
	global $db;
	$this->sql = "SELECT 
				  service.name, details.service_code, details.refno 
				FROM
				  seg_lab_servdetails details 
				  LEFT JOIN seg_lab_serv serv 
				    ON details.refno = serv.refno 
				  LEFT JOIN seg_lab_services service 
				    ON service.service_code = details.service_code 
				WHERE serv.`encounter_nr` = ".$db->qstr($encounter_nr).
				 " AND details.is_served = 1 ";
