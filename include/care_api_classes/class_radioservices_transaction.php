...
class SegRadio extends Core {
	...
	*/	
	...
	function getRadioRequestsEncounter($encounter_nr){
		global $db;
		$this->sql = "SELECT 
					  service.name 
					FROM
					  care_test_request_radio details 
					  LEFT JOIN seg_radio_serv serv 
					    ON details.refno = serv.refno 
					  LEFT JOIN seg_radio_services service 
	...
	function getRadRes($encounter_nr){
		global $db;
		$this->sql = "SELECT 
