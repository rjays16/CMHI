...
*/
	...
	*/			
	...
	function phicReport($from, $to){
		global $db;

		$this->sql = "SELECT 
						  v.`refno`,
						  v.`encounter_nr`,
						  fn_get_person_name (v.`pid`) patient,
						  fn_get_age (v.serv_dt, p.date_birth) age,
						  i.`insurance_nr`,
						  CONCAT(
						    w.`name`,
						    ' ',
						  ...
						  ce.encounter_date,
						  fn_get_person_name (pl.pid) attending_dr,
						  GROUP_CONCAT(s.`name`) services,
						  IFNULL(
						    (SELECT 
						      GROUP_CONCAT(
						        IFNULL(
						    ...
						    FROM
						      ...
						      LEFT JOIN care_icd10_en icd 
						        ON icd.diagnosis_code = ced.code 
						    WHERE ced.encounter_nr = v.`encounter_nr` 
						    GROUP BY ced.encounter_nr),
						    ''
						  ) diagnosis 
						FROM
						  seg_lab_servdetails d 
						  LEFT JOIN seg_lab_serv v 
						    ON v.`refno` = d.`refno` 
						  ...
						  LEFT JOIN care_encounter ce 
						    ON ce.`encounter_nr` = v.`encounter_nr` 
						  LEFT JOIN care_personell pl 
						    ON pl.`nr` = ce.`current_att_dr_nr` 
						  LEFT JOIN care_ward w 
						    ON w.`nr` = ce.`current_ward_nr` 
						  LEFT JOIN care_person_insurance i 
						    ON (
						      i.`pid` = v.`pid` 
						      AND i.`hcare_id` = '18'
						    ) 
						WHERE (
						    v.`encounter_nr` IS NOT NULL 
						    OR v.`encounter_nr` <> ''
						  ) 
						  AND (
						    ce.`encounter_type` = 3 
						  ...
						  AND v.`grant_type` = 'phic' 
						  AND DATE(ce.`encounter_date`) BETWEEN ".$db->qstr($from)." 
						  AND ".$db->qstr($to)." 
						GROUP BY d.`refno` 
						ORDER BY patient ";
