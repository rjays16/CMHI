	...
	*/
		...
		$this->sql = "SELECT 
					  ...
					  IFNULL(
					    ...
					    ." WHERE trans_type = 'PAYMNT' 
					      ...
					      IF(h.is_paid_employee IS NOT NULL, (SELECT 
						    0-(SUM(d.bill_amount) - h.discount) 
						  FROM
						    seg_company_billing_d d 
						  WHERE d.comp_bill_nr = h.comp_bill_nr), 0)
					  ) AS payments 
					FROM ".
					  $this->tb_billing_h ." h 
	...
	function getBilledEmployeesTransmittal($billnr,$case_nrs){
		...
		$this->sql = "SELECT 
						  ...
						  WHERE encounter_nr = scl.`encounter_nr` 
						    ...
						    AND trans_source IN ('ACC', 'XLO', 'MED', 'MSC', 'OPR') 
	...
	function identifyCostCenterTableColumns($trans_source){
		switch($trans_source){
			...
			case 'DOC':
				$this->costcenter_table = "seg_pf_service_details";
				$this->costcenter_refnoColumn = "refno";
				$this->costcenter_chargeColumn = "chrg_amount";
				$this->costcenter_statusColumn = "";
			}	
	}
    ...
    function getSignatoryData() {
		...
		$this->sql = "SELECT 
						`fn_get_personell_firstname_last`(a.`personell_nr`) name,
						a.`signatory_position` position
						FROM
						  `seg_signatory` a 
						  INNER JOIN `seg_signatory_document` b 
						    ON a.`document_code` = b.`document_code` 
						WHERE a.`document_code` = 'comp_trans' ";
    ...
    function getOutstandingCompanyAccounts($filters, $offset=0, $rowcount=15, $bbilled=false)
    {
        ...
        if(!$bbilled){
        	...
        	$Where .= " AND scl.`trans_source` IN ('ACC', 'XLO', 'MED', 'MSC', 'OPR', 'DOC', 'PHA', 'RAD', 'LAB')";

        	#modified sql by monmon :
            $this->sql = "SELECT
            ...
            $this->sql = "SELECT
                              ...
                              sc.`comp_full_name`,
                              IFNULL((SELECT 
								      group_concat(spr.or_no)
								    FROM
								      seg_pay_request spr 
								      LEFT JOIN seg_pay sp 
								        ON sp.or_no = spr.or_no 
								    WHERE spr.service_code = scbh.comp_bill_nr 
								      AND spr.ref_source = 'COM' 
								      AND sp.cancelled_by = ''), scbh.is_paid_employee) flag 
                            FROM
                              care_encounter ce
    ...
    function getUnbilledEmployees($comp_id, $date)
    {
        ...
        $this->sql = $db->Prepare("SELECT
                                    ...
                                    FROM
                                      ...
                                      INNER JOIN `seg_company_ledger` scl
                                        ON ce.`encounter_nr` = scl.`encounter_nr`
                                        AND scl.`trans_type` = 'CHARGE' AND scl.trans_source IN ('ACC', 'XLO', 'DOC', 'MED', 'MSC', 'OPR', 'PHA', 'RAD', 'LAB')
                                    WHERE ce.`is_discharged`
                                      AND NOT EXISTS
                                      (SELECT
                                        *
    ...
    function getCompany($details){
    	...
    	$this->sql = "SELECT comp_id, comp_full_name, comp_name, comp_add, comp_phone_nr, c_type FROM seg_company ".$where." ORDER BY comp_name";
    ...
    function addCompany($details){
    	...
    	$this->sql = "INSERT INTO seg_company (comp_full_name, comp_name, comp_add, comp_email_add, 
    												comp_phone_nr, comp_ceo, comp_hr, c_type, create_id, 
    ...
    function getCompList($type=''){
    	...
    	$this->sql = "SELECT comp_id, comp_name FROM ".$this->tb_company.$where." ORDER BY comp_name ASC";
