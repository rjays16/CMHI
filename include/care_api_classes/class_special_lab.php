	...
	function SearchService($source_req='LD', $is_charge2comp=0, $compID='',$ref_source,$is_cash=1,$discountid='',$discount=0, $is_senior=0, $is_walkin=0, $sc_walkin_discount=0,$group_code,$codenum, $searchkey='',$multiple=0,$maxcount=100,$offset=0,$area=''){
		...
		if ($discountid){
			$with_disc_query = " IF(s.is_socialized=0,
														 IF((s.in_phs=1 AND '$discountid'='PHS' AND $is_cash),(s.price_cash*(1-$discount)),IF($is_cash,IF(FALSE,s.price_cash*(1-$sc_walkin_discount),s.price_cash),s.price_charge)),
														 IF($is_cash,
...
//added by poliam 03/28/2014
	...
	//added by EJ 08/19/2014
	function getPriceIncrease($service_code){
		global $db;
		...
		$this->sql ="SELECT 
					...
					FROM `seg_lab_services` slb
					LEFT JOIN `seg_increase` inc 
					ON slb.`service_code` = $service_code";
	...
	function checkIfEmployee($pid){
		global $db;

		$pid = $db->qstr($pid);
		$this->sql = "SELECT * FROM care_personell WHERE pid = $pid";
	...
	function getDiscount(){
		...
		$this->sql = "SELECT discount FROM seg_discount WHERE discountdesc = 'PHS'";
	...
	//added by EJ 09/08/2014
	function getLaboratoryFree($service_code){
		global $db;

		$service_code = $db->qstr($service_code);

		$this->sql ="SELECT * FROM seg_laboratory_free WHERE service_code = $service_code";
