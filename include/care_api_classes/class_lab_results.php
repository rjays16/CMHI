...
Class Lab_Results extends Core {
		...
		*/
		 ...
		 */
		 ...
		 function get_patient_data($refno, $group_id){
				...
				$this->sql="SELECT ls.*, cp.*, ld.*,
												...
												e.current_ward_nr, e.current_room_nr, w.name as ward_name, e.current_dept_nr, e.encounter_type,
												IF(fn_calculate_age(NOW(),cp.date_birth),fn_get_age(NOW(),cp.date_birth),age) AS age,e.er_opd_diagnosis
		...
		function getLabResult($refno, $group_id) {
				...
				$this->sql="select refno, remarks, group_id, is_confidential, DATE(service_date) AS service_date, med_tech_pid, pathologist_pid,performed_by_pid from seg_lab_resultdata where refno='$refno' AND group_id='$group_id' AND (ISNULL(status) OR status!='deleted');";
								#echo "sql = ".$this->sql;
		if ($this->result=$db->Execute($this->sql)) {
				if ($this->count=$this->result->RecordCount()){
		...
		*/
		...
		function add_lab_results ($data, $refno, $norm_type, $group_id, $service_code='')
		{
				global $db;
				$lo_normal="";
				$hi_normal="";
				...
				if(!empty($data))
				{
					for($ctr= 0; !empty($data[0][$ctr]) && $bSuccess; $ctr++)
					{
						...
						"WHERE p.param_id='".$data[4][$ctr]."' AND (p.norm_type=".$db->qstr($norm_type)." OR p.norm_type = 'none')";
		...
		*/
								#modified by cha, july 6, 2010
		function add_lab_resultdata ($refno, $group_id=0, $date, $med_tech, $pathologist, $is_confidential, $service_code='', $remarks='',$performed_by_dr)
		{
				if($service_code=='')
						$service_code=0;
				global $db;
				...
				$this->sql = "INSERT INTO seg_lab_resultdata(refno, group_id, service_code, service_date, pathologist_pid, med_tech_pid, history, modify_id, modify_dt, create_id, create_dt, status, is_confidential, remarks,performed_by_pid) VALUES('$refno', '$group_id', '$service_code', '$date', '$pathologist', '$med_tech', ".$history.", '".$_SESSION['sess_user_name']."', NOW(), '".$_SESSION['sess_user_name']."', NOW(), '', $is_confidential, ".$db->qstr($remarks).",".$db->qstr($performed_by_dr).");";
				//$error = $db->ErrorMsg();
		...
		*/
								#modified by cha, july 6, 2010
		function update_lab_results ($data, $refno, $norm_type,$group_id,$service_code='')
		{
				global $db;
				$bSuccess=TRUE;
				$db->StartTrans();
		...
		*/
		#modified by cha, july 6, 2010
		function update_lab_resultdata ($refno, $group_id, $date, $med_tech, $pathologist, $is_confidential, $service_code='', $remarks='', $performed_by_dr)
		{
				if($service_code=='')
						$service_code=0;
				global $db;
