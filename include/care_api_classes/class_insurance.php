	...
	*/
	...
	function clearInsuranceList($encounter_nr) {
        ...
        if ($bSuccess) {
        	...
        	#uncomment by monmon : 07/03/2017
...
//									WHERE simi.pid=$pid AND simi.hcare_id=$firm_id AND simi.insurance_nr='$insurance_nr' AND cp.pid IS NULL";
	...
	#-------------
	#added by monmon : 
	public function getPatientInsuranceMemberType($pid){
		global $db;
		$memtype = $db->GetOne('SELECT sm.`memcategory_id` FROM seg_insurance_member_info simi INNER JOIN seg_memcategory sm ON simi.`member_type` = sm.`memcategory_code` WHERE simi.`pid` = '.$db->qstr($pid).' AND simi.`hcare_id` = 18');
		return $memtype;
	}
	public function setPatientInsuranceMemberCategory($encounter,$member_type){
		global $db;
		$hasData = $db->GetOne('SELECT memcategory_id FROM seg_encounter_memcategory WHERE encounter_nr='.$db->qstr($encounter));

		if($hasData){
			$ok = $db->Execute('DELETE FROM seg_encounter_memcategory WHERE encounter_nr = '.$db->qstr($encounter));
			if($ok){
				$ok = $db->Execute('INSERT INTO seg_encounter_memcategory VALUES('.$db->qstr($ecnounter).','.$member_type.')');
				return $ok;
			}
		}
		else{
			$ok = $db->Execute('INSERT INTO seg_encounter_memcategory VALUES('.$db->qstr($encounter).','.$member_type.')');
				return $ok;
		}
	}
