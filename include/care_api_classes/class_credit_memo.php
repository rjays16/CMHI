...
SELECT 
	...
	CASE (r.ref_source)
		...
		WHEN 'PF' 
		    THEN 
		    (SELECT 
		     CONCAT(fn_get_person_name (cp.pid), '\n', 'Professional Fee') 
		    FROM
		      care_personell cp 
		...
		WHEN 'MISC' 
	        THEN IFNULL(
	          (SELECT 
	            CONCAT(NAME,'\n', 'Miscellaneous') 
	          FROM
	            seg_other_services AS o 
	          WHERE r.service_code = o.alt_service_code),
	          '[Deleted Item]'
	        )
		ELSE
			CONCAT('Unknown item','\\n',r.service_code)
...
";
	...
	function addMemoItems($nr, $itemArray) {
		...
		$nr = $db->qstr($nr);
		$this->useMemoDetails();
		$this->sql = "INSERT INTO seg_credit_memo_details(memo_nr,or_no,ref_source,ref_no,service_code,service_name,service_desc,quantity,price,refund_price) VALUES($nr,?,?,?,?,?,?,?,?,?)";
...
LEFT JOIN care_users AS u ON u.login_id=c.personnel WHERE c.memo_nr=$nr";
	...
	function getMemoDetails($nr) {
		...
		$nr = $db->qstr($nr);
		$this->useMemoDetails();
		$this->sql = "SELECT cm.memo_nr,cm.or_no,cm.ref_source,cm.ref_no,cm.service_code,cm.service_name,cm.service_desc,cm.quantity AS refund,cm.refund_price AS refund_price,cm.price,
IFNULL((SELECT qty FROM seg_pay_request AS pr WHERE pr.or_no=cm.or_no AND pr.ref_source=cm.ref_source AND pr.ref_no=cm.ref_no AND pr.service_code=cm.service_code),0) AS quantity,
