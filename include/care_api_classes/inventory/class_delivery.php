...
class Delivery extends Core {     
    ...
    */     
    function saveDeliveryDetail($area_code, $data, $trns_dte, $expiry_date=NULL, $serial=NULL, $lotNo = '', $fg_items = NULL) {
        global $db; 
...
//        }        
        ...
        if(!$data['is_unitperpc']){
            $unitobj->unit_id = $data['small_unit'];
            $unitobj->is_unit_per_pc = 1;
            $data['item_qty'] = $data['item_qty'] * $data['qty_per_pck'];
        }else{
            $unitobj->unit_id = $data['unit_id'];
            $unitobj->is_unit_per_pc = $data['is_unitperpc'];
        }
        
        $unit_price = $data['unit_price'] / $data['qty_per_pck']; // unit price in stock card should be per piece.

        if ($bSuccess) {
            $bSuccess = $invobj->addInventory($data["item_qty"], $unitobj, $expiry_date, $serial->serial_no, $trns_dte, $unit_price, $lotNo);
            if (!$bSuccess) $this->setErrorMsg($db->ErrorMsg());
        }
