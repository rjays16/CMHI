...
//        $db->debug = true;
        ...
        if($this->result) {
            ...
            foreach($result as $k => $val) {
                ...
                if($val['tr_code']=="RCV"){
                	$del_info = $this->getDeliveryInfo($val['tref_no'], $itemCode);
                	if($del_info){
                		$curl_obj = new Rest_Curl();
                		$supplier = json_decode($curl_obj->getSupplier($del_info['supplier_id']), true); 
                		$particulars.="\n".$supplier[1]."(Expire: ".$del_info['expiry_date'].", Lot #: ".$del_info['lot_no'].", Invoice #: ".$del_info['invoice_no'].")";
                	}
                }
...
//        die;
    ...
    function getDeliveryInfo($refno, $item_code){
    	global $db;
