...
class CaseRateClaim extends Claim
{
    ...
    protected function setCf2Node() {
        ...
        $attrs['pDischargeDate'] = $this->formatter->formatDate($encounter->discharge_date);
        $attrs['pDischargeTime'] = $this->formatter->formatTime($encounter->discharge_time);
    ...
    protected function setDiagnosisDischarge() {
        ...
        foreach ($operationDetails as $ops) {
            $attrs = array(
                'pDischargeDiagnosis' => strtoupper($ops->rvs->description)
            );
            $this->appendNode($this->diagnosis, $this->discharge, 'DISCHARGE', $attrs);
            $this->setDischargeRvsCode($ops);
        }
    }
        ...
        if ($result) {
            ...
            */
            ...
            while ($row = $result->FetchRow()) {
                // if ($this->bhousecase) { #comment out by monmon : workaround for showing doctor information
    ...
    protected function setCaseRate() {
        ...
        global $db;
        $sql = $db->Prepare("SELECT sbc.package_id, p.package, sbc.rate_type, p.case_type, sca.acr_groupid
            FROM seg_billing_encounter sbe
            ...
            AND encounter_nr = ? GROUP BY sbc.package_id ORDER BY sbc.rate_type");
