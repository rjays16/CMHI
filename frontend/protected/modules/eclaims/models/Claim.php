...
class Claim extends CareActiveRecord
{
    ...
    public function rules()
    {
            ...
            array('id,transmit_no, encounter_nr, claim_series_lhio', 'safe',
                  'on' => 'search'),
        );
    }
    ...
    {
        return array(
            'encounter'     => array(self::BELONGS_TO, 'EclaimsEncounter',
                                     'encounter_nr'),
            'transmittal'   => array(self::BELONGS_TO, 'EclaimsTransmittal',
                                     'transmit_no'),
            'details'       => array(self::BELONGS_TO,
                                     'EclaimsTransmittalDetails',
                                     array('encounter_nr', 'transmit_no')),
            'status' => array(self::HAS_ONE, 'ClaimStatus', 'claim_id'),
            'voucherCharge' => array(self::HAS_MANY, 'voucherCharge',
                                     'claim_id'),
            'person' => array(
                self::HAS_ONE,
                'EclaimsPerson',
    ...
    /**
     * Retrieves and sorts a list of models based on the current search/filter
     * conditions
     *
    ...
    public function search()
    {
        ...
        /*added by MARK April 21, 2017 | Mod Jeff 05-16-18*/
        $sql
            = "SELECT DISTINCT
                      t.id,
                      t.encounter_nr,
        ...
        return $dataProvider = new CArrayDataProvider(
            $res, array('keyField'   => 'id',
                        'id'         => 'user',
                        'sort'       => array(
                            'attributes' => array(
                                'encounter_nr', 'transmit_no', 'claim_id',
                                'claim_series_lhio', 'bill_dte', 'name_lasted',
                                                                ),
                                                            ),
                        'pagination' => array(
                            'pageSize' => 10,
    ...
    public function searchNews()
    {
        ...
        if ( ! empty($_REQUEST['Transmittal_date'])) {
            ...
            $data_eClaims->andWhere(
                "DATE(transmittal.transmit_dte) BETWEEN '$new_date' AND '$new_date'"
            );
           }
        if ( ! empty($_REQUEST['claim_series_lhio'])) {
            $data_eClaims->andWhere(
                "t.claim_series_lhio= :claim_series_lhio",
                array(':claim_series_lhio' => $_REQUEST['claim_series_lhio'])
            );
           }
        if ( ! empty($_REQUEST['patient_lastname'])) {
            $data_eClaims->andWhere(
                "person.name_last LIKE :name_last",
        ...
        if ( ! empty($_REQUEST['discharge_date'])) {
            ...
            $data_eClaims->andWhere(
                "DATE(encounter.discharge_date) BETWEEN '$new_date' AND '$new_date'"
            );
            }
        ...
        return $dataProvider = new CArrayDataProvider(
            $res, array('keyField'   => 'id',
                        ...
                        'sort'       => array(
                            'attributes' => array(
                                'encounter_nr', 'transmit_no', 'claim_id',
                                'claim_series_lhio', 'discharge_date',
                                'name_lasted',
                                                                ),
                                                            ),
                        'pagination' => array(
...
class XmlValidator
{
    ...
    public function __call($method, $args)
    {
        if ($method == "validate") {
            ...
            return call_user_func_array(
                array($this->_delegate, $method), $args
            );
        }
    }
     ...
     * @return [type]           [description]
     */
    public function onValidateError($pNo, $pString, $pFile = null,
        $pLine = null, $pContext = null
    ) {
        /**
