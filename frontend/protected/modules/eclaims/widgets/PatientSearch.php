 ...
 * @package
 */
class PatientSearch extends TbSelect2 {
    ...
    public function init() {
        $url = Yii::app()->createUrl('eclaims/patient/info');
        $defaultOptions = array(
            'ajax' => array(
                'quietMillis' => 500,
                'url' => Yii::app()->createUrl('eclaims/patient/search'),
            ...
            'placeholder' => 'Enter the search term <small>(LASTNAME, FIRSTNAME or HRN)</small>',
...
js:function(data, container, query) {
    ...
    return partial + ' <i class="fa fa-' + gender + '" style="color:' + genderColor + '"></i>'+
        '<br/>'+
        '<span class="result-age">' + data.age + '</span> '+
        '<span class="result-case-type">' + data.patientType + '</span> '+
