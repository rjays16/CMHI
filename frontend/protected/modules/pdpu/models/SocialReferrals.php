...
class SocialReferrals extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('refer_dt, refer_to, refer_diagnosis, refer_reason, refer_assessment, refer_intervention, pid, encounter_nr, name', 'required'),
	...
	public function search()
	{
		...
		$criteria->compare('t.refer_to',$this->refer_to,true);
		$criteria->compare('t.refer_diagnosis',$this->refer_diagnosis,true);
		$criteria->compare('t.refer_reason',$this->refer_reason,true);
		$criteria->compare('t.create_id',$this->create_id,true);
		$criteria->compare('t.create_dt',$this->create_dt,true);
		$criteria->compare('t.modify_id',$this->modify_id,true);
		$criteria->compare('t.modify_dt',$this->modify_dt,true);
		$criteria->compare('t.history',$this->history,true);
		$criteria->compare('ssp.mss_no', $this->mss_no, true);

		$criteria->select = 't.refer_id, t.refer_dt, t.pid, t.encounter_nr, IFNULL(ssp.mss_no, 0) AS mss_no';
		$criteria->join = 'LEFT JOIN seg_socserv_patient ssp ON t.encounter_nr = ssp.encounter_nr';

		if(!empty($_GET['SocialReferrals']['refer_dt'])) {
			$criteria->addCondition('refer_dt LIKE :refer_dt');
