    ...
    public function actionPrintTransactionHistory($caseNr)
    {
        ...
        if(!empty($transactions)) {
            foreach ($transactions as $key => $transaction) {
                ...
                $data[] = array(
                    'number' => $key+1,
                    'treatment_date' => date('F j, Y', strtotime($transaction->transaction_date)),
                    'time_admitted' => date('h:i A', strtotime($transaction->transaction_date)),
                    'time_discharged' => date('h:i A', strtotime($transaction->datetime_out)),
                );
            }
        }
