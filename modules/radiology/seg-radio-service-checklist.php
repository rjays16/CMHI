...
function populate_checklist(val, id) {
 ...
 xajax_populate_radio_checklist(date,time,day,month,year,val,area_type, id,ptype,is_cash,discountid,discount,is_senior,is_walkin,source_req,isStat,is_charge2comp,compID,encounter_nr, var_ptype);
...
function preSet(){
	...
	list_radio_sections($('radio_area').value);//added by mai 07/08/2014
	populate_checklist('','');
}
							...
							<td align="left">
								<select class="segInput" id="radio_area" name="radio_area" onchange="list_radio_sections(this.value),populate_checklist('','')" style="width:300px;height:25px;">
									...
									<?
										...
										{
											echo '<option value="'.$row['nr'].'" selected>'.$row['name_formal'].'</option>';
										}
									?>
								</select>
