	...
	function BeforeRowRender(){
		...
		if ($this->RENDERROW[2]->Text != $this->CurrentLabSection) {
			...
			if ($row = $objInfo->getAllHospitalInfo()) {
				...
				$total_w = 0;
				$this->Image($root_path.'gui/img/logos/cmhi_logo.jpg',20,8,20);
	 ...
	 # echo "<br>interval = ".$day_interval;
		...
		if ($labserv['encounter_nr']==0){
				...
				$request_name = $person['name_last'].", ".$person['name_first']." ".$person['name_middle'];
		...
		if($person['encounter_type']){
				if ($person['encounter_type']==1){
                 ...
                 $this->SetFont("Arial","B","10");
                 $this->Cell(40,4,date('F d, Y',strtotime($person['date_birth'])),$borderNo,$newLineNo,'L');
                 $this->SetFont("Arial","","8");
                 $this->Cell(15,4,'AGE : ',$borderNo,$newLineNo,'L');
                 $this->SetFont("Arial","B","10");
                 $this->Cell(30,4,$person['age'],$borderNo,$newLineNo,'L');
