...
function populateRequestListByRefNo($refno=0,$batch_nr=0,$fromSS=0, $discount=0, $discountid='',$walkin='',$enc){
        global $db;
        ...
        if ($rs){
            while($result=$rs->FetchRow()) {
                ...
                $orgprice = $pharma->getpriceradio($result['service_code']);
                $objResponse->call("initialRequestList",$orgprice,$INCPHICaddtional,$INCNONPHICaddtional,$ifhasroom,$result['service_code'],$result['dept_short_name'],
    ...
    if ($enc_nr) {
        if ($type=='phic') {
           ...
           /* $setmorethanone == '0';
            ...
            #$total_benefits = 2240;
            $covered = 0;
            $covered = $enc->getPhicCosts($enc_nr, 'rad', $nr);
            //end added by mai
...
require_once($root_path."include/care_api_classes/class_encounter.php"); //added by mai 10-03-2014
