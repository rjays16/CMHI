<?php
	...
	function saveRadioFinding($batch_nr,$findings_nr,$findings,$radio_impression,$findings_date,$r_id,$r_img,$doctor_in_charge){
		global $date_format,$HTTP_SESSION_VARS;
		$objResponse = new xajaxResponse();
		$radio_obj = new SegRadio;
		...
		if ($radio_obj->batchNrHasRadioFindings($batch_nr)){
			...
			return updateRadioFinding($batch_nr,$findings_nr,addslashes(trim($findings)),addslashes(trim($radio_impression)),$findings_date,$r_id,$r_img,$doctor_in_charge,1);
		}
		...
		$findingArray['r_id'] = $r_id; #added by mai 05-26-2014
		$findingArray['r_img'] = $r_img; #added by mai 05-29-2014
	...
	function updateRadioFinding($batch_nr,$findings_nr,$findings,$radio_impression,$findings_date,$r_id,$r_img,$doctor_in_charge,$mod=0){
		global $date_format;
		...
		if($radio_obj->saveAFinding($batch_nr,$findings_nr,stripslashes($findings),trim($radio_impression),$findings_date,$r_id,$r_img,$doctor_in_charge,'Update')){
...
#$objResponse->addAlert("radio-finding.server.php : saveAndDoneRadioFinding : batch_nr='$batch_nr'; service_date='$service_date' ");
		...
		if (updateRadioFindingStatusServiceDate($objResponse,$batch_nr,$service_date,'done',$mod)){
			...
			//added by mai 10-22-2014 For Queing
			$result_queue = new Patient_queue();
			$result_queue->details = array("ref_no"=>$batch_nr, "area"=>"radio");
			$result_queue->addResultQueue();
			//end added by mai
			
			$objResponse->addScriptCall("refreshDashlet", "_a_1_queing_dashlet_refresh"); //added by ma 11-3-2014		
...
#$objResponse->addAlert("_populateRadioFinding : inside 'if ($result)' : result : \n".print_r($result,TRUE));
	...
	include_once($root_path.'include/care_api_classes/class_patient_queue.php'); //added by mai 10-22-2014
