...
function populateRadioServiceList($date='',$time='',$day='',$month='',$year='',$area='',$area_type='',$is_cash=1,$discountid='',$discount=0,$is_senior=0,$is_walkin=1,$dept_nr,$source_req='RD',$isStat=0,$is_charge2comp=0,$compID='',$sElem,$searchkey,$page,$enc='',$sect='', $ptype=0) {
		global $db;
		...
		if ($ergebnis) {
			...
			while($result=$ergebnis->FetchRow()) {
				...
				if ($area_type){
			 	...
			 	if(($requestTime >='17' || $requestDay == "Sunday" || $is_holiday == TRUE) && $ptype == 2) {
...
#---------------------------------------------------
	...
	function populateRadioSections($dept_nr)
		{
			...
			$sql = "SELECT group_code, name FROM seg_radio_service_groups WHERE department_nr=".$db->qstr($dept_nr)." AND status != 'deleted' ORDER BY name ASC";
			$result = $db->Execute($sql);
			$options = '<option value="0">-Select Section-</option>';
			while($row=$result->FetchRow())
			{
				$options.='<option value="'.$row['group_code'].'" selected>'.$row['name'].'</option>';
			}
	...
	function populate_radio_checklist($date='',$time='',$day='',$month='',$year='',$section, $area_type='',$searchkey='', $area='',$is_cash=1,$discountid='',$discount=0,$is_senior=0,$is_walkin=1,$source_req='RD',$isStat=0,$is_charge2comp=0,$compID='',$enc='', $ptype=0)
	{
		global $db;
		$objSS = new SocialService;
		$radio_obj = new SegRadio;
							...
							if($guiDetails["name_type"]=="D")
							 {
				                ...
				                if(($requestTime >='17' || $requestDay == "Sunday" || $is_holiday == TRUE) && $ptype == 2) {
				                	$guiDetails["cash"] = ($guiDetails["cash"]*$increased_value) + $guiDetails["cash"];
				                	$guiDetails["charge"] = ($guiDetails["charge"]*$increased_value) + $guiDetails["charge"];
				                	$guiDetails["net_price"] = ($guiDetails["net_price"]*$increased_value) + $guiDetails["net_price"];
							 	}
