...
#               $name = ucwords($lname).$comma.ucwords($fname);
            ...
            $objResponse->addScriptCall("jsListRows",$sub_dept_nr, $my_count,$row['refno'],$row['rid'],$name,$gender,$date_request,$row['is_urgent'],$repeat,$paid,$or_no,$enctype,$location,$row['pid'],$row['encounter_nr'],$age_,$bdate, $row["attending_dr"], $row["services"]);
            $my_count++;
        }//end while loop
    //end if (ergebnis)
    }else{
        ...
        $tr = "<tr><td colspan=\"17\"  style=\"\">No requests available at this time...</td></tr>";
