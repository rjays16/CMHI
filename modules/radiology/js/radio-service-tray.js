...
function startAJAXSearch(searchID, page) {
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateRadioServiceList('"+date+"','"+time+"','"+day+"','"+month+"','"+year+"','"+ptype+"','"+area_type+"','"+is_cash+"','"+discountid+"','"+discount+"','"+is_senior+"','"+is_walkin+"','"+aRadioDept+"','"+source_req+"','"+isStat+"','"+is_charge2comp+"','"+compID+"','"+searchID+"','"+keyword+"',"+page+", '"+encounter_nr+"', '"+dept_sect+"','"+var_ptype+"')",100);
...
function prepareAdd_NotPackage(id,ctscan,mri) {
	...
	//if (checkRequestDetails(id)){
		...
		details.incprCePHIC = $('incprCePHIC'+id).value;
		details.incprCeNONPHIC = $('incprCeNONPHIC'+id).value;
		details.hasroom = $('hasroom'+id).value;
...
function addProductToList(phic,nonphic,hasrum,listID, id, name, grp_code, dept_name, cash, charge, sservice,net_price, available) {
