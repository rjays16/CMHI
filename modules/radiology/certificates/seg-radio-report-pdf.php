...
// Hospital Logo
//$pdf->Image($root_path.'gui/img/logos/cmhi_logo.jpg',45,10,15);
$pdf->Image($root_path.'gui/img/logos/cmhi_logo.jpg',25,10,22);
...
$pdf->SetFont($fontStyle,"", $fontSizeLabel-1);
$pdf->multiCell(50, 3 ,strtoupper($service_name), "",1,'');
...
$pdf->multiCell(65, 4 ,strtoupper($clinical_info), "",2,'');

#Dept
$pdf->setXY(130,$pdf->getY()-4);
$pdf->SetFont($fontStyle,"", $fontSizeLabel-1);
$pdf->Cell(12, 3 , 'Area :', "",0,'');
$pdf->SetFont($fontStyle,"", $fontSizeLabel-1);
$pdf->multiCell(50, 3 ,strtoupper($area), "",1,'');
...
foreach($batchNrArrayInfo as $batchNrInfo){
    ...
    if ($radio_impression_final){
        $pdf->Ln(12);
        $pdf->Cell(0, 2 , strtoupper('Impressions : '), "", 1,'');
        $pdf->Ln();
    }
    ...
    if($radio_obj->hasBatchNR($batch_nr,count($findings_array))){
       ...
       $Fdoc = $doctors_final ? mb_strtoupper(mb_convert_encoding($doctors_final, "ISO-8859-1", 'UTF-8')) : "";
    }
	...
	$pdf->Ln(3);
	$pdf->Cell(100, 10 , '', "", 0,'');
	$pdf->Cell(0, 3 , 'Result Encoded by : '.ucwords($findings_encoder).($findings_encoder ? ', RRT' : ''), "", 0,'');
	$pdf->Ln(5);
	$pdf->Cell(100, 5 , '', "", 0,'');
