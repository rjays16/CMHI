...
$img_path = $root_path.radresult_img; /*added by mai 05-29-2014*/
$thumb_path = $root_path.radresult_img_thumb; /*added by mai 06-02-2014*/
...
<script language="javascript">
		...
		function checkFindingForm(){
                ...
                $j=jQuery.noConflict(); /* added by mai 06-02-2014*/
				/*
				if (str==''){
				...
				else if(!$j.trim($F('r_id'))){ /* updated by mai 06-02-2014*/
					alert('RID should have a value');
					$('r_id').focus();
		...
		/*added by mai 08-10-2014
		functions: upload photo radiology results
		*/
		...
		function add_browse(){
				...
				var form_upload = document.getElementById('paramedit');

				for(var i=0; i<form_upload.length; i++){
					if(form_upload.elements[i].type == "file"){
						count++;
					}
				}

				var div_image = document.getElementById('result_images'); 

				var input = document.createElement('input');
				input.type = "file";
				input.name = "result_img";
				input.id = "result_img_"+count;

				input.onchange = function(){
									upload_image(div_upload, count, input);
								}
				
				var button = document.createElement('button');
				button.innerHTML = "Remove";
				button.onclick = function(){
									remove_image(div_upload, div_image);
								}

				div_upload.appendChild(input);
				div_upload.appendChild(button);

				div_image.appendChild(div_upload);
			}

			function upload_image(div_upload, count, input){
				var $j = jQuery.noConflict();
			
				var formData = new FormData();
				formData.append("result_img", $j('#result_img_'+count).get(0).files[0]);

				$j.ajax({
					type: "POST",
					url: "<?=$root_path?>modules/radiology/radio-upload-image.php",
					async: false,
	                contentType: false,
	                processData: false,
	                dataType: 'json', 
					...
					success: function(result){
						if(result.message){
							...
							href.text = "| PREVIEW - "+result.file_name;
							div_upload.id = result.file_name;
							href.onclick = function(){
												preview_image(result.file_name);
											};

							div_upload.appendChild(href);
							div_upload.removeChild(input);

							$j('#result_img_name').val($j('#result_img_name').val() + (($j('#result_img_name').val()=="") ? "" : ",") + result.file_name);	
						}
					},
			...
			function preview_image(file_name){
				window.open('<?=$img_path?>'+file_name, file_name+"-Preview","target=_blank, width=500, height=500");
			}

			function remove_image(div_upload, div_image){
				div_image.removeChild(div_upload);
				remove_filename(div_upload.id);
			}

			function remove_div_upload(remove_button){
				var div_upload = remove_button.parentNode;
				var div_image = document.getElementById('result_images');

				remove_image(div_upload, div_image);
			}

			function remove_filename(id){
				var filename = document.getElementById('result_img_name');
				var arrayOfFilename = filename.value.split(",");
				var inputResultFilename = document.getElementById('result_img_name');

				inputResultFilename.value = "";
				
				for(var i=0; i<arrayOfFilename.length; i++){
					if(arrayOfFilename[i]!=id){
						inputResultFilename.value = inputResultFilename.value + ((inputResultFilename.value =="") ? "": ",") + arrayOfFilename[i];
					}
				}
			}
		...
		function saveFinding(){
				...
				if (checkFindingForm()){
						...
						xajax_saveRadioFinding($F('batch_nr'),$F('findings_nr'),str,str2,$F('findings_date'),$F('r_id'),$F('result_img_name'),null);
		...
		function updateFinding(){
				...
				if (checkFindingForm()){
						...
						xajax_updateRadioFinding($F('batch_nr'),$F('findings_nr'),str,str2,$F('findings_date'),$F('r_id'),$F('result_img_name'),null);
                        xajax_setDoctorNr($F('batch_nr'),$F('findNR'),$F('SenDoc'),$F('JunDoc'),$F('ConDoc'));
                        addImpression();
				}
		}
...
-->
		...
		<tr align="center">
				<td  bgcolor=#dde1ec colspan=2>
						...
						<form action="<?php echo $thisfile; ?>" method="post" name="paramedit" id="paramedit" onSubmit="return checkFindingForm()" enctype="multipart/form-data">
...
?>																
																<tr>
																		<!-- added by mai 05-26-2014 -->
																		<td class="a12_b" bgcolor="#fefefe" style="padding-left:4px; color:red">RID</td>
																		<td bgcolor="#ffffee" class="a12_b">
                                                                                <input type="text" style="width: 500px;" name="r_id" id="r_id" value = "<?= $radioRequestInfo['r_id'] ?>">
																		</td>
																</tr>
																<tr>
																...
																<tr>
																		<td class="a12_b" bgcolor="#fefefe" style="padding-left:4px">Result Image</td>
																		<td bgcolor="#ffffee" class="a12_b">
																				<input id="result_img_name" name="result_img_name" value="<?=$radioRequestInfo['r_img']?>" type="hidden"/>
																				<button onclick="add_browse(); return false;">add image</button>
																				<br>
																				<div id="result_images" border="1">

																					<?php 
																						if($radioRequestInfo['r_img']){
																							$img_filename = explode(",", $radioRequestInfo['r_img']);

																							for($i=0; $i<count($img_filename); $i++){
																								echo '<div id="'.$img_filename[$i].'">';
																									echo '<button onclick="remove_div_upload(this); return false;">Remove</button>';
																									echo '<a href="javascript:void(0);" onclick="preview_image('."'".$img_filename[$i]."'".');">| PREVIEW - '.$img_filename[$i].'</a>';
																								echo '</div>';
																							}
																						  }
