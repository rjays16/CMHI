 ...
 */
 ...
 if ($_GET["src"]) {
	 ...
	 $smarty->assign('sToolbarTitle',"Billing Main::List of Patients");
...
if (!$_POST["applied"]) {
		if (isset($_SESSION["filteroption"])) {
				if (isset($_SESSION["filteroption"][0])) $_REQUEST["chkspecific_"] = strcmp($_SESSION["filteroption"][0], 'true') == 0;
		...
		if (isset($_SESSION["filtertype"])) {
				...
				if (is_null($_SESSION["filteroption"])) {
					$_REQUEST['chkspecific_'] = true;
				}

				$_REQUEST["selrecord"] = "inpatient";
		}
}
...
if ($_REQUEST['chkspecific_']) {
		switch(strtolower($_REQUEST["selrecord"])) {
				case "inpatient":
						$filters["INPATIENT"] = $_REQUEST["inpatient"];
				break;
				case "erpatient":
						$filters["ERPATIENT"] = $_REQUEST["erpatient"];
				break;
				case "outpatient":
						$filters["OUTPATIENT"] = $_REQUEST["outpatient"];
				break;
		}
}
...
if ($_REQUEST['chkspecific']) {
		switch(strtolower($_REQUEST["selrecordby"])) {
				case "name":
				...
				break;
				case "encounter_nr":
						$filters["encounter_nr"] = $_REQUEST["encounter_nr"];
				break;
		}
}
/*end added by mai*/
										...
										<table width="100%" border="0" cellpadding="2" cellspacing="0">
												<tr>
														<td width="50" align="right">
															<input type="checkbox" id="chkspecific_" name="chkspecific_" onclick="selrecordOnChange(); keepFilters(0);emptier(); disableSearch();" <?= ($_REQUEST['chkspecific_'] ? 'checked' : '') ?>/>
														</td>
														...
														<td>
																...
																<select class="jedInput" name="selrecord" id="selrecord" onchange="selrecordOnChange(); keepFilters(0);emptier();disableSearch();"/>
																		<option value="inpatient" <?= $_REQUEST["selrecord"]=="inpatient" ? 'selected="selected"' : '' ?>>Inpatient</option>
																		<option value="erpatient" <?= $_REQUEST["selrecord"]=="erpatient" ? 'selected="selected"' : '' ?>>ER Patient</option>
																		<option value="outpatient" <?= $_REQUEST["selrecord"]=="outpatient" ? 'selected="selected"' : '' ?>>Out Patient</option>
													...
													<td width="50" align="right">
														...
														<td>
																...
																<select class="jedInput" name="selrecordby" id="selrecordby" onchange="selrecordOnChange(); keepFilters(0);emptier();disableSearch();"/>
																		<option value="name" <?= $_REQUEST["selrecordby"]=="name" ? 'selected="selected"' : '' ?>>Name</option>
																		<option value="pid" <?= $_REQUEST["selrecordby"]=="pid" ? 'selected="selected"' : '' ?>>HRN</option>
																		<option value="encounter_nr" <?= $_REQUEST["selrecordby"]=="encounter_nr" ? 'selected="selected"' : '' ?>>Case No.</option>
                                                                        
																</select>
																<td>
																<span>
																		<input class="jedInput" name="keyword" id="keyword" onblur="keepFilters(0);" onkeyup="disableSearch();" type="text" size="30" value="<?= $_REQUEST['keyword'] ?>"/>
																</span>	
														</td>
												</tr>
												<!-- end added by mai -->
