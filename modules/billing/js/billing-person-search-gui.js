...
function prepareSelect(id, billdate, billnr, ddate, f_ddate, age, phic_nr, type, totalbill, bill_dte, comp_name, insurance_name) {
		var nr = $('nr'+id).value;
        ...
        window.parent.$('admission_dte').value = formatDate(new Date(getDateFromFormat(adm_dt, 'NNN d, yyyy hh:mma')), 'yyyy-MM-dd HH:mm:ss');
		...
		if(window.parent.$('refno_promi')){
			if(promi_refno){
				window.parent.$('refno_promi').value = promi_refno;
				window.parent.$('spanBtnSave').innerHTML = "Update";
				window.parent.$('mode_of_promi').value = 'edit';
				window.parent.getData(promi_refno);
			}
		}

		if(window.parent.$('comp_name')){
			window.parent.$('comp_name').value=comp_name;
		}

		if(window.parent.$('insurance_name')){
			window.parent.$('insurance_name').value=insurance_name;
		}
		...
		switch(jumpType) {
				...
				case PREV_PAGE:
						...
						if (bEncountersList)
							...
							startAJAXSearch('searchkey',currentPage-1);
				break;
				case NEXT_PAGE:
						...
						if (bEncountersList)
							...
							startAJAXSearch('searchkey',parseInt(currentPage)+1);
				break;
				case LAST_PAGE:
						if (currentPage >= lastPage) return false;
						if (bEncountersList)
							...
							startAJAXSearch('searchkey',lastPage);
				break;
		}
}
...
function addPerson(listID, phic_nr, id, pname, dob, ddate, f_ddate, c_prd, c_class, c_type, sex, addr, status, nr, type, ad, bill_dte, totalbill, promi_refno, comp_name, insurance_name, age, billdate, billnr, indx) {
...
//		var bnoshow = ((typeof dschrg_dt == 'undefined') || (typeof dschrg_tm == 'undefined')) ? 0 : 1;
		...
		if (list) {
				...
				if (id) {
						...
						if(bnoshow){ 
							encfxn =  " prepareSelect('"+id+"', '"+billdate+"', '"+billnr+"','"+ddate+"','"+f_ddate+"','"+age+"','"+phic_nr+"',"+type+",'"+totalbill+"','"+bill_dte+"','"+comp_name+"','"+insurance_name+"');";
						}else{
							encfxn =  " prepareSelect('"+id+"','','','"+ddate+"','"+f_ddate+"','"+age+"','"+phic_nr+"',"+type+",'"+totalbill+"','"+bill_dte+"','"+comp_name+"','"+insurance_name+"')";
						}
