...
function getCompanyList($type=''){ //maimai 01-21-2015
	$objResponse = new xajaxResponse();
	$comp_obj = new Company();

	$result = $comp_obj->getCompList($type);

	$objResponse->assign("comp_name","innerHTML", ""); //empty option value

	$objResponse->call("appendCompname", 0, "ALL");
	while($row = $result->FetchRow()){
		$objResponse->call("appendCompname", $row['comp_id'], strtoupper($row['comp_name']));
	}
