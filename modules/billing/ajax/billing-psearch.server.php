<?php
    ...
    function populatePersonList($sElem,$searchkey,$page,$include_firstname,$include_encounter=TRUE,$scase_no = '', $hasOnlyFinalBill=FALSE) {
        ...
        if(trim($searchkey) || trim($scase_no)){ //added by mai 09-02-2014
            $ergebnis=$person->SearchForBilling($searchkey,$maxRows,$offset,"name_last","ASC",$include_firstname, $scase_no, $hasOnlyFinalBill);
        }else{
            $ergebnis=$person->ListBillingMgh($maxRows,$offset, $hasOnlyFinalBill);
        }
...
//                    $enc_dt, $result["parent_encounter_nr"]);
                ...
                $objResponse->addScriptCall("addPerson","person-list",
                    ...
                    $confinetyp, $result["sex"],$addr, $result["status"], $result["encounter_nr"], $result["encounter_type"], $enc_dt, $result["bill_dte"],number_format($result['total_bill'],2), $result['promi_refno'], $result["comp_name"], $result["insurance_name"], $age);
            }
        }
...
#                    $admission_dt = @formatDate2Local($rowEnc["admission_dt"], $date_format);
                ...
                $objResponse->addScriptCall("addPerson","person-list",
                    ...
                    $enc_dt, $result["bill_dte"],number_format($result['total_bill'],2), $result['promi_refno'], $result['comp_name'], $result["insurance_name"], $age, $result["bill_dte"], $result["bill_nr"], $j++);
            }
        }
        if (!$rows) $objResponse->addScriptCall("addPerson","person-list",NULL);
        if ($sElem) {
