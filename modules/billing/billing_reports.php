...
$smarty->assign('sOnLoadJs','onLoad="setCompList();"');
...
function setCompList(){
	var type = $("careof_type").value;
	xajax_getCompanyList(type);
}

function appendCompname(id, name){
	$j('#comp_name').append('<option value="'+id+'">'+name+'</option>');
}
	...
	$j('#print_params').dialog({
		...
		buttons:{
			Print:function(){
				    ...
				    }else if(selected_report =="care_of"){
				    	params = "&date_from="+$('care_of_from').value+"&date_to="+$('care_of_to').value+"&type="+$("careof_type").value
				    					+"&comp_id="+$("comp_name").value+"&has_fb="+$("has_fb").checked;
				    	rep_script = "care_of.php";
				    }else{
				    	month = $('acr_month').value;
				    	year = $('acr_year').value;
				    	params = "month=" + month + "&year=" + year;
		...
		<tbody>
			...
			<tr>
				<td nowrap="nowrap" align="right" class="segPanel">
						...
						<tr>
							<td width="20%" align="center" nowrap="nowrap"><strong>Select Report Type</strong>
								<select class="segInput" name="selreport" id="selreport" onchange="selOnChange()"/>
									...
									<optgroup label="List of Patients with Care of">
										<option value="care_of">List of Patients with Care of</option>
									</optgroup>
								</select>
							</td>
						</tr>
					...
					<!--  -->
					<!-- List of Patients with Care of 01-21-2015 -->
					<table name="selOptions" width="100%" border="0" cellpadding="2" cellspacing="0" segOption="care_of" style="display:none">
						<tr>
							<td align="right">Type:</td>
							<td align="left" nowrap="nowrap">
								<select class="segInput" id="careof_type" onchange="setCompList();" name="param" paramName="formatD" segOption="care_of">
						...
						<tr>
							...
							<td align="left" nowrap="nowrap">
								<select class="segInput" id="comp_name" name="param" paramName="formatD" segOption="care_of">
								</select>
							</td>
						</tr>
						<tr>
							...
							<td nowrap="nowrap" align="left">
								<input class="segInput" name="param" id="care_of_from" type="text" size="12" value="" paramName="date" segOption="care_of"/>
								<img src="<?= $root_path ?>gui/img/common/default/show-calendar.gif" id="tg_care_of_from" align="absmiddle" style="cursor:pointer;"  />
								<script type="text/javascript">
									Calendar.setup ({
										inputField : "care_of_from", ifFormat : "<?= $phpfd ?>", showsTime : false, button : "tg_care_of_from", singleClick : true, step : 1
									});
								</script>

								to

								<input class="segInput" name="param" id="care_of_to" type="text" size="12" value="" paramName="date" segOption="care_of"/>
								<img src="<?= $root_path ?>gui/img/common/default/show-calendar.gif" id="tg_care_of_to" align="absmiddle" style="cursor:pointer;"  />
								<script type="text/javascript">
									Calendar.setup ({
										inputField : "care_of_to", ifFormat : "<?= $phpfd ?>", showsTime : false, button : "tg_care_of_to", singleClick : true, step : 1
									});
								</script>
							</td>
						</tr>
						<tr>
							<td align="right">Has Final Bill: </td>
							<td align="left" nowrap="nowrap">
								<input type="checkbox" id="has_fb" name="param" paramName="formatD" segOption="care_of"/>
							</td>
						</tr>
					</table>
					<!-- end care of -->
