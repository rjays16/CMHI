...
if ($patient_enc['hcare_id']==18){
	$ins_result = $person_obj->getMemberInsuranceInfo($patient_enc['pid'], 18);
	$phic_membership = $ins_result['insurance_nr'];
}else{
	$phic_membership = "NON-MED";
}
...
$smarty->assign('LDCompany','Company :');
$smarty->assign('sComp',$comp_obj->getCompanyName($encounter_nr));

$smarty->assign('LDAdmPhysician','Admitting Physician :');
$smarty->assign('sAdmPhysician',$patient_enc['consulting_dr']);
/*end added by mai*/
...
if($mode=='show'){
	...
	if (($encounter_type_a==3)||($encounter_type_a==4)||($encounter_type_a == 2))
...
<?php
	...
	$AMPM = date("A",strtotime($patient_enc['discharge_time']));
		if ($AMPM == 'AM') {
			$smarty->assign('sAM','selected');
			$smarty->assign('sPM','');
		}else{
			$smarty->assign('sAM','');
			$smarty->assign('sPM','selected');
		}
	#set format time onkeyup event
	$smarty->assign('sFormatTime','value="'.date("h:i",strtotime($patient_enc['discharge_time'])).'" onChange="setFormatTime(this,\'selAMPM\')"');   # burn added : June 6, 2007
