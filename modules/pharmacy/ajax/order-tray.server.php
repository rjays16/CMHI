...
//added by julius-02-07-2017
	function populateProductList($sElem,$page,$keyword,$pid=NULL,$discountID=NULL,$area=NULL,$enc,$disable_qty=false) {
		...
		$encounter = $enc=="null" ? $pc->getencounter($pid) : $enc;
		$insurance_type = $pc->getinsrnceTyp($encounter);
		$room_types=$pc->getRoomTypes($encounter);	
		if(!empty($encounter))
		{
			//added by julius 02-07-2017
				$INCPHICaddtional =".".$pc->insurancePHIC($room_types,$insurance_type);
				$INCNONPHICaddtional = ".".$pc->insuranceNONPHIC($room_types);
				$Ifhasrooms = $room_types ? 1 : 0;	
...
//				}
				...
				$details->priceincreaseperoomPHIC = $INCPHICaddtional;
				$details->priceincreaseperoomNONPHIC = $INCNONPHICaddtional;
				$details->Ifhasroom = $Ifhasrooms;
				//end
				$details->origchrgprice = $result["chrgrpriceppk"];
				$details->origcashprice = $result["dprice"];
				$details->origcashpricesc = $result["cashscprice"];
				$details->origchargepricesc = $result["chargescprice"];
