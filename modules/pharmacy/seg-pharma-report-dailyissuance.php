...
*/
	...
	function RepGen_Pharma_Issuance ($area, $date, $encoder, $shift_start, $shift_end, $full) {
		...
		# 165
		$this->colored = FALSE;
		$this->ColumnWidth = array(28,124,22,22);
		$this->RowHeight = 6;
		$this->Alignment = array('L','L','R','R');
	...
	function Header() {
		...
		$this->Image($root_path.'gui/img/logos/cmhi_logo.jpg',40,8,15);
...
#			exit;
	...
	function FetchData() {		
		...
		if ($this->date) {
			$whereOrder[]="DATE(oi.serve_dt)='$this->date'";
			...
			if (is_numeric($this->shift_start) && is_numeric($this->shift_end)) {
				if ($this->shift_start != $this->shift_end) {
					...
					$whereOrder[] = "oi.serve_dt>='".date("YmdHis",$start_time)."' AND oi.serve_dt<='".date("YmdHis",$end_time)."'";
			...
			elseif (is_numeric($this->shift_start)) {
				$start_time = $dTime + $this->shift_start*3600;
				$whereOrder[] = "oi.serve_dt>='".date("YmdHis",$start_time)."' AND oi.serve_dt<=NOW()";
		...
		if ($this->encoder) {
			$whereOrder[]="oi.serve_id=".$db->qstr($this->encoder);
...
(SELECT 
    SUM(quantity) 
  FROM
    seg_pharma_order_items AS oi 
  WHERE oi.serve_status = 'S' 
    AND oi.bestellnum = p.`bestellnum` $sqlOrder GROUP BY oi.`bestellnum`) AS total_issued,
(SELECT SUM(ri.quantity) FROM seg_pharma_return_items AS ri LEFT JOIN seg_pharma_returns AS r ON r.return_nr=ri.return_nr
WHERE ri.bestellnum=p.bestellnum $sqlReturn GROUP BY ri.bestellnum) AS total_returns
FROM care_pharma_products_main AS p\n";
		
		if (strtolower($this->full) != "yes") $sql.= "HAVING total_issued>0 OR total_returns>0\n";
