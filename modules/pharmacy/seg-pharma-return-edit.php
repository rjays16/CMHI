...
$comp_obj = new Company(); //added by mai 07-21-2014
...
if (isset($_POST["submitted"]) && !$_REQUEST['viewonly']) {
    ...
    try {
        if ($return->save()) {
            ...
            $_POST['trans_amount'] = array_sum($_POST['refund']);
            $_POST['grant'] = "company";
            $_POST['trans_type'] = "CRMEMO";
            $_POST['trans_source'] = "PHA";
            $_POST['refno'] = $returnID;
    
           if($comp_obj->saveChargetoCompanyTransaction($_POST)){
            //success
           }else{
                $smarty->assign('sysErrorMessage', '<strong>Saving Failed</strong>');
                exit;
           }
            //end added by mai
...
# Render form values
$smarty->assign('sReturnNr', '<input class="segInput" type="text" id="return_nr" name="return_nr" size="15" value="'.($returnID ? $returnID : $return->generateReturnId()).'" readonly="readonly"/><input type="hidden" name="comp_id" id="comp_id" value="'.$_POST['comp_id'].'">');
