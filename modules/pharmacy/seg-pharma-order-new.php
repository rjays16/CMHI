...
$pharmaprod = new SegPharmaProduct();
		...
		if($total == 0){
			if($_POST['save_prescription']){
				...
				if(!$saveok){
					...
					$smarty->assign('sItems','<tr><td colspan="10" class="segPanel3">Successfully Added Prescription</td></tr>');
	...
	//		$smarty->assign('sRemarks',$_REQUEST['comments']);
			...
			if ($itemsResult) {
				$oRows = "";
			 	...
			 	//Added by EJ 10/09/2014
				require_once($root_path . 'include/care_api_classes/inventory/class_inventory_helper.php');
				$invHelper = new InventoryHelper();
				if ($info['serve_status'] == 'S') {
					$invHelper->removeStock($oItem['bestellnum'], $oItem['quantity'], $info['pharma_area'], $Ref, SALE);
				}
...
?>
	...
	function openOrderTray() {
		...
		var area = $('area').value;
		var pid = $('pid').value;
		var enc = $('encounterset').value; //added by julius 02-07-2017
        var url = 'seg-order-tray.php?pid='+pid+'&area='+area+'&d='+discount+'&enc='+enc;
        var goadd = true;
...
/*        }else{
	...
	function validate() {
		...
		if(confirm('Process this pharmacy request?')){
			send_custom_notif('_a_1_queing_pharma_req_queue', 'Incoming Pharmacy Request for '+$('ordername').value); //added by mai 10-23-2014
			return true;
		}else{
			return false;
		}
...
if ($_REQUEST['encounterset']) {
	...
	$vitals = $vitalsign_obj->get_latest_vital_signs($person['pid'], $person['encounter_nr']); //added by mai 11-19-2014
}
...
if (isset($_POST["submitted"]) && !$saveok) {
	$smarty->assign('sOrderItems',"
				<tr>
					<td colspan=\"9\">Order list is currently empty...</td>
				</tr>");
...
//		$addr.=" ".$person["prov_name"];
	...
	$_POST['age'] = $person['age'];
	$_POST['weight'] = $vitals['weight'].' kg.';
	$_POST['gender'] = strtolower($person['sex']) == 'f' ? 'Female' : 'Male';
}
...
while($row=$prod->FetchRow()){
	$checked=strtolower($row['area_code'])==strtolower($_GET['area']) ? 'selected="selected"' : (strtolower($row['area_code'])=='ph' ? 'selected="selected"' : ""); //updated by mai 08/11/2014
...
<option value="CMAP">CMAP</option> -->
                                            ...
                                            <input type='hidden' id='is_cash'/>"); //updated by mai 07-18-2014
...
//added by EJ 09/08/2014
$is_employee = $splab->checkIfEmployee($_GET['pid']);
$is_dependent = $pharmaprod->checkDependent($_GET['pid']);

if ($is_employee == TRUE || $is_dependent == TRUE) {
 	$discount_id = 'PHS';
 } 

$smarty->assign('sOrderEncID','<input id="pid" name="pid" type="hidden" value="'.$_POST["pid"].'"/>');
$smarty->assign('sOrderDiscountID','<input id="discountid" name="discountid" type="hidden" value="'.$discount_id.'"/>');
...
$smarty->assign('sWeight', '<span id="weight">'.$_POST['weight'].'</span>');
$smarty->assign('sAge', '<span id="age">'.$_POST['age'].'</span>');
$smarty->assign('sSWClass',($discount_id ? $discount_id : 'None'));
