...
include_once($root_path.'include/care_api_classes/inventory/class_sku_inventory.php');
	...
	/*added by mai 09-23-2014*/
	require_once($root_path.'include/care_api_classes/class_personell.php');
	$personell_obj = new Personell();
	/*end added by mai*/
	
	//added by mai 10-11-2014
 ...
 $smarty->assign('sWindowTitle',"Pharmacy::Serve request");
	...
	$check = $order_obj->checkIfCashTrans($Ref, $itemArray);
...
$pers_info = '';
if($_POST['pid']){
	$pers_info = $pers_obj->getAllInfoArray($_POST['pid']); 
	switch($pers_info['sex']){
		case 'f':
			$pers_info['sex'] = 'Female';
			break;
		case 'm':
			$pers_info['sex'] = 'Male';
			break;
...
$smarty->assign('sAge', '<input class="segInput" type="text" size="33" style="font:bold 12px Arial;" readonly value="'.$pers_info['age'].'"'); //added by mai 10-11-2014
$smarty->assign('sGender', '<input class="segInput" type="text" size="33" style="font:bold 12px Arial;" readonly value="'.$pers_info['sex'].'"'); //added by mai 10-11-2014
...
$smarty->assign('sComments','<textarea class="segInput" name="comments" cols="16" rows="2" style="float:left; margin-left:5px; font-size:12px; font-weight:bold; font-style:italic; color:red;" '.$readOnlyAll.'>'.$_POST['comments'].'</textarea>');
...
//			}
		...
		<tr".($toggle?"":" class=\"alt\"").">
			...
			<td>
				<span style='font:bold 12px Arial;color:#000066'>{$row["artikelname"]}</span><br>
				<div style='font:normal 11px Arial; color:#404040'>
				{$row["generic"]}
				</div>
			</td>
