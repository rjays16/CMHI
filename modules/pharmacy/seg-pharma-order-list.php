...
function addRow(details) {
	...
	if (list) {
		...
		if (dBody) {
			...
			if (details["FLAG"]=="1") {
				if(details["is_cash"] == ''){ //added by mai 10-21-2014
					var options = 
					'<a title=\"Print Prescription\" onmouseover="tooltip(this.title)" onmouseout="nd()"><img class=\"segSimulatedLink\" src=\"<?= $root_path ?>images/cashier_edit.gif\" border=\"0\" align=\"absmiddle\" style=\"margin:1px;\" onclick=\"printPrescription(\''+details["encounter_nr"]+'\', \''+details["prescription_id"]+'\');\"/></a>';
				}else{
					var options = 
...
$smarty->assign('sPrescription', '<input type="checkbox" id="is_prescription" name="is_prescription" '.($_REQUEST['is_prescription'] ? 'checked="checked"' : '').'/>');
