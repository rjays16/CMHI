...
function enableSearchBtn(enable){ 	/*added by mai 08-15-2014*/
	$('searchButtonPharma').disabled = true;

	if(enable){
		$('searchButtonPharma').disabled = false;
	}
}
...
}else{ /*added by mai 08-15-2014*/
	$_REQUEST['chkserve'] = 'on';
	$_REQUEST['selserve'] = 'N';
}
...
function seldateOnChange() {
	...
	disableNav()
						...
						<tr>
							<td align="right"><input type="checkbox" id="chkserve" name="chkserve" <?= ($_REQUEST['chkserve'] ? 'checked="checked"' : '') ?>  onclick="enableChildren($('tdServe'),this.checked); enableSearchBtn(this.checked);"/></td>
						...
						<tr>
							<td></td>
							<td colspan="2">
								<input id="searchButtonPharma" type="submit" value="Search"  class="segButton"/>
							</td>
						</tr>
					</table>
				</td>
