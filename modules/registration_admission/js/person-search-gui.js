...
function prepareSelect(id) {
    ...
    var is_chargelock = $('is_chargelock'+id).value; //added by mai 10-01-2014
    var gender_field = window.parent.$('gender'); //added by mai 11-13-2014
    var age_field = window.parent.$('age'); //added by mai 11-13-2014
	...
	if(grant_type && is_chargelock != 1){
		...
		if(has_insurance && has_insurance != 'None'){
			window.parent.$("iscash0").checked = true;
			window.parent.$("iscash1").checked = false;
	...
	if(window.parent.$('is_chargelock')){
		if(is_chargelock == 1){
			window.parent.$('is_cash').value = 1;
			window.parent.$('is_chargelock').value = 1;
			window.parent.$('iscash0').checked = false;
			window.parent.$('iscash1').checked = true;
			window.parent.$('iscash0').disabled = true;
...
function addPerson(listID, details) {
	...
	var id=details.id,
			is_chargelock=details.is_chargelock,
			lname=details.lname,
			fname=details.fname,
			mname=details.mname,
			dob=details.dob,
			...
			rowSrc = '<tr>'+
									'<td>'+
                                        ...
                                        '<input type="hidden" id="category'+id+'" value="'+category+'">'+
                                         '<input type="hidden" id="is_chargelock'+id+'" value="'+details.is_chargelock+'">'+ // added by mai 10-01-2014
