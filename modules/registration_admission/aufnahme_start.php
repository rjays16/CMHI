							...
							if($update || $encounter_nr)
						{
							...
							$itemno=$itemname;
								...
								#-----------------------------------------------
									...
									{
									//added by mai 10-13-2014 - for queing
										if($HTTP_POST_VARS['encounter_type'] == 2){
												$queue_obj->details = array('encounter_nr' => $encounter_nr,
																				'dr_nr'=> $HTTP_POST_VARS['current_att_dr_nr'], 'dept_nr'=>$HTTP_POST_VARS['current_dept_nr']);
												if($queue_obj->actionQueue()){
											 ...
											 ||($_POST['vital_rr'])||($_POST['vital_pr'])){
																				if($db->Affected_Rows() > 0) {
																						$dataVital = array(
																								...
																								'height_ft'=> $_POST['vital_hf'],
																								'height_in'=> $_POST['vital_hi']
																						);
							...
							$encounter_obj->setDataArray($HTTP_POST_VARS);
									...
									if($bSuccess = $encounter_obj->insertDataFromInternalArray())
									{
										...
										//added by mai 10-13-2014 - for queing
										if($HTTP_POST_VARS['encounter_type'] == 2){
												$queue_obj->details = array('encounter_nr' => $HTTP_POST_VARS['encounter_nr'],
																				'dr_nr'=> $HTTP_POST_VARS['current_att_dr_nr'], 'dept_nr'=>$HTTP_POST_VARS['current_dept_nr']);
												if($queue_obj->actionQueue()){
										...
										#edited by VAN 10-13-09
										 if ((($_POST['vital_bp_sys'])&&($_POST['vital_bp_dias']))||($_POST['vital_t'])||($_POST['vital_w'])
													||($_POST['vital_rr'])||($_POST['vital_pr'])||($_POST['vital_hf'])||($_POST['vital_hi'])){
															$dataVital = array( 
																								'encounter_nr' => $HTTP_POST_VARS['encounter_nr'],
																								'date' => date('Y-m-d H:i:s'),
																								'pid' => $pid,
																								...
																								'modify_id' => $_SESSION['sess_temp_userid'],
																								'modify_dt' => date('Y-m-d H:i:s'),
																								'height_ft'=> $_POST['vital_hf'], //added by mai 05-24-2014
																								'height_in'=> $_POST['vital_hi'] //added by mai 05-24-2014
																						);
...
<script type="text/javascript" src="<?=$root_path?>js/wsnotif.js"></script>
...
function ajxGetBedRoom(nr_bed, patient, patient2, patient_is_mgh, locked_beds){ /* updated by mai added patient is may go home and locked beds param 07-04-2014*/
	var d = document.aufnahmeform;
	var aWardNr = d.current_ward_nr;
	var aRoomNr = d.current_room_nr;
	...
	var room = "<?php echo $current_room_nr; ?>";
	var ward = "<?php echo $current_ward_nr; ?>"; /*added by mai 07-4-2014*/
	...
	if (dBody) {
		...
		if (nr_bed!=0) {
			for (i=1; i <= nr_bed; i++){
				...
				if ((document.getElementById('bed_nr').value==i)&&(aRoomNr.value==room) && (aWardNr.value==ward)){ /*updated by mai added check ward number 07-04-2014*/
				...
				if(patient[aRoomNr.value][i]){
					if (patient[aRoomNr.value][i].toLowerCase()=='m'){
						//sex = 'Male Patient';
						sex = '<img src="../../gui/img/common/default/spm.gif" border="0"> Occupied';
						if ((document.getElementById('bed_nr').value==i)&&(aRoomNr.value==room)&&(aWardNr.value==ward))
					...
					}else if (patient[aRoomNr.value][i].toLowerCase()=='f'){
						//sex = 'Female Patient';
						sex = '<img src="../../gui/img/common/default/spm.gif" border="0"> Occupied';
						if ((document.getElementById('bed_nr').value==i)&&(aRoomNr.value==room)&&(aWardNr.value==ward))
							vacant = '<input type="radio" id="current_bed_nr" checked name="current_bed_nr" onClick="getCurrentBed(this.value);" value="'+i+'"">';
						...
						if(jQuery.inArray(num.toString(), locked_beds)>=0){
							sex = '<img src="../../gui/img/common/default/patdata.gif" border="0"><span style="color:red"> Locked</span>';
							vacant = '<img src="../../gui/img/common/default/padlock.gif" border="0"> ';
						/*end added by mai*/
						}else{
					...
					/*added by mai 07-04-2014*/
					if(patient_is_mgh[aRoomNr.value][i] == 1){
						mgh = 'MGH';
					}
					/*end added by mai*/
				}else{
				...
				newRowSrc = '<tr>' +
									...
									'<td align="center"><span style="color:red">'+mgh+'</span></td>'+ /*added by mai 07-04-2014*/
								'</tr>';
	...
	*/
	...
	shortcut("F2",
		function(){
			...
			if (ans){
				checkPreviousTrxn(1);
					$('bsave').innerHTML = '<a href="javascript:void(0);"><img id="saveButton" name="saveButton" src="../../gui/img/control/default/en/en_bsav" border=0  title="<?=$LDSaveData?>"  align="absmiddle" onclick="gotoCheck();"></a>';
			}else
    ...
    function submitForm(){
        if (chkform(document.aufnahmeform)){
        	//insert script for refreshing dashlet here
	...
	if($GLOBAL_CONFIG['patient_name_middle_show']&&$name_middle){
		...
		if($blood_group){
			...
			$smarty->assign('blood_group',$$buf);
				...
				$rowvitaldataResult = $vitals_obj->getLatesVitalDetailsbyPid($pid,$encounter_nr);
												...
												<tr>
														...
														<td width="45%" align="right" valign="top">
																<table>
																		...
																		<tr>
																				<td width="45%">
																						<span style="font: 11px Arial;">Height</span>
																				</td>
																				<td width="55%">
																						<input class="segInput" id="vital_hf" name="vital_hf" type="text" tabindex=5 size="1" maxlength=3 style="padding-left:4px;font:bold 11px Arial;" value="'.$rowvitaldata['height_ft'].'" onkeydown="return key_check(event, this.value)"/><span> <span style="font: 11px Arial;">ft. </span> / <span>
																						<input class="segInput" id="vital_hi" name="vital_hi" type="text" tabindex=5 size="1" maxlength=3 style="padding-left:4px;font:bold 11px Arial;" value="'.$rowvitaldata['height_in'].'" onkeydown="return key_check(event, this.value)" /> <span style="font: 11px Arial;">in. </span>
																				</td>
																		</tr>
																</table>
														</td>
