...
#-------------added by VAN 02-01-08
	...
	function setBeds($ward_nr, $room_nr){
		...
		if (($ward_nr)&&($room_nr)){
			...
			/*added by mai 07-04-2014 get locked beds*/
			$locked_obj = &$ward_obj->getActiveRoomInfo($room_nr,$ward_nr);
			if(is_object($locked_obj)){
				while($lock=$locked_obj->FetchRow()){
					$locked_beds = explode("/", $lock['closed_beds']);
				}
			}
			...
			if ($rs){
				...
				if ($ward_nr){
					...
					if(is_object($patients_obj)){
						...
						$patient = array();
						$patient2 = array();
						$patient_is_mgh = array(); /* added by mai 07-04-2014*/
						while($buf=$patients_obj->FetchRow()){
							$patient[$buf['room_nr']][$buf['bed_nr']]=$buf['sex'];	
							$patient2[$buf['room_nr']][$buf['bed_nr']]=$buf['bed_nr'];
							$patient_is_mgh[$buf['room_nr']][$buf['bed_nr']]=$buf['is_maygohome']; /*added by mai 07-04-2014*/
						}
					}
				}
				...
				if ($ward_obj->rec_count==0){
					...
					for($i=$bed_nr+1; $i<=$result["nr_of_beds"];$i++){
						...
						$patient_is_mgh[$room_nr][$i]=0; /*added by mai 07-04-2014*/
					}
				}
				...
				$objResponse->addScriptCall("ajxGetBedRoom",$result["nr_of_beds"], $patient, $patient2, $patient_is_mgh, $locked_beds); /*updated by mai added patient is may go home and locked beds status 07-04-2014*/
				$objResponse->addScriptCall("chckRoomModeStatus");
			}
		}else{
