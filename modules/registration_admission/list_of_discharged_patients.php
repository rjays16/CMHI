 ...
 */
 ...
 if ($_GET["src"]) {
	 ...
	 $smarty->assign('sToolbarTitle',"List of Discharged Patients");
...
# $smarty->assign('pbHelp',"javascript:gethelp('products_db.php','search','$from','$cat')");
 ...
 # Window bar title
 $smarty->assign('sWindowTitle',"List of Discharged Patients");
...
//		xajax_addSelectedEncounter(enc_nr);
		...
		function openInfo(encounter_nr, pid, dr_nr){
			var url = '../../modules/clinics/seg-compre-discharge.php?pid='+pid+'&encounter_nr='+encounter_nr;

			return overlib(
		            OLiframeContent(url, 1000, 500, 'fOrderTray', 0, 'auto'),
		            WIDTH,600, TEXTPADDING,0, BORDER,0,
		            STICKY, SCROLL, CLOSECLICK, MODAL,
		            CLOSETEXT, '<img src=../../images/close_red.gif border=0 >',
		            CAPTIONPADDING,2,
		            CAPTION,'Comprehensive/Discharge Information',
		            MIDX,0, MIDY,0,
		            STATUS,'Comprehensive/Discharge Information');
		}
...
$title_sufx = (!$_GET['src']) ? 'Discharged Patient' : 'Claims to Transmit';
...
if (!$_GET['src'])
		$result = $objpatients->getDischargedPatients($filters, $list_rows * $current_page, $list_rows, $keyword);
else
		$result = $objpatients->getDischargedPatients($filters, $list_rows * $current_page, $list_rows, $keyword);
				...
				if ($src_link == "") {
                       ...
                       if($patient_type == 3 OR $patient_type == 4){
						...
						$rows .= "<tr class=\"$class\">
													...
													if($_GET['user_origin'] == 'billing'){
														$rows .="<td>".($row['isphic'] == "PHIC" ? "<span style='cursor: pointer;' onclick='setCoverage(".'"'.$row['encounter_nr'].'"'.")'>PHIC</span>" : "")."</td>";
														
														if(!$row['is_chargelock']){
															$img_src = "<img style='cursor: pointer' title='lock' onclick='lock(".'"'.$row['encounter_nr'].'"'.", 1)' src='".$root_path."gui/img/common/default/lock_open.png'/>";	
														}else{
															$img_src = "<img style='cursor: pointer' title='unlock' onclick='lock(".'"'.$row['encounter_nr'].'"'.", 0)' src='".$root_path."gui/img/common/default/lock_delete.png'/>";	
														}
													...
													$rows .= "<td align='center'><img onclick='openInfo(\"".$row['encounter_nr']."\",\"".$row['pid']."\",\"".$row['current_att_dr_nr']."\");' style='cursor: pointer;' src = '".$root_path."images/cashier_edit.gif'></td>";
						$count++;
								...
								<td nowrap="nowrap" align="left" class="jedPanel">
										<table width="100%" border="0" cellpadding="2" cellspacing="0">
												...
												<tr>
														...
														<td width="15%" nowrap="nowrap" align="left">Discharge Date</td>
