...
//																				$ok = $person_obj->updateNameofPerson($_POST['principal'][$k], $details_array);
														...
														#added by monmon : 
														$membertype = $pinsure_obj->getPatientInsuranceMemberType($pid);
														if($pinsure_obj->setPatientInsuranceMemberCategory($encounter_nr,$membertype));
														#end monmon
...
//	echo "class = ".$insurance_class_nr;
				...
				$smarty->assign('sBtnAddItem','<a href="javascript:void(0);" id="addinsurance"
										 onclick="return overlib(
													 OLiframeContent(\''.$url.'pid='.$pid.'&encounter_nr='.$encounter_nr.'&frombilling=1\', 600, 450, \'fOrderTray\', 1, \'auto\'),
