...
if (($encounter_type==3)||($encounter_type==4)){
	...
	if (($encounter_type==3)||($encounter_type==4)){
        ...
        $bill_fromdate = $admission_dt;

	}elseif(($encounter_type==1) || ($encounter_type==2)){
        $bill_fromdate = $encounter_date;
    }
...
<script  language="javascript">
	...
	*/
	...
	function BillingItem(is_ER){
		...
		return overlib(
					OLiframeContent('../../modules/billing_new/billing-main-new.php?sid=<?php echo "$sid&lang=$lang"?>'+
                                    '&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&area='+area+'&enc_nr=<?=$encounter_nr?>'+
                                    '&dr_nr=<?=$dept_belong["personell_nr"]?>&dept_nr=<?=$dept_belong["dept_nr"]?>'+
                                    '&bill_fromdate=<?=$bill_fromdate?>&is_dr=<?=$is_doctor?>',
									850, 440, 'fGroupTray', 0, 'auto'),
											WIDTH,850, TEXTPADDING,0, BORDER,0,
									STICKY, SCROLL, CLOSECLICK, MODAL,
									CLOSETEXT, '<img src=<?=$root_path?>/images/close.gif border=0 onClick="">',
										...
										<td width="45%" align="right" valign="top">
												<table>
														...
														<tr>
																...
																<td width="55%">
																	<input class="segInput" readonly id="vital_hf" name="vital_hf" type="text" tabindex=5 size="1" maxlength=3 style="padding-left:4px;font:bold 11px Arial;" value="'.$rowvitaldata['height_ft'].'" onkeydown="return key_check(event, this.value)"/><span style="font: 11px Arial;">ft/</span>
																	<input class="segInput" readonly id="vital_hi" name="vital_hi" type="text" tabindex=5 size="1" maxlength=3 style="padding-left:4px;font:bold 11px Arial;" value="'.$rowvitaldata['height_in'].'" onkeydown="return key_check(event, this.value)" /><span style="font: 11px Arial;">in</span>
																</td>
														</tr>
												</table>
										</td>
...
if (($encounter_type == 2) || ($encounter_type == 3) || ($encounter_type == 1)) {
	...
	$smarty->assign('segCompanyLabel',"Care Of (C/O)");
