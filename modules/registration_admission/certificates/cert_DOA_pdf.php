...
if($email = $objInfo->getHospitalContact('main_info_email')){
	$email = "   EMAIL: ".$email;
}

if($phone_no = $objInfo->getHospitalContact('main_info_phone')){
	$phone_no = "Tel. Number: ".$phone_no;
}

$contactInfo = $phone_no.$email;
...
$pdf->setFont($fontStyle,"", $fontSizeInput);
$pdf->Cell(20,4, '',$border2, 0, 'C');
$pdf->Cell(0,4,$contactInfo,$border2, 1, 'C');
...
$pdf->Image('../../../gui/img/logos/cmhi_logo.jpg',20,18,25,25);
$pdf->Ln(2);
$pdf->SetLineWidth(0.3);
$pdf->Line(24, 45, 210-8, 45);
$pdf->SetLineWidth(0);
//end art
...
if(stripslashes($is_DOA_reason)){
	$is_DOA_reason = ' due to '.stripslashes(strtoupper($is_DOA_reason));
}
...
$pdf->Ln(13);
$pdf->MultiCell(179,5, '         This is to certify that '.$name_patient.', '.strtoupper($age).$gender.','.
									' arrived in our hospital on '.date("F d, Y",strtotime($encounter_date)).' at '.date("h:i A").
									' and was declared dead on arrival'.$is_DOA_reason.'.', $border2,'J');

$pdf->Ln(9);
$pdf->Cell(179,5,'              This certification is issued for whatever purpose it may serve.',$border2, 'L');
$pdf->Ln(15);
...
$pdf->Cell(55,7, $attending_physician_name, "",1,'L');
$pdf->setFont($fontStyle,"",$fontSizeText);
$pdf->Cell(55,7, 'Attending Physician', "",1,'L');

if($lic_no = $pers_obj->get_Personell_info($attending_physician_nr)){
	$licnumber = "Lic. No. ".$lic_no['license_nr'];
}

$pdf->Cell(55,7, $licnumber, "",0,'L');
...
$pdf->Ln(15);
$pdf->SetFont($fontStyle,"", $fontSizeText-3);
