...
$pdf->SetTopMargin(2);
	...
	$pdf->SetFont("Arial","","11");
	$pdf->Cell(0,4,"#2081 NATIONAL HIGHWAY SALVACION, PANABO CITY",$border2,1,'C');

//File No.. Line -2
$pdf->Ln(10); #edited by art 01/18/2014
...
$pdf->Image('../image/cmhi_logo.jpg',18,10,24,24);
//end art
...
$pdf->MultiCell(180, 6,'            This is to certify that '.stripslashes(strtoupper($name_last)).", ".stripslashes(strtoupper($name_first)).' '.stripslashes(strtoupper($name_middle)).
					' , '.$age.' old, '.mb_strtoupper($civil_status).', and was examined, treated '.$confine.'in this hospital on/from '.$dateconfine.
...
if ($medCertInfo['is_medico_legal']){
	...
	$pdf->Ln(15);
	$pdf->cell(55, 4,"ADVISED TO REST FOR: ","",0,"L");
	$pdf->cell(20, 4,$_GET['advised'],"B",0,"L");

	$pdf->Ln(12);
	$pdf->cell(55, 4,"MAY GO BACK TO WORK: ","",0,"L");
	$pdf->cell(20, 4,$_GET['may_go_back']." ","",0,"L");

	$pdf->Ln(12);
	$pdf->cell(50, 4,"RECOMMENDATIONS: ","",0,"L");
	$pdf->Ln(10);
	$pdf->SetFont($fontStyle,"I",$fontSizeText);
	$pdf->MultiCell(175,4,strtoupper($medCertInfo['recommendations']),0,'J',0);
	$pdf->SetFont($fontStyle,"",$fontSizeText);
...
if ($medCertInfo['is_doc_sig']){
	...
	$name_doctor = "".$docInfo['name_first']." ".$docInfo['name_2']." ".$dr_middleInitial." ".$docInfo['name_last'].($docInfo["other_title"] ? ", ".$docInfo["other_title"] : "");
	 }else{
	...
	$pdf->setFont($fontStyle,"B",$fontSizeText);
	$pdf->Cell(100,6,'',"",0,"");
	$pdf->Cell(75,6,strtoupper($name_doctor),"",0,"L");
	...
	$pdf->Cell(75,6,"Lic No.".trim($docInfo['license_nr']),"",0,"L");
...
$pdf->Cell(50,6,"WITHOUT "."CMHI"." SEAL","",0,"R");
...
$pdf->code ="NOT VALID FOR MEDICO-LEGAL CASES";
//end art
