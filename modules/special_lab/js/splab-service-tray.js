...
function startAJAXSearch(searchID, page) {
	...
	var pid = window.parent.$('hrn').innerHTML;
	var is_notphic = ((window.parent.$('grant_type')).value == 'phic' ? 0 : 1);
	...
	if (searchEL) {
		...
		AJAXTimerID = setTimeout("xajax_populateSpecialLabServiceList('"+pid+"','"+date+"','"+time+"','"+day+"','"+month+"','"+year+"','"+ptype+"','"+area_type+"','"+encounter_nr+"','"+ref_source+"','"+is_cash+"','"+discountid+"','"+discount+"','"+is_senior+"','"+is_walkin+"','"+aLabServ+"','"+source_req+"','"+isStat+"','"+is_charge2comp+"','"+compID+"','"+searchID+"','"+keyword+"',"+page+",'"+var_ptype+"')",100);
...
switch (user_origin.value){
	...
	//if (checkRequestDetails(id)){
		...
		details.incprCepPHIC = $('incprCePHIC'+id).value;
		details.incprCepNONPHIC = $('incprCeNONPHIC'+id).value;
		details.hasrooms = $('hasroom'+id).value; 
...
function addProductToList(phic,nonphic,hasrum,listID, id, name, grp_code, code_num,iscode, cash, charge, sservice,in_lis,oservice_code,ipdservice_code,net_price, available, is_blood_product, is_package, is_profile, child_test) {
	...
	if (list) {
		...
		if (id) {
				...
				rowSrc = "<tr>"+
									...
									'	<input id="incprCePHIC'+id+'" type="hidden" value="'+phic+'"/>'+
									'	<input id="incprCeNONPHIC'+id+'" type="hidden" value="'+nonphic+'"/>'+
									'	<input id="hasroom'+id+'" type="hidden" value="'+hasrum+'"/>'+
									'</td>'+
