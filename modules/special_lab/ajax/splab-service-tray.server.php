...
function populateSpecialLabServiceList($pid=0,$date='',$time='',$day='',$month='',$year='',$area='',$area_type='',$encounter_nr=0,$ref_source='LB',$is_cash=1,$discountid='',$discount=0,$is_senior=0,$is_walkin=1,$group_code,$source_req='LD',$isStat=0,$is_charge2comp=0,$compID='',$sElem,$searchkey,$page, $ptype=0) {
		global $db;
		...
		if ($ergebnis) {
			...
			while($result=$ergebnis->FetchRow()) {
                ...
                if ($result['is_profile']){
                	...
                	else if(($requestTime >='17' || $requestTime <= '7' || $requestDay == "Sunday" || $is_holiday == TRUE) && $ptype == 2)
                	...
                	if($prices){
                		...
                		if($encounter_nr){
                			//$objResponse->alert('3');
                			$objResponse->addScriptCall("addProductToList",$INCPHICaddtional,$INCNONPHICaddtional,$Ifhasroom,"request-list",$result["service_code"],
...
#---------------------------------------------------
	...
	function populate_lab_checklist($pid_hrn=0,$date='',$time='',$day='',$month='',$year='',$section,$area_type='', $searchkey='', $area='',$ref_source='LB',$is_cash=1,$discountid='',$discount=0,$is_senior=0,$is_walkin=1,$source_req='LD',$isStat=0,$is_charge2comp=0,$compID='',$encounter_nr, $ptype=0)
	{
		global $db;
		$objSS = new SocialService;
		$srv = new SegSpecialLab();
							...
							if($guiDetails["name_type"]=="D")
							 {
			                	...
			                	if ($discountid == 'PHS') {
			                		$labfree = $srv->getLaboratoryFree($guiDetails["service_code"]);
			                		if ($labfree) {{
				                		$other_cash = 0;
				                		$other_charge = 0;
				                		$other_net = 0;
			                			...
			                			$discount_price = $srv->getDiscount();
				                		$other_cash = $other_cash - ($other_cash*$discount_price);
				                		$other_charge = $other_charge - ($other_charge*$discount_price);
				                		$other_net = $other_net -($other_net*$discount_price);

			                		}
			                	}
				              	else if(($requestTime >='17' || $requestDay == "Sunday" || $is_holiday == TRUE) && $ptype == 2) {
				                	$other_cash = ($other_cash*$increased_value) + $other_cash;
				                	$other_charge = ($other_charge*$increased_value) + $other_charge;
				                	$other_net = ($other_net*$increased_value) + $other_net;
							 	}
