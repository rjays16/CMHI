...
define(BILATERAL_TUBAL_LIGATION, '5-663');
define(VASECTOMY, '5-636');
define(CAESAREAN, '%cesarean%');
define(PHILHEALTH, 18);
define(NORMAL_DELIVERY_CODE, 'O80');
define(CAESAREAN_DELIVERY_CODE, 'cesarean');
...
//E. SURGICAL OUTPUT - TOP 10 Procedures
    $procedure_sql = "SELECT
                c.description,
                        ...
                        FROM
                          care_encounter_procedure AS ed 
                          INNER JOIN care_encounter AS e 
                            ON e.encounter_nr = ed.encounter_nr 
                          INNER JOIN care_ops301_en AS c 
                            ON c.CODE = ed.CODE  
                          LEFT JOIN seg_encounter_diagnosis AS diagnosis 
                            ON diagnosis.encounter_nr = e.encounter_nr 
                          LEFT JOIN care_icd10_en AS icd 
                            ON icd.diagnosis_code = diagnosis.code 
            AND icd.`diagnosis_code` NOT LIKE ".$db->qstr(CAESAREAN)."
                        WHERE e.STATUS NOT IN (
                            'deleted',
                            'hidden',
                            'inactive',
            ...
            AND c.`code` NOT IN (".$db->qstr(VASECTOMY).",".$db->qstr(BILATERAL_TUBAL_LIGATION).")
                          AND DATE(e.discharge_date) BETWEEN ".$db->qstr($first_date)." 
                          AND ".$db->qstr($last_date)." 
            AND ed.type_nr IN ($type_nr) 
            AND ed.encounter_type IN ('2')
                          AND IF(
                            INSTR(c.CODE, '.'),
                            SUBSTR(
                              c.CODE,
...
// E.1 TOTAL SURGICAL STERILIZATION
    $sterilization_sql = "SELECT
                ...
                AND c.`code` NOT IN (".$db->qstr(VASECTOMY).",".$db->qstr(BILATERAL_TUBAL_LIGATION).")
                              AND DATE($date_based) BETWEEN ".$db->qstr($first_date)." 
                              AND ".$db->qstr($last_date)." 
                AND ed.type_nr IN ($type_nr) 
                AND ed.encounter_type IN ($patient_type)
                              AND IF(
...
// F. OBSTETRICAL PROCEDURES (TOTAL NUMBER OF DELIVERIES) = PHIC
    $obstetrical_delivery_phic_sql = "SELECT
                                          SUM(
                                            CASE
                                              WHEN EXISTS (SELECT ins.hcare_id FROM seg_encounter_insurance AS ins WHERE ins.encounter_nr = e.encounter_nr AND ins.hcare_id = '18')
                                              AND ed.CODE = ".$db->qstr(NORMAL_DELIVERY_CODE)."
                                              THEN 1 
                                              ELSE 0 
                                            END
                                          ) AS normal,
