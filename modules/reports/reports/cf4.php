<?php
    ...
    $params->put('hci_name', $row['hosp_name']);
   	$params->put('province', "DAVAO DEL NORTE");
    ...
    $dis_sql = "SELECT 
                ...
                WHERE ssd.encounter_nr = " . $enc_no . "";
    $dis_res = $db->Execute($dis_sql);
    while($row = $dis_res->FetchRow()){
        $params->put('discharge_diagnosis', $row['final_diagnosis']);
    }
    ...
    while ($row = $dt_res->FetchRow()){
        ...
        $params->put('date_discharged', $row['discharge_date'] != "0000-00-00" && $row['discharge_time'] != "00:00:00" ? date('m-d-Y', strtotime($row['discharge_date']))." ".date('h:i:a', strtotime($row['discharge_time'])) : "                      ");
        $params->put('age', $row['discharge_date'] != "0000-00-00" ? $person->getFullAge($person_info['date_birth'], $row['discharge_date']) : $person->getFullAge($person_info['date_birth'], $row['encounter_date']));
    }
...
//    var_dump($ob_sql);die;
	...
	foreach ($ss_result as $key) {
		if($key['sign_symptoms'] == 1) {
			$params->put('sign_and_symp_1', 1 );
		}else if ($key['sign_symptoms'] == 2) {
			$params->put('sign_and_symp_2', 1 );
		}else if ($key['sign_symptoms'] == 3) {
			$params->put('sign_and_symp_3', 1 );
		}else if ($key['sign_symptoms'] == 4) {
			$params->put('sign_and_symp_4', 1 );
		}else if ($key['sign_symptoms'] == 5) {
			$params->put('sign_and_symp_5', 1 );
		}else if ($key['sign_symptoms'] == 6) {
			$params->put('sign_and_symp_6', 1 );
		}else if ($key['sign_symptoms'] == 7) {
			$params->put('sign_and_symp_8', 1 );
		}else if ($key['sign_symptoms'] == 8) {
			$params->put('sign_and_symp_7', 1 );
		}else if ($key['sign_symptoms'] == 9) {
			$params->put('sign_and_symp_9', 1 );
		}else if ($key['sign_symptoms'] == 10) {
			$params->put('sign_and_symp_10', 1 );
		}else if ($key['sign_symptoms'] == 11) {
			$params->put('sign_and_symp_11', 1 );
		}else if ($key['sign_symptoms'] == 12) {
			$params->put('sign_and_symp_12', 1 );
		}else if ($key['sign_symptoms'] == 13) {
			$params->put('sign_and_symp_13', 1 );
		}else if ($key['sign_symptoms'] == 14) {
			$params->put('sign_and_symp_14', 1 );
		}else if ($key['sign_symptoms'] == 15) {
			$params->put('sign_and_symp_15', 1 );
		}else if ($key['sign_symptoms'] == 17) {
			$params->put('sign_and_symp_17', 1 );
		}else if ($key['sign_symptoms'] == 18) {
			$params->put('sign_and_symp_18', 1 );
		}else if ($key['sign_symptoms'] == 19) {
			$params->put('sign_and_symp_19', 1 );
		}else if ($key['sign_symptoms'] == 20) {
			$params->put('sign_and_symp_20', 1 );
		}else if ($key['sign_symptoms'] == 21) {
			$params->put('sign_and_symp_21', 1 );
		}else if ($key['sign_symptoms'] == 22) {
			$params->put('sign_and_symp_22', 1 );
		}else if ($key['sign_symptoms'] == 23) {
			$params->put('sign_and_symp_23', 1 );
		}else if ($key['sign_symptoms'] == 25) {
			$params->put('sign_and_symp_24', 1 );
		}else if ($key['sign_symptoms'] == 26) {
			$params->put('sign_and_symp_25', 1 );
		}else if ($key['sign_symptoms'] == 27) {
			$params->put('sign_and_symp_26', 1 );
		}else if ($key['sign_symptoms'] == 28) {
			$params->put('sign_and_symp_28', 1 );
		}else if ($key['sign_symptoms'] == 30) {
			$params->put('sign_and_symp_31', 1 );
		}else if ($key['sign_symptoms'] == 29) {
			$params->put('sign_and_symp_30', 1 );
		}else if ($key['sign_symptoms'] == 32) {
			$params->put('sign_and_symp_32', 1 );
		}else if ($key['sign_symptoms'] == 33) {
			$params->put('sign_and_symp_29', 1 );
		}else if ($key['sign_symptoms'] == 34) {
			$params->put('sign_and_symp_33', 1 );
		}else if ($key['sign_symptoms'] == 35) {
			$params->put('sign_and_symp_34', 1 );
		}else if ($key['sign_symptoms'] == 36) {
			$params->put('sign_and_symp_35', 1 );
		}else if ($key['sign_symptoms'] == 37) {
			$params->put('sign_and_symp_16', 1 );
		}else if ($key['sign_symptoms'] == 38){
            $params->put('opt_2', 1 );
            $params->put('opt_2_values', $key['pains']);
        }else if($key['sign_symptoms'] == X){
            $params->put('opt_3', 1);
            $params->put('opt_3_values', $key['others']);
        }
