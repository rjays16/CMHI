...
class RepGen_Prescription extends RepGen {
	...
	function Header()
	{
		...
		$this->Cell($total_w,4,strtoupper($hospitalInfo['hosp_name']),$border2,1,'C');
		$this->SetFont("Arial","","11");
		$this->Cell($total_w,3,$hospitalInfo['hosp_addr1'],$border2,1,'C');
		$this->SetFont("Arial","","11");
		$this->Cell($total_w,3,"Tel No. ".$hospital->getHospitalContact('main_info_phone'),$border2,1,'C');
		$this->Ln(3);
...
//        die;
		...
		$this->Ln(1);
		$this->SetFont("Arial","","9");
		$this->Cell(12,3, "Patient: ", $border2, 0, "L");
		$this->SetFont("Arial","","8");
		...
		$this->SetFont("Arial","","8");
		$this->SetTextColor(0, 0, 0);
		$this->Cell(0,3, strtoupper($person_info["sex"]), "B", 0, "C");
		...
		$this->SetTextColor(0, 0, 255);
        $this->Cell(13,3, "Address: ", $border2, 0, "L");
		$this->SetFont("Arial","","8");
		$x = $this->GetX();
		$y = $this->GetY();
    	...
    	$this->Line(24,24,77,24);
    	$this->SetTextColor(0, 0, 0);
        $this->MultiCell(55,3, mb_strtoupper($address),$border2, "L");
		...
		$this->SetFont("Arial","", "9");
		$this->SetTextColor(0, 0, 255);
		$this->Cell(10,3, "Date: ", $border2, 0, "R");
		$this->SetFont("Arial","","10");
		$this->SetTextColor(0, 0, 0);
		$this->Cell(0,3.5, $today, "B", 1, "C");

		$this->Image($root_path.'gui/img/common/default/rx3.jpg',$this->GetX(),$this->GetY()-3,10,11);
		#$this->Ln(15);
	...
	//	parent::Footer();
        ...
        $row = $this->Prescription->getPrescriberInfo($this->person_data['attending_physician_nr']);
		...
		*/
		...
		$this->SetTextColor(0, 0, 0);
		$this->Cell(100 - strlen($row['name'])-50,3,"",0,0);
		$this->Cell(0,3, $row['name'] . ', ' . $doctorNameSuffix, "B", 1, 'L');
		//end Nick

		$length = 99 - strlen($row['name']) - 50;
		//echo strlen($row['name']);
		$this->Ln(1);
		$this->Cell($length,3,"",0,0);
		$this->SetFont("Arial","","9");

		$this->SetTextColor(0, 0, 255);
		$this->Cell(14,3, "Lic. No: ", "", 0, "L");
		$this->SetFont("Arial","","9");
		$this->SetTextColor(0, 0, 0);
		$this->Cell(0,3, $row["license_nr"], "B", 1, 'L');
		...
		$this->SetTextColor(0, 0, 255);
		$this->Cell(14,3, "PTR No: ", $border2, 0, "L");
		$this->SetFont("Arial","","9");
		$this->SetTextColor(0, 0, 0);
		$this->Cell(0,3, $row["ptr_nr"], "B", 1, 'L');
		...
		$this->SetTextColor(0, 0, 255);
		$this->Cell(14,3, "S2 No: ", $border2, 0, "L");
		$this->SetFont("Arial","","9");
		$this->SetTextColor(0, 0, 0);
		$this->Cell(0,3, $row["s2_nr"], "B", 1, 'L');
	...
	function AfterData()
	{
		...
		$details = $this->Prescription->getPrescriptionPrint($this->prescription_id);
//        echo "<pre>";
...
//				$this->SetXY($x,$y+5);
                ...
                $this->SetFont('Arial', '', 12);
				$this->MultiCell(0,4, 
					$counter.".) ".strtoupper($name)." #".number_format($row['quantity'],0)."", 
					$border2, 'L');
				$this->Ln(0.5);
				$this->SetFont('Arial', '', 10);
				$this->MultiCell(0,4, 
