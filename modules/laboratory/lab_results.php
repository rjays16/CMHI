<?php
		...
		*/
		...
		require_once($root_path."include/care_api_classes/class_patient_queue.php"); //added by mai 10-22-2014
...
function ReloadWindow(pid, source){
	//edit by VAN 04-27-2011
	 ...
	 if (source=='lab')
			window.parent.location = 'seg-lab-request-order-list.php?done=0&searchkey='+pid;
...
<?php
		...
		if($_GET["service_code"]){
				...
				$exploded_service_code = explode("%2C", $get_service_code);
				$imploded_service_code = implode("','", $exploded_service_code);
				$service_codes = "'".$imploded_service_code."'";
		}
		...
		else
		{
				...
				$pathologist = 0;
				$med_tech = "";
				$sql = "select service_date, med_tech_pid, pathologist_pid, performed_by_pid, remarks FROM seg_lab_resultdata WHERE refno='$refno' AND group_id='$group_id'  AND (ISNULL(`status`) OR `status`!='deleted');";
				...
				if($result!=NULL && $resdata = $result->FetchRow())
				{
					 ...
					 $performed_by_pid = $resdata["performed_by_pid"];
				...
				$performed_by_pid = $_REQUEST["permormed_by_dr"];
				...
				if($submit=="SAVE" || $submit=="SAVE_AND_DONE"){
						...
						if($group_id==0 || $group_id==""){
								$sql = "SELECT p.*, r.result_value, r.unit, s.name as group_name
												...
												LEFT JOIN seg_lab_services AS s ON s.service_code = p.service_code
												LEFT JOIN seg_lab_result AS r ON r.param_id = p.param_id AND r.refno='$refno' AND (ISNULL(r.status) OR r.status!='deleted')
												WHERE (p.norm_type= '$norm_type' OR p.norm_type = 'none')  AND s.service_code='$service_code' ORDER BY p.order_nr ASC";
						}
						else{
								$sql = "SELECT p.*, r.result_value, r.unit, pg.name as group_name, gp.order_nr as order2, IF(ISNULL(d.refno), 0, 1) AS enabled
												FROM seg_lab_result_groupparams as gp
												INNER JOIN seg_lab_result_params as p ON p.service_code = gp.service_code AND (p.norm_type= '$norm_type' OR p.norm_type = 'none')  AND (ISNULL(p.status) OR p.status NOT IN ('deleted'))
												LEFT JOIN seg_lab_result as r ON p.param_id = r.param_id AND r.refno='$refno' AND (ISNULL(r.status) OR r.status!='deleted')
												LEFT JOIN seg_lab_result_paramgroups as pg ON pg.param_group_id = p.param_group_id
												LEFT JOIN seg_lab_servdetails AS d ON d.service_code=p.service_code AND d.refno='$refno'
												WHERE gp.group_id=$group_id AND (ISNULL(gp.status) OR gp.status!='deleted')
												...
												LEFT JOIN seg_lab_result_params as p ON p.service_code = g.service_code_child AND (p.norm_type= '$norm_type' OR p.norm_type = 'none')  AND (ISNULL(p.status) OR p.status NOT IN ('deleted'))
												LEFT JOIN seg_lab_result as r ON p.param_id = r.param_id AND r.refno='$refno' AND (ISNULL(r.status) OR r.status!='deleted')
												LEFT JOIN seg_lab_result_paramgroups as pg ON pg.param_group_id = p.param_group_id
												LEFT JOIN seg_lab_servdetails AS d ON (d.service_code=g.service_code OR d.service_code=p.service_code) AND d.refno='$refno'
												WHERE gp.group_id=$group_id AND (ISNULL(gp.status) OR gp.status!='deleted')
												AND p.service_code IS NOT NULL AND p.group_id IS NOT NULL
						...
						if($result)
						{
								...
								if($group_id)
										$sql_c = "SELECT * FROM seg_lab_resultdata WHERE refno = '$refno' AND group_id='". $group_id."'";
								else
										$sql_c = "SELECT * FROM seg_lab_resultdata WHERE refno = '$refno' AND service_code='". $service_code."'";
								#echo $sql_c;
								...
								if($status=="add")
								{
									 ...
									 $bSuccess = $lab_results->add_lab_resultdata($refno, $group_id, $_POST["date"], $med_tech_pid, $_POST["pathologist"],$conf,$service_code, $_POST['remarks'], $_POST['permormed_by_dr']);
									 
									 if($bSuccess) $bSuccess = $lab_results->add_lab_results($lab_result, $refno, $norm_type, $group_id, $service_code);
								...
								else if($status=="edit")
								{
									 ...
									 if(isset($_POST["is_confidential"]))
										 ...
										 $conf = 0;
									 $bSuccess = $lab_results->update_lab_resultdata($refno, $group_id, $_POST["date"], $med_tech_pid, $_POST["pathologist"],$conf,$service_code, $_POST['remarks'], $_POST['permormed_by_dr']);
									 
									 if($bSuccess) $bSuccess = $lab_results->update_lab_results($lab_result, $refno, $norm_type, $group_id, $service_code);
									 if (!$bSuccess) {
											 $db->FailTrans();
								...
								if($submit=="SAVE_AND_DONE" && $bSuccess){
										...
										//added by mai 10-22-2014 For Queuing
										$result_queue = new Patient_queue();
										$result_queue->details = array("ref_no"=>$refno, "area"=>"lab");
										$result_queue->addResultQueue();
										//end added by mai
				...
				elseif($submit=="VIEW PDF"){
						if($lab_results->getGroupForm($group_id)){
							$x = '../../modules/repgen/pdf_lab_results_specialforms.php?pid='.$pid.'&refno='.$refno.'&group_id='.$group_id.'&service_code='.$service_code.'&date='.$date;
						}else{
							$x = '../../modules/repgen/pdf_lab_results.php?pid='.$pid.'&refno='.$refno.'&group_id='.$group_id.'&service_code='.$service_code.'&date='.$date;
						}
...
<script type="text/javascript" src="<?=$root_path?>js/wsnotif.js?t=<?=time()?>"></script>
...
function ConfirmDone(){
		...
		if(CheckFields()){
				...
				if(answer){
					refreshDashlet('_a_1_queing_dashlet_refresh');
					return true;
				}else{
					return false;
				}
						...
						<tr>
							<td height="149" valign="top" bgcolor="#FFFFFF" class="carlpanel"><table width="100%" border="0" cellpadding="0" cellspacing="2">
									<tr>
										...
										<td width="46%"><table height="64" border="0" cellpadding="1" cellspacing="5" class="carlpanel">
												<tr>
													...
													<td width="34%"><input name="age" id="age" type="text" size="20"  value="<?= $age ?>" readonly="readonly"/></td>
									...
									<tr>
										...
										<?php
												...
												if($group_id==0 || $group_id==""){
														...
														$sql = "SELECT p.*, r.result_value, r.unit, s.name as group_name
																		...
																		LEFT JOIN seg_lab_services AS s ON s.service_code = p.service_code
																		LEFT JOIN seg_lab_result AS r ON r.param_id = p.param_id AND r.refno='$refno' AND (ISNULL(r.status) OR r.status!='deleted')
																		WHERE (p.norm_type= '$norm_type' OR p.norm_type = 'none') AND s.service_code='$service_code' AND (p.status IS NULL OR p.status = '') ORDER BY p.order_nr ASC";
												}
												else{
										...
										?>
										<tr><td colspan=2 class='carlpanel' style='font-size:12px'><b>Remarks</b></td><td colspan=5 class='carlpanel' style='font-size:12px'><b><input type="text" id="remarks" size="90" name="remarks" value="<?=$remarks?>"></b></td></tr>
									</table>
									<table width="100%" border="0" cellpadding="1" cellspacing="2" >
												...
												<tr>
													<td width="30%" class="carlPanelHeader">Performed by</td>

													<td width="70%" class="carlpanel">
														<select name="permormed_by_dr" id="permormed_by_dr" <?php echo $performed_by_pid;?>>
														<?php
															#edited by VAN 03-30-10
															$sql = "SELECT pr.pid,
																				CONCAT(IF(ISNULL(trim(cp.name_last)), '', CONCAT(trim(cp.name_last), ', ')),IF(ISNULL(trim(cp.name_first)), '', CONCAT(trim(cp.name_first), ' ')), IF(ISNULL(trim(cp.name_middle)), '', CONCAT(substring(trim(cp.name_middle),1,1), '. '))) as name
