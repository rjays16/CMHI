<?php
        ...
        function ColHeaderRequest(&$objResponse, $tbId){
                ...
                $tbNr = substr($tbId, 4);
                $th  = "<thead><tr><th colspan=\"15\" id=\"mainHead".$tbNr."\">";
                $th .= "</th></tr></thead>";
                ...
                $thead .= "<td width=\"13%\" class=\"".$class."\" align=\"center\"><b>Requested By</b></td>";
                ...
                $thead .= "<td width=\"5%\" class=\"".$class."\" align=\"center\"><b>Status</b></td>";
...
$all_results = array();
                ...
                //usort($all_results, 'compare_time');                                 
                        ...
                        foreach($all_results as $result)
                       {
                                ...
                                /*added by mai 08132014*/
                                    $status_request = "not_served";
                                    $count_served_query = "SELECT 
                                                              IFNULL(SUM(td.count_served), 0) AS count_served,
                                                              SUM(td.count_req) AS count_req 
                                                            FROM
                                                              (SELECT 
                                                                (SUM(sr.is_served) - SUM(is_deleted)) AS count_served,
                                                                sv.no_serial AS count_req 
                                                              FROM
                                                                seg_lab_servdetails sd 
                                                                LEFT JOIN seg_lab_serv_serial sr 
                                                                  ON sd.`refno` = sr.`refno` 
                                                                LEFT JOIN seg_lab_services sv 
                                                                  ON sd.`service_code` = sv.`service_code` 
                                                              WHERE sd.refno = ".$db->qstr(trim($result['refno']))."
                                                                AND sv.is_serial = 1 
                                                                AND sr.`is_deleted` <> 1 
                                                                AND sd.status <> 'deleted'
                                                              UNION
                                                              SELECT 
                                                                is_served AS count_served,
                                                                '1' AS count_req 
                                                              FROM
                                                                seg_lab_servdetails sd 
                                                                LEFT JOIN seg_lab_services sv 
                                                              ...
                                                              WHERE sd.refno = ".$db->qstr(trim($result['refno']))."
                                                                AND sv.is_serial <> 1
                                                                AND sd.status <> 'deleted') td ";
                                    
                                    $count_served_result = $db->Execute($count_served_query);
                                    if ($count_served_result){
                                            $count_served_res = $count_served_result->FetchRow();
                                            if($count_served_res['count_served']!=0){
                                                if($count_served_res['count_served'] == $count_served_res['count_req']){
                                                    $status_request = "served";  
                                                }else{
                                                    $status_request = "incomplete"; 
                                                }
                                            }
                                    }

                                    $request_name = "";
                                    $get_personell_query = "SELECT 
                                                              cu.personell_nr
                                                            FROM
                                                              seg_lab_serv s 
                                                              INNER JOIN care_users cu
                                                                ON s.create_id = cu.login_id 
                                                                AND s.refno = ".$db->qstr(trim($result['refno']));

                                    $get_personell_result = $db->Execute($get_personell_query);
                                    if($get_personell_query){
                                        $request_name = "Name";
                                        $get_personell_res = $get_personell_result->FetchRow();
                                        $request_by = $personell_obj->get_Person_name($get_personell_res['personell_nr']);
                                        $request_name = $request_by['name_first']." ".$request_by['name_2']." ".$request_by['name_last'];
                                    }
                                ...
                                if ($mod){
                                        ...
                                        $objResponse->addScriptCall("jsRequest",$sub_dept_nr,$my_count,trim($result["refno"]),$name,$requestDate,$urgency,$or_no, $labstatus, $paid, $repeat,trim($result['encounter_nr']),trim($result["pid"]),floor($age),mb_strtolower($result["sex"]),$location, $enctype,$or_no,$result["is_cash"], $with_res,$result["is_repeat"], $status_request, $request_name);
                                        //1
                                }else{
                                        ...
                                        $objResponse->addScriptCall("jsRequest",$sub_dept_nr,$my_count,trim($result["refno"]),$name,$requestDate,$urgency,$or_no, $labstatus, $paid, $repeat,trim($result['encounter_nr']), trim($result["pid"]),floor($age),$sex_,$location, $enctype, $result["is_cash"],$with_res,$result["is_repeat"], $status_request, $request_name);
                                        //2
                                }
                          $my_count++;
                        }# end of while loop
    ...
    function populateLabResultList($done, $sElem,$searchkey,$page,$include_firstname,$mod, $encounter_nr='', $is_doctor=0,$pid='',$ref_source='LB') {
            ...
            if ($ergebnis) {
                ...
                while($result=$ergebnis->FetchRow()) {
                        ...
                        $objResponse->addScriptCall("addResultList","RequestList",$result['name'],$result['service_code'],date('Y-m-d',strtotime($result['service_date'])), $result['group_id'], $result['refno'], $result['form_id']);
