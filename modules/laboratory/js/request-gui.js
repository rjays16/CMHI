...
function appendOrder(list,details) {
	...
	if (list) {
		...
		if (dBody) {
			...
			var cashprice, forwarding, monitor, everyhour/*, no_takes*/;
			...
			if (details) {
						...
						if (items[i].value == details.id) {
							...
							if ((details.sservice==1) && ((person_discountid) && (person_discountid!=" ")) && ((person_discountid=="C1")||(person_discountid=="C2")||(person_discountid=="C3"))){
								...
								//commented by VAN 10-27-08
																if (($F('orig_discountid')=='DMC') || ($F('orig_discountid')=='DMCDep') || ($F('orig_discountid')=='DMCCD') || ($F('orig_discountid')=='DMCConDep') || ($F('orig_discountid')=='DMCPGI'))
																		tot_price = 0;
																else
																	 tot_price = prc;
				...
				*/
				...
				monitor = '<input type="checkbox" id="monitor'+id+'" name="monitor'+id+'" value="1" onClick=formonitoring(\''+id+'\'); />';
				everyhour = '<input type="text" id="hour'+id+'" size="3" maxlength="2" name="hour'+id+'" disabled value="" oncClick=checkValidity(this.value); onkeydown="return key_check(event, this.value)" />';

				//------added by CHA, Feb 1, 2010-----
				no_takes = '<input type="text" id="numtake'+id+'" size="3" maxlength="2" name="numtake'+id+'" disabled value="" oncClick=checkValidity(this.value); onkeydown="return key_check(event, this.value)" />';
				//-----end CHA------------------------
				src =
					...
					'<input type="hidden" name="request_flag[]" id="rowFlag'+id+'" value="" />'+
					...
					'<td width="5%" id="is_monitor-row'+id+'" '+toolTipTextHandler+' align="center">'+monitor+'</td>'+
					'<td width="5%" id="everyhour-row'+id+'" '+toolTipTextHandler+' align="center">'+everyhour+'</td>'+
			...
			document.getElementById('index').value = items.length;
...
function checkCharityAmount(totalNONSocializedAmount){
	...
	if (dAdjAmount) {
		...
		adjusted_amount=dAdjAmount.value.replace(",","");
	}
 if (($F('orig_discountid')=='DMC') || ($F('orig_discountid')=='DMCDep') || ($F('orig_discountid')=='DMCCD') || ($F('orig_discountid')=='DMCConDep') || ($F('orig_discountid')=='DMCPGI')){
		//do nothing
 }else{
...
function refreshDiscount() {
	...
	*/
	if (person_discountid=="C1")
		...
		//net = document.getElementsByName('pcashbc[]');
	}
	...
	//if (($F('orig_discountid')=='DMC') || ($F('orig_discountid')=='DMCDep') || ($F('orig_discountid')=='DMCCD') || ($F('orig_discountid')=='DMCConDep')){
			 //do nothing
	//}else
...
function refreshTotal() {
	...
	*/
	...
	if (document.getElementById('repeat').checked){
		...
		netTotal.innerHTML = formatNumber(Math.round(totalamt).toFixed(2),2);

				//document.getElementById('show-discount').value = formatNumber(total.toFixed(2),2);
	}else{
		subTotal.innerHTML = formatNumber(total.toFixed(2),2);
		discountTotal.innerHTML = "-&nbsp;&nbsp;"+formatNumber(Math.round(totalDiscountedAmount).toFixed(2),2);
		netTotal.innerHTML = formatNumber(Math.round(totalNet).toFixed(2),2);

				//document.getElementById('show-discount').value = formatNumber(totalDiscountedAmount.toFixed(2),2);
	}
	/*
	subTotal.innerHTML = formatNumber(total.toFixed(2),2);
...
function viewHistory(pid,encounter_nr){
	window.open("seg-lab-request-history.php?pid="+pid+"&encounter_nr="+encounter_nr+"&showBrowser=1","viewRequestHistory","width=620,height=440,menubar=no,resizable=yes,scrollbars=yes");
}
