...
function jsRequest(sub_dept_nr,No,refno, name, req_date, urgency, or_no, service, code, repeat, pid,age,sex,location,enctype, with_res, group_id, with_result,isrepeat,is_confidential){
						...
						if (done==1){
								serve ='onclick="return overlib(OLiframeContent(\'lab_results.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp='+1+'&mode=update&update=1&status=edit&refno='+refno+'&pid='+pid+'&service_code='+code+'&group_id='+group_id+'&done=0&user_origin=<?=$user_origin?>\', 830, 430, \'flab-list\', 1, \'auto\'), ' +
								 ...
								 if ((with_result==1)||(group_id!=0)){
									 labresult ='onclick="return overlib(OLiframeContent(\'lab_results.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp='+1+'&mode=update&update=1&status=add&refno='+refno+'&pid='+pid+'&service_code='+code+'&group_id='+group_id+'&done=1&user_origin=<?=$user_origin?>\', 830, 430, \'flab-list\', 1, \'auto\'), ' +
											'WIDTH, 840, TEXTPADDING, 0, BORDER, 0, STICKY, SCROLL, CLOSECLICK, MODAL, CLOSETEXT, \'<img src=../../images/close.gif border=0 onClick=refreshWindow(key,pagekey);>\', '+
								 ...
								 //edited by VAN 01-09-10
								 if ((with_result==1)||(group_id!=0)){
									 labresult ='onclick="return overlib(OLiframeContent(\'lab_results.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp='+1+'&mode=update&update=1&status=add&refno='+refno+'&pid='+pid+'&service_code='+code+'&group_id='+group_id+'&done=0&user_origin=<?=$user_origin?>\', 830, 430, \'flab-list\', 1, \'auto\'), ' +
