...
*/
    ...
    # Added by Jarel 09/07/2014 Create personnel object 
	...
	#Added by Jarel 10/07/2014
	$req_doc = $personell_obj->get_Person_name3($dr_nr);
	if($req_doc){
		while ( $row_doc = $req_doc->Fetchrow() ) {
			$request_doctor_name = mb_strtoupper($row_doc['dr_name'].($row_doc['drtitle'] ? ', '.$row_doc['drtitle'] : ''));
		}
	}#End by Jarel 10/07/2014
    ...
    if (($pid == "") || (strlen($pid) < 6))
{
	...
	if ($_GET['ptype'] || $_POST['source'])
		$ptype = (($_GET['ptype'])? $_GET['ptype'] : $_POST['source']);
				...
				if ((isset($POST['repeat']))&&($_POST['repeat'])){
					...
					if (($isCorrectInfo)||($allow_radiorepeat)){
						...
						if($refno = $srvObj->saveLabRefNoInfoFromArray($_POST)){
                            $saveok=$srvObj->update_LabRefno_Tracker($_POST['refno']); //1 save
					...
					if($refno = $srvObj->saveLabRefNoInfoFromArray($_POST)){
                        $success = 1;
                        $saveok=$srvObj->update_LabRefno_Tracker($_POST['refno']); //2 save
                    ...
                    if (!$saveok){ /*edited by mai, added save ok1 07-15-2014*/
                        $srvObj->FailTrans();
						$smarty->assign('sysErrorMessage','<strong>Error:</strong> '.$srvObj->sql);
                    }else{    
                        $saveok1=$comp_obj->saveChargetoCompanyTransaction($_POST); /*added by mai 07-15-2014*/    
...
//								'</script>';
					...
					if($srvObj->updateLabRefNoInfoFromArray($_POST)){
							...
							#added by VAN 06-02-2011
							if ($_POST['for_manual_payment'])
                                $saveok=$srvTempObj->save_ManualPayment($_POST['refno'], $manual_data); //1 update
							else{
                                $saveok=$srvTempObj->ManualPayment($_POST['refno'], $manual_data); //2 update
                                $saveok=$srvTempObj->resetRequestFlag($_POST['refno'], $manual_data, 'seg_lab_servdetails'); //2 update
							}
...
#added by daryl
    ...
    $smarty->assign('sDiscountShow','<input type="checkbox" name="issc" id="issc" onclick="seniorCitizen();" '.(($_POST["issc"])?'checked="checked" ':'').' ><label for="issc" style="font:bold 11px Tahoma;">Senior citizen</label>');
	...
	$smarty->assign('sOrderDate','<span id="show_orderdate" class="jedInput" style="font-weight:bold; color:#0000c0; padding:0px 2px;width:200px; height:24px">'.($submitted ? date($fulltime_format,strtotime($_POST['orderdate'])) : $curDate_show).'</span>
		<input class="jedInput" name="orderdate" id="orderdate" type="hidden" value="'.($submitted ? date($dbtime_format,strtotime($_POST['orderdate'])) : $curDate).'">');
	...
	$smarty->assign('sComments','<textarea class="segInput" name="comments" id="comments" wrap="physical"  cols="30" rows="'.$row.'" style="overflow-y:scroll; overflow-x:hidden; float:left; margin-left:5px; font-size:12px; font-weight:normal; font-style:italic">'.$comments.'</textarea>');
...
$iflabwalk = $labwalk['ifwalk'];
                                ...
                                "<select class=\"jedInput\" name=\"grant_type\" id=\"grant_type\" ".$disabled." 
                                     onchange=\" if (warnClear()) { emptyTray(); checkCharge(this.value); changeChargeType(); return true;} else {return false;}\">
                                         $options
                                            ...
                                            <input type='hidden' id='comp_id' name='comp_id' value='".$comp_id."'/>".'<input type="hidden" id="is_chargelock" name="is_chargelock" value="'.$patient['is_chargelock'].'"'); /*edited by mai added hidden field remaining balance company*/
	...
	$smarty->assign('sCompanyName',$compName); //edited by mai 10-01-2014
	...
	#--------------
	//Added by Jarel 10/06/2014 For Search box
	if($ptype == 'doctor') 
		$smarty->assign('sSearchService','<input class="segInput" placeholder="Search Laboratory Services" id="searchservice" name="searchservice" type="text" size="30" style="font:bold 15px Arial;"/>');
...
?>
	<script type="text/javascript" language="javascript">
		...
		if (refno){
			xajax_populateRequestListByRefNo(refno, ref_source, fromSS, discount, discountid,enc);
...
?>
    ...
    <!-- added by Jarel 10-07-2014 -->
    <input type="hidden" name="request_doctor_name" id="request_doctor_name" value="<?=$request_doctor_name?>">
...
$smarty->assign('sMainBlockIncludeFile','laboratory/lab-request-new'.(( $ptype == 'doctor' ) ? '2' : '').'.tpl');
