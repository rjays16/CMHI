		...
		if ($saveok) {
			...
			$count=0;
				foreach ($_POST["items"] as $i=>$v) {
						...
						$uprice  = $_POST['uprice_big'][$i];                   // unit price
                 ...
                 // Associated serial no.
                        ...
                        $saveok = $objdel->saveDeliveryDetail($_POST['rcv_area'], $data, $_POST["delivery_date"], $expiry, $objserial, $lotNo, $fg_item);
...
if (isset($_GET["refno"])) {
		$smarty->assign('sDeliveryDetails',"
								<tr>
										<td colspan=\"12\">Delivery is currently empty ...</td>
								</tr>");
		...
		if ($result = $objdel->getDeliveryHeader($_GET["refno"])) {
				...
				if ($result = $objdel->getDeliveryDetails($_GET["refno"])) {
						...
						if ($result->RecordCount()) {
                            ...
                            $script .= "xajax_goAddItem(items0, units0, expirys0, serials0, ispcs0, qtys0, uprices0, lotnos0, qty_per_pck0, small_unit0, is_fg0);";
								$script .= "</script>";
								$src = $script;
						}
						else
