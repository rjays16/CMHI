...
//                }
            // added by ken for supplying data in suppliers and area that assign FIS PO.,
                ...
                details.price_add = (parseFloat(nprice)/parseFloat(details.qty_per_pck)); //unit price per small unit
                ...
                src = '<tr class="wardlistrow'+alt+'" id="row_'+details.id+srow+'">' +                        
                          ...
                          '<input name="uprice_big[]" id="uprice_big_'+details.id+srow+'" value="'+nprice+'" type="hidden">'+
                          ...
                          '<td width="1%"><input type="checkbox" onclick="setFG('+details.id+srow+');" id="chkfg_'+details.id+srow+'" '+((details.is_fg=='1') ? 'checked="checked"' : '') +' name="fg_item[]" ></td>'+
...
function applyQtyPerPck(id, srow){
    var qty = $("editqty_"+id+srow).value; 
    var qty_per_pck = $("qty_per_pck_"+id+srow).value;
    var uprice_per_pck = $("uprice_big_"+id+srow).value;

    var unit_price = parseFloat(uprice_per_pck)/parseFloat(qty_per_pck);

    $("qty_"+id+srow).value = qty;   
    $("qty_per_pck_"+id+srow).setAttribute('value', qty_per_pck);

    $("edituprice_"+id+srow).setAttribute('value', unit_price);
    $("rowuprice_"+id+srow).innerHTML = formatNumber(Number(unit_price),2);

    $("rowtotal_"+id+srow).innerHTML = formatNumber(Number(qty) * Number(unit_price) * Number(qty_per_pck), 2);
...
function updateItem(id, qty, expiry_dt, serial_no, price_add, srow, lot_no){
    ...
    $('rowtotal_'+id+srow).innerHTML = formatNumber(Number(qty) * Number(price_add) * Number(qty_per_pck), 2);
