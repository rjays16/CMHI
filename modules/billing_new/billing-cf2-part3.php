...
class PhilhealthForm2Part3 extends TCPDF {
    ...
    function getBillingDetails(){
		...
		$sql = "SELECT 
				  ...
				  WHERE dr_nr = 0 
				    AND refno = sbe.`bill_nr`) AS total_hci_discount,
				  (SELECT SUM(dr_claim) FROM seg_billing_pf a WHERE a.bill_nr = sbe.bill_nr AND a.hcare_id='18' 
					...
					(SELECT 
					    SUM(
					      IFNULL(sbch.total_msc_coverage, 0) + IFNULL(sbch.total_acc_coverage, 0) + IFNULL(sbch.total_med_coverage, 0) + IFNULL(sbch.total_srv_coverage, 0) + IFNULL(sbch.total_ops_coverage, 0)
					    ) 
					  FROM
					    seg_billing_coverage sbch 
					  WHERE sbch.bill_nr = sbe.`bill_nr` 
					    AND sbch.hcare_id <> '18') AS total_other_hmo_hci,
					  (SELECT 
					    SUM(
					      IFNULL(sbch.`total_d1_coverage`, 0) + IFNULL(sbch.`total_d2_coverage`, 0) + IFNULL(sbch.`total_d3_coverage`, 0) + IFNULL(sbch.`total_d4_coverage`, 0)
					    ) 
					  FROM
					    seg_billing_coverage sbch 
					  WHERE sbch.bill_nr = sbe.`bill_nr` 
					    AND sbch.hcare_id <> '18') AS total_other_hmo_doc
				FROM
				  care_encounter ce 
		...
		if ($result = $db->Execute($sql)){ 
			if ($result->RecordCount()) {
                ...
                $this->pid = $row['pid'];
                $this->other_hmo_hci = $row['total_other_hmo_hci'] > 0 ? "/" : "";
                $this->other_hmo_doc = $row['total_other_hmo_doc'] > 0 ? "/" : "";
			}
		} 
...
function addProfessionalFees($doctors){
	...
	function addPart3(){
		...
		$Smember1="";
		$Smember2="";
		$Shmo1= $this->other_hmo_hci;
		$Shmo2= $this->other_hmo_doc;
		$Sothers1="";
		$Sothers2="";
