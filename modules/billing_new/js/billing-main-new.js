...
function preset(){
    ...
    $j( "#btnCSFp2" ).button({text: true,icons: {primary: "ui-icon-folder-open"}});
  ...
  $j('#btnSave').click(function saveBill(){
    ...
    if ($('isFinalBill').checked){
        ...
        if(((parseFloat(totalHealthInsurancePF)==0) || (parseFloat(totalHealthInsuranceHF)==0)) && (isphic) ){
            if(answer)
                xajax_saveThisBilling(data, check, details, com_details, isNewBorn);
            else{
    ...
    $j('#btnInsurance').click(function() {
        ...
        .html('<iframe style="border: 0px; " src="' + pageInsurance + '" width="100%" height=450px></caiframe>')
        .dialog({
            autoOpen: true,
            modal: true,
            show: 'fade',
...
function saveDoctorCoverage()
{
    ...
    $j.each(company_field, function(i, field){
        ...
        data_com.push({"enc":data["enc"], "amount":field.value, "itemcode":itemcode, "dr_nr":dr_nr});
    });
...
function saveHCICoverage()
{
    ...
    var company_field = $j('.save-com-details').serializeArray();
    ...
    $j.each(company_field, function(i, field){
        var itemcode = $j('#'+field.name).attr("itemcode");
        data_com.push({"enc":data["enc"], "amount":field.value, "itemcode":itemcode, "dr_nr":0});
    });

    xajax_saveHCICoverage(data);
    xajax_saveDiscountHCI(data_discount);
    xajax_saveComdetails(data_com);
}
...
function jsAccommodationList(data, total_charge, b_new){
    ...
    if (!isNaN(type_nr)) {
        if (source=='BL'){
            prefx = '<td align="center" width="3%"><img src="../../images/btn_delitem.gif" class="imgdelete" style="border-right:hidden; cursor:pointer; '+hide+';" onclick="promptDelAccom('+ward+',\''+source+'\',\''+room+'\',\''+rm_nr+'\')"></td>'+
                    '<td style="border-left:hidden" width="52%">'+
                        'Room No. :'+rm_nr+'<br>'+
                        'Room Type:'+type_desc+
                    '</td>';
            }else{
                 prefx = '<td align="center" width="3%"><img src="../../images/btn_delitem.gif" class="imgdelete" style="border-right:hidden; cursor:pointer; display:'+b_new+';" onclick="promptDelAccom('+ward+',\''+source+'\',\''+room+'\',\''+rm_nr+'\')"></td>'+
                    '<td style="border-left:hidden" width="52%">'+
                        'Room No. :'+rm_nr+'<br>'+
                        'Room Type:'+type_desc+
                    '</td>';
...
function promptDelAccom(id, type, rm_type, rm_nr){
    $('delAccomType').value = type;
    $('room').value = id;
    $('ward').value = rm_type;
    // jQuery('#dialogAccDelConfirm').dialog('open');   
    if(confirm('Do you really want to remove this accommodation?'))
    jsDeleteAccommodation(rm_nr);
    
}//end promptDelAccom

function jsDeleteAccommodation(rm_nr){
    var room_info = new Object();
    ...
    xajax_delAccommodation(room_info,rm_nr);
    ...
    if (isSponsoredMember() || isHSM() || isPHS() /*|| isInfirmaryOrDependent.trim() != ''*/) {
        ...
        if (isSponsoredMember() && !isPAYWARD()) {
           ...
           /* if(isInfirmaryOrDependent.trim() != ''){
            ...
            $j('#DiscountTotal').html(numFormat(final_discount));
            $j('#netamnt').html(numFormat(parseFloat(net) - (parseFloat(coverage) + parseFloat(final_discount) + parseFloat(deposit)) ));
...
function showSoa(){
    ...
    if($('isdied').checked){
        ...
        //added Nikko 08/20/14
        var flag_header = 0;
        var isHeader= confirm("With header?");
        ...
        urlholder = rpath+'modules/billing_new/bill-pdf-summary_new.php'+seg_URL_APPEND+
                          ...
                          '&nr='+bill_nr+'&IsDetailed='+detailed+'&deathdate='+deathdate+'&wHeader='+flag_header;
...
function calculateDetails()
{
    ...
    $j.each(company_field, function(i, field){
        total_company += parseFloat(field.value);
    });
    ...
    $j('#company_total_doc').html(numFormat(total_company));
    $j('#doc_com').val(total_company);
...
function applySC(dr_nr,area)
{
    ...
    var total_charge = (dr_charge_orig * per_sc_discount);
    
    if($('is_sc_discount_'+dr_nr+'_'+area).checked){
        $j('#applied_display_'+dr_nr+'_'+area).html(numFormat(parseFloat(total_charge).toFixed(2)));
        $j('#sc_discount_'+dr_nr+'_'+area).val(parseFloat(total_charge).toFixed(2));
        $j('#applied_sc_'+dr_nr+'_'+area).val( parseFloat(total_charge).toFixed(2));
    }else{
        $j('#applied_display_'+dr_nr+'_'+area).html('0.00');
        $j('#sc_discount_'+dr_nr+'_'+area).val(0);
        $j('#applied_sc_'+dr_nr+'_'+area).val(0);
        $j('#doc_charge_'+dr_nr+'_'+area).val(dr_charge_orig);
        $j('#doc_charge_display_'+dr_nr+'_'+area).html(numFormat(dr_charge_orig));
    }
    calculateDetails();
...
function calculateDetailsHCI()
{
    ...
    $j('#company_total').html(numFormat(total_company));
...
function checkNegationHCI(obj)
{
    ...
    $j.each(hcare_field, function(i, field){ 
        var hcare_id = $j('#'+field.name).attr('hcareid');
        $j('#hci_total_'+hcare_id).html(numFormat(parseFloat($j('#hci_total_'+hcare_id).html().replace(',','')) + parseFloat(field.value)));
        if(hcare_id==PHIC)
            phic_coverage +=  parseFloat(field.value);
    ...
    if((parseFloat(phic_coverage) > parseFloat(totalHealthInsuranceHF)) || (total_excess_x < 0)) {
        alert('The amount you enter is Greater than PHIC Maximum Coverage \n or Greater than Actual Charge.');
        $j('#'+obj.id).val(0)
        $j('#total_18').html(0);
        $j.each(hcare_field, function(i, field){ 
            if(obj.name == field.name){
...
function applyCheckDetails(obj)
{
    var excess_field_x = $j('.calc-excess-hci').serializeArray();
    var area = $j('#'+obj.id).attr('itemcode');
    var hcareid = $j('#'+obj.id).attr('hcareid');
    var charge = $j('#hci_charge_'+area).val().replace(',','');

    if(obj.checked){
        var amount = 0;
        $j.each(excess_field_x, function(i, field){
            var area_attr = $j('#'+field.name).attr('itemcode');
            if(area == area_attr)
                amount += parseFloat(field.value.replace(',',''));
        });
        total_amount = parseFloat(charge) - parseFloat(amount);
        $j('#coverage_'+hcareid+'_'+area).val(total_amount);
    }else{
        $j('#coverage_'+hcareid+'_'+area).val(numFormat(0));
    }
...
function applyCheckDetailsDR(obj)
{
    var excess_field_x = $j('.calc-excess').serializeArray();
    var area = $j('#'+obj.id).attr('itemcode');
    var hcareid = $j('#'+obj.id).attr('hcareid');
    var dr_nr = $j('#'+obj.id).attr('dr_nr');
    var charge = $j('#doc_charge_'+dr_nr+'_'+area).val().replace(',','');

    if(obj.checked){
        var amount = 0;
        $j.each(excess_field_x, function(i, field){
            var area_attr = $j('#'+field.name).attr('itemcode');
            var dr_attr = $j('#'+field.name).attr('itemcode');

            if(area == area_attr && dr_nr == dr_attr)
                amount += parseFloat(field.value.replace(',',''));

        });
        total_amount = parseFloat(charge) - parseFloat(amount);
        $j('#coverage_'+hcareid+'_'+dr_nr+'_'+area).val(total_amount);
    }else{
        $j('#coverage_'+hcareid+'_'+dr_nr+'_'+area).val(numFormat(0));
    }
