...
function editInsurance(pid, id) {
	$('id').value = 'update';	
	showDialogBox(id);		
	xajax_getInsuranceInfo(pid, id);
}

function deleteInsurance(pid, id) {
	xajax_deleteInsurance(pid, id);
...
function js_getApplicableInsurance() {
	...
	xajax_getApplicableInsurance(pid);
...
function addApplicableInsurance(id, desc, acc, xlso, meds, ops, misc, doc) {
	...
	if (id) {
		...
		srcRow += '<a title="Edit" href="#"><img class="segSimulatedLink" src="'+root_path+'images/cashier_edit.gif" border="0" align="absmiddle" onclick="editInsurance(\''+pid+'\', \''+id+'\')"/></a>&nbsp;&nbsp;'+
					'<a title="Delete" href="#"><img class="segSimulatedLink" src="'+root_path+'images/cashier_delete.gif" border="0" align="absmiddle" onclick="if (confirm(\'Delete this insurance?\')) deleteInsurance(\''+pid+'\', '+id+')"/>'+
					'</a>';
		srcRow += '</td></tr>';
	}
	else {
