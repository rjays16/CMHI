...
function calculateSub(enc)
{
    ...
    if($j('#check-select-employee-'+enc).is(':checked')){
        ...
        $j('#show-sub-total').html(numFormat(sub_total - temp_total));
    }
    calculateTotal();
}
...
{   
    ...
    $j("#count_items").val(array_cases.length);
}

function selectAll(ischecked){
    if(ischecked){
        $j('.generate-bill').prop('checked', true);    
        sumAll();
    }else{
        $j('.generate-bill').prop('checked', false);
        $j('#show-sub-total').html(numFormat(0));
        calculateTotal();
        case_numbers.length = 0;
        array_cases.length = 0;
    }
...
function sumAll(){
    var charge_comp = $j('.checkall');
    var sum = 0;

    charge_comp.each(function(){
        var case_no = $j(this).attr('encounter_nr');
        getCaseNumbers(parseFloat(case_no));
        sum += parseFloat($j(this).val().replace(',',''))
    });

    $j('#show-sub-total').html(numFormat(sum));
    calculateTotal();
}
