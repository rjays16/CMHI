...
require_once($root_path."include/care_api_classes/class_company.php"); //added by mai 08/07/14
...
define('COL03_WIDTH', 29);
define('COL04_WIDTH', 20);
define('COL05_WIDTH', 20);
define('COL06_WIDTH', 28);
define('COL07_WIDTH', 20); /*added by mai 07-09-2014*/
define('COL08_WIDTH', 20); /*added by mai 07-09-2014*/

define('FOOTER_COL01', 82);
...
class BillPDF extends FPDF {
	...
	var $head_name;
	var $head_position;
	var $wHeader; //added Nikko 8/20/14
    ...
    var $columns = array('actual_charge'=>COL03_WIDTH, 'sc_discount'=>COL04_WIDTH, 
    						'phic'=>COL05_WIDTH, 'other_insurance'=>COL06_WIDTH, 'ar_discount'=>COL07_WIDTH, 'excess'=>COL08_WIDTH);
    var $equal_width = 0;
    /*end added by mai*/
	/*
	 * Constructor
	...
	function BillPDF($enc='', $bill_dt = "0000-00-00 00:00:00", $bill_frmdt = "0000-00-00 00:00:00", $old_bill_nr = '', $bcomp=false, $deathdate) {
		...
		#added by VAN 02-14-08
		 ...
		 $pg_size = array($this->in2mm(8.5), $this->in2mm(12));                 // Default to long bond paper --- modified by LST - 04.13.2009
...
#----------------------- LST - 06-21-2008
		...
		//added by Nikko 8/20/2014
		$this->wHeader = $_GET['wHeader'];

		if($this->wHeader == 1){
			$row['hosp_name']    = "CAINGLET MEDICAL HOSPITAL INCORPORATED";
			$row['hosp_addr1']   = "# 2081 NATIONAL HI-WAY SALVACION, PANABO CITY";
			$this->Image('../../gui/img/logos/cmhi_logo.jpg',20,10,22,20);
...
#---------------------- LST  - 06-21-2008
	...
	function ReportFooter() {
        ...
        if ($this->clerk_italized)
		    ...
		    $this->SetFont($this->fontType, $this->fontStyle, 9);
	    ...
	    $this->Cell(200,-5,"Telephone No. : 628-5222", "", 1, 'C');
	    $this->Cell(119,-5,"Date Signed : ".$this->getSignDate(), "", 1, 'R');
	    // $this->Cell(130,-5,"getSignDate()", "", 1, 'C');
	 //    $this->Cell(38,200,"Telephone Number :", 0, '', 0, false, '');
		// $this->Cell(15, 200, "628-5222", 0, '', 0, false, ''); // johnmel


		$this->Cell(FOOTER_COL01, 4, "Prepared by:", "", 1, '');
		$this->Ln(4);
		/*$this->Cell(4, 4, "", "", 0, '');
        ...
        $this->Cell(FOOTER_COL01, 4, "", "", 0, '');
        $this->Cell(20, 4, "", "", 0, '');*/
        $this->Cell(FOOTER_COL02, 4, "Address", "T", 1, 'C');
    
        $this->Ln(6);
        $this->Cell(FOOTER_COL02, 4, "Contact #", "T", 1, 'C');
        
        $this->Ln(6);
        $this->Cell(FOOTER_COL02, 4, "Relationship to Member", "T", 1, 'C');
...
//		$row = $this->personData->FetchRow();
	...
	function ReportTitle() {
		//added by Nikko 08/27/14
		if($this->wHeader == 1) $this->Ln(2);
		else $this->Ln(7);

		//end

		$this->SetFont($this->fontType, "B", "11");
...
*/
			...
			$this->Cell(1, 4, ":", '', 0, 'R');
			$this->MultiCell($this->in2mm(4.4), 4, strtoupper($saddr1.$saddr2.$saddr3), 0, 'L');

			//Admitted
			$this->Cell(26.6, 4, "Days Admitted :", "", 0, '');
			/*$this->Cell(2, 4, ":", "", 0, 'R');*/
...
//			$this->Cell($this->in2mm(4), 4, "( ".$sward_name." )".($sCaseType == '' ? '' : " - ".$sCaseType), "", 0 ,'');
            ...
            $this->Cell(23, -20, "Date Discharge : ", "", 0, '');
            if ($this->objBill->isFinal()) {
                $this->Cell(50, -20, "  ".$billdte." ", "", 0, 'L');
            }
			...
			if ($this->isphic && $this->IsDetailed) {
				...
				if($result) {
					while($row=$result->FetchRow()){
						if($row['rate_type']==1){
			                ...
			                $this->Cell(30, 4, "First Case Rate", 0, 0, 'L');
			                $this->SetFont("Times", "B", "10");
			                $this->Multicell(150, 4,": ".$row['package_id'], 0, 'L');
			                $this->SetFont("Times", "", "10");
						}else {
			                $this->SetFont("Times", "", "10");
			                $this->Cell(30, 4, "Second Case Rate", 0, 0, 'L');
			                $this->SetFont("Times", "B", "10");
			                $this->Multicell(150, 4,": ".$row['package_id'], 0, 'L');
			                $this->SetFont("Times", "", "10");
						}
				...
				// Added by Johnmel 10-11-2018
	            if ($this->finalDiagnosis($this->encounter_nr)) {
	                $this->SetFont("Times", "", "10");
	                $this->Cell(30, 4, "Final Diagnosis", 0, 0, 'L');
	                $this->SetFont("Times", "B", "10");
	                $this->Multicell(150, 4,": ".$this->finalDiagnosis($this->encounter_nr), 0, 'L');
	                $this->SetFont("Times", "", "10");
	            }
			...
			/*added by mai 08/07/2014*/
			$comp_obj = new Company();
			if($comp_name = $comp_obj->getCompanyName($this->encounter_nr)){
				$this->Cell(20, 4, "Company", "", 0, '');
				$this->Cell(1, 4, ":", "", 0, 'R');
				$this->Cell(10, 4, strtoupper($comp_name), "", 1, '');
			}
			/*end added by mai*/
	...
	function getSignDate() {
		return $signature_date = strftime("%m-%d-%Y", strtotime($this->bill_date));
	}

	// Added by Johnmel 10-11-2018
    public static function FinalDiagnosis($encounterNr)
    {
        global $db;//$db->debug = 1;
        return $db->GetOne("SELECT final_diagnosis FROM seg_soa_diagnosis WHERE encounter_nr = ?",$encounterNr);
    }
...
//				$this->Cell(GEN_COL01-4, 4, "#", "TB", 0, 'C');
				...
				$this->SetFont($this->fontStyle, $this->fontType, "9");
				$this->Cell($this->in2mm(GEN_COL02), 4, "Particulars", "TB", 0, 'C');
				$this->Cell(COL_MID, 4, " ", "", 0, '');
				
				$headers = array("actual_charge"=>"Actual Charges", 
								"sc_discount"=>"Discounts", "phic"=>"PHIC", 
								"other_insurance"=>"Other Insurance/s", "ar_discount"=>"Discount", "excess"=>"Excess");
		...
		foreach($hsp_pfs_benefits as $key=> $value) {
			...
			foreach($proffees_list as $key=>$profValue){
				if($value->role_area == $profValue->role_area) {
					...
					$drName = $profValue->dr_first." ".$profValue->dr_last;
...
#Admitting -- Admitting -- Admitting
			if($d1_count>0){
				...
				if($d1_count>0  && $this->IsDetailed);
					...
					$this->Pf_Sub_Total($d1_totalPF, $d1_totalSenior, $d1_totalPhic, $d1_totalOtherIns, $d1_totalDiscount, $d1_totalExcess,"Admitting");
			}
#Consulting -- Consulting -- Consulting
			if($d2_count>0){
				...
				$this->Pf_Sub_Total($d2_totalPF, $d2_totalSenior, $d2_totalPhic, $d2_totalOtherIns, $d2_totalDiscount, $d2_totalExcess,"Consulting");
			}
#Surgeon -- Surgeon -- Surgeon
			if($d3_count>0){
				...
				if($d3_count>0 && $this->IsDetailed);
					...
					$this->Pf_Sub_Total($d3_totalPF, $d3_totalSenior, $d3_totalPhic, $d3_totalOtherIns, $d3_totalDiscount, $d3_totalExcess, "Surgeon");
			}
#Surgeon -- Surgeon -- Surgeon
			if($d4_count>0){
				$d4_totalPF = 0;
				...
				if($d4_count>0 && $this->IsDetailed);
					...
					$this->Pf_Sub_Total($d4_totalPF, $d4_totalSenior, $d4_totalPhic, $d4_totalOtherIns, $d4_totalDiscount, $d4_totalExcess, "Anesthesiologist");
			}
...
#edited by Nick, 1/4/2014
	function Pf_Sub_Total($total,$senior,$phic,$other_insurance,$discount,$excess,$role){

		$pf_total_sub = array("sc_discount"=>$senior, "phic"=>$phic, "other_insurance"=>$other_insurance, "ar_discount"=>$discount);
//		$this->Cell(GEN_COL01, 4, " ", "", 0, '');
...
//			$totalExcess += round($v, 0);
		...
		$totalexcess = $total_serv - ($total_senior + $total_phic + $total_other_insurance + $total_discount);


		$totals = array("sc_discount"=>$total_senior, "phic"=>$total_phic, "other_insurance"=>$total_other_insurance, "ar_discount"=>$total_discount);
...
#edited by Nick, 1/4/2014
	function Less($totalExcess){
        ...
        foreach ($this->objBill->prev_payments as $val) {
            $this->Cell(GEN_COL01, 4, "", "", 0, '');
            $this->Cell(COL03_WIDTH, 4, "    OR#: " .$val->getORNo(),0, 0, '');
            $this->Cell(0, 4, number_format($val->getAmountPaid(), 2, '.', ','), 0, 1, 'R');
        }
...
/*        $totalOBpayments = $this->objBill->getOBAnnexPayment(); //TODO8
        ...
        $totalDiscount = $this->areas_discounts_total['sc_discount']['serv'] + $this->areas_discounts_total['sc_discount']['pf'] +
        					...
        					$this->areas_discounts_total['ar_discount']['serv'] + $this->areas_discounts_total['ar_discount']['pf'];
		...
		$this->SetFont("Arial", "B", "10");
		$this->Cell(COL03_WIDTH + COL04_WIDTH + COL05_WIDTH + COL06_WIDTH + COL07_WIDTH, 4, " ", "", 0, 'R');
        //$this->Cell(COL_MID, 4, " ", "", 0, '');
...
// echo json_encode($data);
		...
		$xlo = array(); //added by maimai 02-06-2015
		$xlo[0]['label'] = "Laboratory";
		$xlo[1]['label'] = "Radiology";
		$xlo[2]['label'] = "Supplies";
		$xlo[3]['label'] = "Others";
		...
		foreach ($data as $data_key => $data_value) {//foreach
			...
			if($data_value['source']=='LB'){
				$lab_count++;
				$xlo[0]['total'] += $data_value['total'];
			}
			if($data_value['source']=='RD'){
				$rad_count++;
				$xlo[1]['total'] += $data_value['total'];
			}
			if($data_value['source']=='MS' || $data_value['source']=='SU'){
				$sup_count++;
				$xlo[2]['total'] += $data_value['total'];
			}
			if($data_value['source']=='OA'){
				$oth_count++;
				$xlo[3]['total'] += $data_value['total'];
			}
		}//foreach

		$this->total_xlo = $total;
		$xlo_count=$lab_count + $rad_count + $sup_count + $oth_count;
		$this->Cell($this->in2mm(GEN_COL02), 4,"X-Ray, Lab, & Others", "", ($xlo_count>0 && $this->IsDetailed)?1:1, '');
		if($xlo_count > 0){#if1
			if($this->IsDetailed){
...
//		return $inches * (0.35/(1/72));
	...
	function Discounts(){
		...
		$sc_ar_discount = $billing->getDiscounts_Serv($this->bill_nr, $this->encounter_nr); //discounts hospital
		foreach($sc_ar_discount as $key){
			...
			$this->areas_discounts_total['phic']['serv'] += $key['phic'];
			$this->areas_discounts_total['other_insurance']['serv'] += $key['other_insurance'];
		}

		$sc_ar_discount =  $billing->getDiscounts_Pf($this->bill_nr, $this->encounter_nr); //discounts doctor
		foreach($sc_ar_discount as $key){
			...
			$this->areas_discounts_total['phic']['pf'] += $key['phic'];
	...
	function calculateColumnWidth(){
			...
			$this->equal_width= (137-$totalWidth)/count($this->discounted_areas)-0.7;
	}

	//added by maimai 02-06-2015
	function breakXLO($xlo){
		
		$this->SetFont("Arial", "I", "8");
		for($i=0; $i<count($xlo); $i++){
			$this->Cell(COL_MID+5, 4, "", "", 0, '');
			$this->Cell($this->in2mm(GEN_COL02)+2, 4,$xlo[$i]['label'], "",0, '');
			$this->Cell(COL_MID, 4, "", "", 0, '');
			$this->Cell(($this->columns["actual_charge"]+$this->equal_width-(COL_MID+5)), 4, number_format($xlo[$i]['total'], 2, '.', ','), "", 1, 'R');
		}

		$this->SetFont("Arial", "", "9");
		$this->Cell(0, 0,"", "",1, '');
		$this->Cell($this->in2mm(GEN_COL02)+2, 4,"Total (X-Ray, Lab & Others)", "",0, '');
	}
	//maimai
	...
	function calculateTotalExcess($area, $charges, $dr_nr){
		$areas_discounts = array('sc_discount', 'phic', 'other_insurance', 'ar_discount');
		$total_discount_area = 0;
		for($i=0; $i<count($areas_discounts); $i++){
			$total_discount_area += $this->areas_discounts[$area][$dr_nr][$areas_discounts[$i]];
		}
		$this->areas_discounts[$area][$dr_nr]['excess'] = $charges - $total_discount_area;
