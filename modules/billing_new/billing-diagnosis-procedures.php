...
$final_diag_val = $enc_obj->getFinalDiagnosis($encounter_nr);
...
function addDiagnosisToList(listID, diagnosis_nr, code, description, doctor, bAddedByBilling, altdesc, isprimary, altcode) {
	...
	if (list) {
		...
		if (code) {
			...
			stmp = (bAddedByBilling) ? '<img style="cursor:pointer" title="Remove!" src="../../images/btn_delitem.gif" border="0" onclick="xajax_rmvCode('+diagnosis_nr+', \''+create_id+'\');"/>&nbsp;&nbsp;' : '&nbsp;';
			...
			if (frombilling==1){	
				...
				rowSrc = '<tr class="wardlistrow'+alt+'" id="row'+addslashes(diagnosis_nr)+'">'+
								...
								'<td id="icd_value" align="center">'+code+'</td>'+
...
function getFinalDiagnosis(){
    ...
    xajax_getFinalDiagnosis(icd_code, case_nr);
}
...
<div style="display:block; border:1px solid #8cadc0; overflow-y:hidden;overflow-x:hidden; height:650px; width:99%; background-color:#e5e5e5">
	<table border="1" class="segPanel" cellspacing="2" cellpadding="2" width="99%" align="center">
		...
		<tr>
			<td>Case Number</td>
			<td id="case_number" style="font-weight: 700;"><?=$encounter_nr?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><textarea style="width:100%; overflow-x:hidden;" wrap="physical" cols="53" rows="3" id="final_diagnosis"><?=$final_diag_val['final_diagnosis']?></textarea></td>
		</tr>
		<tr>
			<td colspan="2">
				<input style="float:right;" id="btnAddIcdCode" style="cursor:pointer" onclick="saveFinalDx()" height="10" type="button" value="Save">
			</td>
		</tr> 
		<!-- johnmel -->
	</table>
