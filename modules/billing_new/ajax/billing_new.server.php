...
require_once($root_path.'include/care_api_classes/class_pf_charge.php'); //added by mai 08-27-2014
require_once($root_path.'include/care_api_classes/class_promissory_note.php'); //aaded by mai 09-10-2014
...
function delAccommodation($room_info, $room_nr) {

	$objResponse = new xajaxResponse();
	$objBilling = new Billing();

	$delOk = $objBilling->deleteAccommodation($room_info, $room_nr);
...
function populateBill($enc,$bill_dte,$bill_frmdte,$death_date,$firstCode='',$secondCode='')
{
	...
	if ($billinfo = $objBill->hasSavedBill($enc)) {
		...
		$details->bill_dte = date('Y-m-d H:i:s');
...
/*added by mai 08-26-2014*/
function deleteDrExaminations($encounter_nr, $id){

	$objResponse = new xajaxResponse();
	$pf_obj = new Pf_charge();
	
	if($pf_obj->deletePfByEncounter($encounter_nr, $id)){
		$objResponse->call("populateBill");
	}else{
		$objResponse->alert("Deletion of attending doctor failed");
	}
...
function saveThisBilling($data, $final, $details, $com_details, $isNewBorn) {
	$objResponse = new xajaxResponse();
	$objBilling = new Billing();
    ...
    if($saveDetails){
	   	...
	   	$data_ledger['amount'] = $data['excess'];
	   	$data_ledger['pay_type'] = 'bill';
	   	$data_ledger['description'] = 'Patient Bill';
        ...
        if($ok_del)
            $ok_ledger = $objPerson->savePersonLedgerInfo($data_ledger);
        else
            $ok_ledger = false;
    }
	...
	if($final == '1' && (ENABLE_FIS)){
		...
		// $result_doc = $curl_obj->inpatientPF($data['billdate'], $data['pid'], $bill_nr, $data['encounter']); // used for
		$result_com = $curl_obj->companyCharge($data['encounter'], $data['billdate'], $comp_amount, $bill_nr, $data['pid']);
		...
		if($result_acc && $result_charge && $result_cov && $result_discount && $result_lab && $result_radiology && $result_pharmacy && $result_misc && $result_dep && $result_com /* && $result_debit_adjustment&& ($result_doc || is_null($result_doc))*/){
			$ok_fis = true;
		}else{
			$ok_fis = true;
		}	
	...
	if($savebill && $saveDetails && $ok_com && $ok_ledger && $ok_fis){
            $db->CommitTrans();
...
function populateAdjCoverageDoctor(&$objResponse, &$objBilling, $details)
{
                        ...
                        while($row_com1 = $result_com1->FetchRow()){
                            ...
                            $inputs_com = "<input id=\"coverage_".$row_com['comp_id']."_".$details->dr_nr."_".$details->area."\" dr_nr=\"".$details->dr_nr."\"
                                                name=\"coverage_".$row_com['comp_id']."_".$details->dr_nr."_".$details->area."\" class=\"segInput numeric calc-excess save-com-doc\"
                                                    ...
                                                    onchange=\"if(!checkNegationHCI(this)){calculateDetails();}\" value=\"".number_format($row_com1['amount'],2,'.','')."\"
                            ...
                            $footer .= "<input type=\"hidden\" id=\"doc_com\" company_id=\"".$row_com['comp_id']."\"
                                ...
                                <th class=\"rightAlign\" id=\"company_total_doc\">0.00</th>";
                        }
                    }else{
                        $check1 = "<input id=\"apply_".$row_com['comp_id']."_".$details->dr_nr."_".$details->area."\"
                                    class=\"segInput\" type=\"checkbox\" ".(($row_com1['amount']) ? 'checked="checked"' : '' )."
			...
			if ($result1->RecordCount()) {
				while ($row1 = $result1->FetchRow()) {
                    $check = "<input id=\"apply_".$row['hcare_id']."_".$details->dr_nr."_".$details->area."\"
                                    ...
                                    itemcode=\"".$details->area."\" hcareid=\"".$row['hcare_id']."\" dr_nr=\"".$details->dr_nr."\"
                                    onclick=\"applyCheckDetailsDR(this)\">";
	...
	$check_sc = "<input id=\"is_sc_discount_".$details->dr_nr."_".$details->area."\" 
			  class=\"segInput\" type=\"checkbox\" ".(($sc_discount>0) ? 'checked="checked"' : '' )."
			  onclick=\"applySC('".$details->dr_nr."','".$details->area."')\" value=\"0.8\" PercentDiscount =\"0.2\" />";

    $check_all_sc = "<input id=\"check_all_sc\"
					  class=\"segInput\" type=\"checkbox\" ".(($sc_discount>0) ? 'checked="checked"' : '' )."
					  onclick=\"applySCHCI('".$value."')\" value=\"0.2\" />";

    if($senoir){
        $th .= "<th width=\"12%\" nowrap=\"nowrap\">".$check_all_sc." Senior 20%</th>";
        $td .= "<td style=\"font:bold 12px Arial;\">".$check_sc."
                    	<span id=\"applied_display_".$details->dr_nr."_".$details->area."\" >".number_format($sc_discount, 2, '.' ,',')."</span>
                    </td>";
        $footer .= "<th class=\"rightAlign\" style=\"font:bold 12px Arial;\" id=\"total-senior\"></th>";
    }
	...
	$tr_details ="<tr>
					...
					<input type=\"hidden\" class=\"calc-excess calc-senior save-discount-pf\" id=\"applied_sc_".$details->dr_nr."_".$details->area."\" name=\"applied_sc_".$details->dr_nr."_".$details->area."\" value=\"".$sc_discount."\"/>
...
* Created By Jarel
* Created On 04/12/2014
* Populate data in adjusted coverage (HCI's Area)
* @param object objResponse
* @param object objBilling
* @param object details
* @return mixed objResponse
...
function populateAdjCoverageHCI(&$objResponse, &$objBilling, $encounter)
{
    ...
    $com_area = array('ACC','XLO','MED', 'OPR' ,'MSC');
	...
	foreach ($area as $key => $value) {
        ...
        //Add Company UI Jarel
        $result_com = $objCompany->getCompanyDetailsByEnc($encounter);
        if($result_com){
            if($result_com->RecordCount()){
                while($row_com = $result_com->FetchRow()){
                    $th .= 	"<th width=\"2%\" nowrap=\"nowrap\" > </th>
                             <th width=\"10%\" nowrap=\"nowrap\">Company</th>";
                    $result_com1 = $objCompany->getOtherCompanyDetails($encounter,$value);
                    if($result_com1){
                        if($result_com1->RecordCount()){
                            while($row_com1 = $result_com1->FetchRow()){
                                $check1 = "<input id=\"apply_".$row_com['comp_id']."_".$value."\"
                                    class=\"segInput\" type=\"checkbox\" ".(($row_com1['amount']) ? 'checked="checked"' : '' )."
                                    itemcode=\"".$value."\" hcareid=\"".$row_com['comp_id']."\" onclick=\"applyCheckDetails(this)\">";

                                $com_limit = (($row_com1['amount_limit'])? 'NO LIMIT' : number_format($row_com1['amount_limit'],2,'.',''));
                                $company_total += $row_com1['amount'];

                                $inputs_com = "<input id=\"coverage_".$row_com['comp_id']."_".$value."\"
                                                name=\"coverage_".$row_com['comp_id']."_".$value."\" class=\"segInput numeric calc-excess-hci save-com-details saving-com-details\"
                                                    type=\"text\" style=\"width:100%; text-align:right\" onfocus=\"this.select()\"
                                                    onchange=\"if(!checkNegationHCI(this)){calculateDetailsHCI();}\" value=\"".number_format($row_com1['amount'],2,'.','')."\"
                                                    itemcode=\"".$value."\" comp_area=\"".$com_area[$key]."\" refsource=\"1\" company_id=\"".$row_com['comp_id']."\">" ;

                                $td .= "<td class=\"centerLeft\" width=\"2%\">$check1</td>
                                        <td class=\"centerLeft\" width=\"10%\">$inputs_com</td>";

                                $footer .= "<th class=\"rightAlign\"></th>
                                            <th class=\"rightAlign\" id=\"company_total\">0.00</th>";
                            }
                        }else{
                            $check1 = "<input id=\"apply_".$row_com['comp_id']."_".$value."\"
                                    class=\"segInput\" type=\"checkbox\" ".(($row_com1['amount']) ? 'checked="checked"' : '' )."
                            ...
                            $inputs_com = "<input id=\"coverage_".$row_com['comp_id']."_".$value."\"
                                                name=\"coverage_".$row_com['comp_id']."_".$value."\" class=\"segInput numeric calc-excess-hci save-com-details saving-com-details\"
                                                    ...
                                                    itemcode=\"".$value."\" refsource=\"1\" comp_area=\"".$com_area[$key]."\"  company_id=\"".$row_com['comp_id']."\">" ;
		...
		if ($result->RecordCount()) {
			while($row = $result->FetchRow()){
				...
				if ($result1->RecordCount()) {
					...
					while ($row1 = $result1->FetchRow()) {
                        ...
                        $check1 = "<input id=\"apply_".$row['hcare_id']."_".$value."\"
                                    class=\"segInput\" type=\"checkbox\" ".(($row1['dr_claim']) ? 'checked="checked"' : '' )."
                                    itemcode=\"".$value."\" hcareid=\"".$row['hcare_id']."\" onclick=\"applyCheckDetails(this)\">";
			...
			$check_sc = "<input id=\"is_sc_discount_".$value."\" 
					  class=\"segInput\" type=\"hidden\" ".(($sc_discount>0) ? 'checked="checked"' : '' )."
					  onclick=\"applySCHCI('".$value."')\" value=\"0.2\" />";
            ...
            if($senoir){
                $th .= "<th width=\"12%\" nowrap=\"nowrap\">".$check_all_sc." Senior 20%</th>";
                $td .= "<td class=\"rightAlign\" style=\"font:bold 12px Arial;\">".$check_sc."
		                	<span id=\"applied_display_".$value."\" >".number_format($sc_discount, 2, '.' ,',')."</span>
		                </td>";
                $footer .= "<th class=\"rightAlign\" style=\"font:bold 12px Arial;\" id=\"total-senior-hci\"></th>";
            }
			...
			$tr_details ="<tr>
						...
						<input type=\"hidden\" class=\"calc-excess-hci calc-senior-hci save-discount-hci\" id=\"applied_sc_".$value."\" name=\"applied_sc_".$value."\" value=\"".$sc_discount."\"/>
			...
			$objResponse->assign('hiHIC','innerHTML',number_format($coverage + $company_total,2,'.',','));
            $objResponse->assign('company-limit-hci','innerHTML',$com_limit);
...
* Save Promisory
*/
function savePromi($mode, $due_date, $encounter_nr, $amount, $remarks, $is_sum, $is_install, $refno){
	
	$objResponse = new xajaxResponse();
	$objPromi = new Promissory_note();
	
	if($mode == 'add'){
		if($refno = $objPromi->insertPromi($due_date, $encounter_nr, $amount, $remarks, $is_sum, $is_install)){
		...
		if($objPromi->updatePromi($refno, $due_date, $amount, $remarks, $is_sum, $is_install)){
...
function getPromiDetails($refno){
	$objResponse = new xajaxResponse();
	$objPromi = new Promissory_note();
	
	$result = $objPromi->selectPromiDetails($refno);
	if($row = $result->FetchRow()){
		$objResponse->call("resetData", $row['due_date'], $row['is_sum'], $row['is_install'], $row['amount'], $row['remarks'], date("F j, Y", strtotime($row['due_date'])));
	}else{
		$objResponse->alert("Unable to fetch promissory details.");
	}

	return $objResponse;
}

function getCompanyName($enc){
	$objResponse = new xajaxResponse();
	$objComp = new Company();

	$objResponse->assign('comp_name','value',$objComp->getCompanyName($enc));
...
function getPFforFIS($billdate, $pid, $bill_nr, $enc, $frm_dte){
    ...
    if(is_array($hsp_pfs_benefits) && (count($hsp_pfs_benefits) > 0)) {
	    ...
	    return $curl_obj->inpatientPF($billdate, $pid, $bill_nr, $data, $enc);
	}
}
