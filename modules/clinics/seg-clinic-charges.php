...
function openSpLabRequest()
{
	...
	return overlib(
		OLiframeContent('<?=$root_path?>modules/special_lab/seg-splab-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&area=clinic&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>&dr_nr=<?=$doc_nr?>&dept_nr=<?=$dept_nr?>&user_origin=splab&ischecklist=1&ptype=<?=$ptype?>',
...
function openICLabRequest() 
{
	...
	return overlib(
		OLiframeContent('<?=$root_path?>modules/ic_lab/seg-iclab-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&area=clinic&area_type=<?=$area_type?>&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>&dr_nr=<?=$doc_nr?>&dept_nr=<?=$dept_nr?>&user_origin=iclab&ischecklist=1&ptype=<?=$ptype?>',
			800, 370, 'fGroupTray', 0, 'auto'),
		WIDTH,410, TEXTPADDING,0, BORDER,0,
		STICKY, SCROLL, CLOSECLICK, MODAL,
		CLOSETEXT, '<img src="<?=$root_path?>/images/close.gif" border=0 onclick="requestByDate();">',
...
{
	var pageLabRequest = '<?=$root_path?>modules/laboratory/seg-lab-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&area=clinic&area_type=<?=$area_type?>&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>&dr_nr=<?=$doc_nr?>&dept_nr=<?=$dept_nr?>&user_origin=lab&ischecklist=1&ptype=<?=$ptype?>';
    if(ptype=='doctor'){
		autoTagging();   
		...
		return overlib(
			OLiframeContent(pageLabRequest,
				800, 370, 'fGroupTray', 0, 'auto'),
			WIDTH,410, TEXTPADDING,0, BORDER,0,
			STICKY, SCROLL, CLOSECLICK, MODAL,
			CLOSETEXT, '<img src="<?=$root_path?>/images/close.gif" border=0 onclick="requestByDate();">',
...
function openBloodRequest() {
	if(ptype=='doctor') autoTagging();	
	return overlib(
		OLiframeContent('<?=$root_path?>modules/bloodBank/seg-blood-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&area=clinic&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>&dr_nr=<?=$doc_nr?>&dept_nr=<?=$dept_nr?>&ptype=<?=$ptype?>&user_origin=blood&ischecklist=1',
			800, 370, 'fGroupTray', 0, 'auto'),
		WIDTH,410, TEXTPADDING,0, BORDER,0,
		STICKY, SCROLL, CLOSECLICK, MODAL,
		CLOSETEXT, '<img src="<?=$root_path?>/images/close.gif" border=0 onclick="requestByDate();">',
...
{
	var pageRadioRequest = '<?=$root_path?>modules/radiology/seg-radio-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&popUp=1&area=clinic&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>&dr_nr=<?=$doc_nr?>&dept_nr=<?=$dept_nr?>&ischecklist=1&is_dr=<?=$is_dr?>&ptype=<?=$ptype?>';
	if(ptype=='doctor'){
		autoTagging();
...
/*added by mai 08-18-2014*/
function openPfRequest() 
{
	var pagePFRequest = '<?=$root_path?>modules/clinics/seg_pf_request.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&from=CLOSE_WINDOW&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>&mode=new&area=<?=$ptype?>';
	if(ptype=='doctor'){
		autoTagging();
...
{
	var pagePharmaRequest = '<?=$root_path?>modules/pharmacy/seg-pharma-order.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&from=CLOSE_WINDOW&area='+area+'&pid=<?=$pid?>&encounterset=<?=$encounter_nr?>&is_dr=<?=$is_dr?>&billing=1&request_source=<?=$request_source?>&ptype='+ptype;
	if(ptype=='doctor'){
		autoTagging();   
...
{
	var pageMiscRequest = '<?=$root_path?>modules/dialysis/seg-misc-request-new.php?sid=<?php echo "$sid&lang=$lang"?>&clear_ck_sid=<?php echo "$clear_ck_sid"?>&from=CLOSE_WINDOW&pid=<?=$pid?>&encounter_nr=<?=$encounter_nr?>&mode=new&area=<?=$ptype?>';
	if(ptype=='doctor'){
		autoTagging();
...
function initializeTab(id)
{
	...
	if($('is_ic').value==1) {
		...
		switch(id)
		{
			...
			case 4: 
				AJAXTimerId = setTimeout("xajax_populatePfRequests('"+enc_nr+"', '"+src+"', '"+billed+"', '"+seldate+"')",50);
				break;
			/*end added by mai*/
		}
	}
...
function autoTagging(){
	xajax_autoTagging($J('#encounter_nr').val(),dr_nr,or_no);
}
