...
require_once($root_path."include/care_api_classes/class_company.php"); //added by mai 07-21-2014
require_once($root_path."include/care_api_classes/class_pf_charge.php"); //added by mai 08-25-2014
...
function populatePfRequests($encounter_nr, $ptype, $is_bill_final, $date){
	global $db;
	$objResponse = new xajaxResponse();
	$pf_obj = new Pf_charge();
	$pf_requests = $pf_obj->getPfRequests($date, $encounter_nr);

	$objResponse->assign("pf_requests", "innerHTML", "");
	$total_cash = 0;
	$total_charge = 0;

	if($pf_requests){
		while($ref=$pf_requests->FetchRow()){
			$objResponse->call("createTableHeader", "pf_requests", "pf_list".$ref['refno'], $ref['refno'], $ref['charge_type']);
			$res = $pf_obj->getPfRequestsByRefno($ref['refno']);
			if($res){
				$req_flag = false;
				$total_amount = 0;
				while($row=$res->FetchRow()){
					switch(strtolower($row['request_flag'])){
						case 'paid':
							$request_flag = '<img src="../../images/flag_paid.gif"/>';
							$req_flag=true;
							break;
						default:
							 if($ref['is_cash'] == 1){
							 	$request_flag = '<img src="../../images/btn_unpaiditem.gif"/>'; 
							 }else{
							 	$request_flag = '<span style="color:green; font-weight:bold;" >CHARGE</span>';
							 }
							 $req_flag=false; 
							 break;
					}
					...
					$data = array(
                        ...
                        'item_code'=>$row["code"],
                        'item_qty'=>$row["quantity"],
                        'total_prc'=>parseFloatEx($row["chrg_amnt"]),
                        'item_prc'=>parseFloatEx($row["chrg_amnt"])
                    );

					$objResponse->call("printRequestlist", "pf_requests", "pf_list".$ref['refno'], $data);

					$total_amount+=$row["chrg_amnt"];
                    $total_amountCash+=parseFloatEx($row["net_price"]);
				}
                ...
                if(strtolower($ref["charge_type"])=="cash") {
			 	...
			 	if(strtolower($ref["request_source"])==strtolower($ptype))
					$notPtype = false;
				else
					$notPtype = true;

				if($req_flag==false && $notPtype==false && $is_bill_final==0) {
					$buttons = '<button class="segButton" onclick="openEditRequest(\'pf_requests\',\''.$ref['refno'].'\');return false;" style="cursor: pointer;"><img src="../../gui/img/common/default/page_edit.png"/>Edit</button>'.
								'<button class="segButton" onclick="openDeleteRequest(\'pf_requests\',\''.$ref['refno'].'\');return false;" style="cursor: pointer;"><img src="../../gui/img/common/default/cancel.png"/>Delete</button>';
			 	}else {
					$buttons = '<button class="segButton" disabled="disabled"><img src="../../gui/img/common/default/page_edit.png"/>Edit</button>'.
								'<button class="segButton" disabled="disabled"><img src="../../gui/img/common/default/cancel.png"/>Delete</button>';
				}
		...
		$objResponse->assign("pf-total-cash", "innerHTML", number_format($total_cash, 2));
		$objResponse->assign("pf-total-charge", "innerHTML", number_format($total_charge, 2));
	}
	return $objResponse;
}
...
function populateIpRequests($encounter_nr,$ptype,$is_bill_final, $date)
{
	...
	$filters = array('inpatient'=>$encounter_nr,'area'=>'PH', 'date'=>$date);
		...
		while($row=$res->FetchRow())
		{
			...
			if($result!==FALSE) {
				...
				while($row2=$result->FetchRow())
				{
					...
					if($row2['artikelname']){ //added by mai 10-07-2014
				...
				#editted by CELSY 08/25/10
			 		...
			 		if(strtolower($row["request_source"])==strtolower($ptype))
						$notPtype = false;
					else
						$notPtype = false;
...
function deleteRadioServiceRequest($ref_nr)
{
	...
	if ($radio_obj->deleteRefNo($ref_nr)){
		$curl_obj->inpatientRadioItem($ref_nr);
		$objResponse->alert("The request is successfully deleted.");
	}else{
		$objResponse->alert("The request cannot be deleted. It is already or partially paid or it has a result already.");
	}
...
function deletePfRequest($refno){
	$objResponse = new xajaxResponse();
	$pf_obj = new Pf_charge();
	if($save_ok = $pf_obj->deletePfRequest($refno)){
		$objResponse->alert("Professional fee requests are now successfully deleted");
	}else{
		$objResponse->alert("Unable to delete professional fee request");
	}
...
function deleteChargetoCompanyTransaction($refno, $source){
	$company = new Company();
	$deleteTransaction = $company->deleteChargetoCompanyTransaction($refno, $source);

	if($deleteTransaction){
		//success
	}else{
		$objResponse->alert("Unable to delete company transaction");
	}
...
function computeTotalPayment($pid, $encounter_nr)
{
	...
	(SELECT SUM(ld.price_cash*ld.quantity)
		...
		WHERE l.encounter_nr='$encounter_nr' AND l.is_cash=1 AND l.ref_source='LB' AND DATE(l.serv_dt)=DATE(NOW()) AND l.status<>'deleted') AS `lab_total_cash`,
	(SELECT SUM(ld.price_cash*ld.quantity)
		FROM seg_lab_servdetails AS ld
		INNER JOIN seg_lab_serv AS l ON ld.refno=l.refno
		WHERE l.encounter_nr='$encounter_nr' AND l.is_cash=0 AND l.ref_source='LB' AND DATE(l.serv_dt)=DATE(NOW()) AND l.status<>'deleted') AS `lab_total_charge`,
	(SELECT SUM(rd.price_cash)
		FROM care_test_request_radio AS rd
		INNER JOIN seg_radio_serv AS r ON rd.refno=r.refno
		WHERE r.encounter_nr='$encounter_nr' AND r.is_cash=1 AND DATE(r.request_date)=DATE(NOW()) AND rd.status <> 'deleted' AND r.status<>'deleted') AS `radio_total_cash`,
	(SELECT SUM(rd.price_cash)
		FROM care_test_request_radio AS rd
		INNER JOIN seg_radio_serv AS r ON rd.refno=r.refno
		WHERE r.encounter_nr='$encounter_nr' AND r.is_cash=0 AND DATE(r.request_date)=DATE(NOW()) AND rd.status <> 'deleted' AND r.status<>'deleted') AS `radio_total_charge`,
	(SELECT SUM(ph.pricecash*ph.quantity)
		...
		WHERE p.encounter_nr='$encounter_nr' AND p.is_cash=1 AND DATE(p.orderdate)=DATE(NOW()) ) AS `ip_total_cash`,
	(SELECT SUM(ph.pricecash*ph.quantity)
		FROM seg_pharma_order_items AS ph
		INNER JOIN seg_pharma_orders AS p ON ph.refno=p.refno
		WHERE p.encounter_nr='$encounter_nr' AND p.is_cash=0 AND DATE(p.orderdate)=DATE(NOW()) ) AS `ip_total_charge`,
	...
	(SELECT SUM(md.chrg_amnt*md.quantity)
		...
		WHERE m.encounter_nr='$encounter_nr' AND m.is_cash=0 AND DATE(m.chrge_dte)=DATE(NOW()) ) AS `misc_total_charge`, 
		  ...
		  SUM(d.`chrg_amount`) 
		FROM
		  seg_pf_service_details d 
		  LEFT JOIN seg_pf_service s 
		    ON s.`refno` = d.`refno` 
		WHERE s.`encounter_nr` = '$encounter_nr' 
		  AND s.`is_cash` = 1) pf_cash,
		  (SELECT 
		  SUM(d.`chrg_amount`) 
		FROM
		  seg_pf_service_details d 
		  LEFT JOIN seg_pf_service s 
		    ON s.`refno` = d.`refno` 
		WHERE s.`encounter_nr` = '$encounter_nr' 
		  AND s.`is_cash` = 0) pf_charge";
	//$objResponse->alert($sql); 
	$result = $db->Execute($sql);
	$data = $result->FetchRow();

	$total_cash = parseFloatEx($data["lab_total_cash"]+$data["iclab_total_cash"]+$data["bb_total_cash"]+$data["splab_total_cash"]+$data["radio_total_cash"]+$data["ip_total_cash"]+$data["mg_total_cash"]+$data["misc_total_cash"]+$data['pf_cash']);
	$total_charge = parseFloatEx($data["lab_total_charge"]+$data["iclab_total_charge"]+$data["bb_total_charge"]+$data["splab_total_charge"]+$data["radio_total_charge"]+$data["ip_total_charge"]+$data["mg_total_charge"]+$data["misc_total_charge"]+$data['pf_charge']);
