...
class PatientInformation extends Dashlet {
	...
	*/
	public function processAction( DashletAction $action )
	{
		...
		elseif($action->is('updateSmokerDrinkerData'))
		{
		...
		}else if($action->is('updateImportantInfo')){
			global $db;

			$userid = $_SESSION["sess_temp_userid"];
			$encounter_nr = $action->getParameter('enc');
			$important_info = $action->getParameter('important_info');
			...
			if($db->Execute($sql2)){
				$response->alert('Successfully Saved');
				$response->call('assignInfo', $important_info);
			}else{ 
				$response->alert('Error saving: '.$sql2);
			}
		}	
	...
	*/
	public function render($renderParams=null) {
		...
		if ($mode->is(DashletMode::VIEW_MODE))
		{
			...
			$query = "SELECT p.pid, e.encounter_nr, r.religion_name, fn_get_person_name_mname(p.pid) `fullname`, p.sex, e.current_att_dr_nr,\n".
					"e.official_receipt_nr `or`, fn_get_complete_address(p.pid) `address`, fn_get_age(DATE(NOW()), date_birth) `age`,\n".
					"e.chief_complaint, e.smoker_history, e.drinker_history, e.encounter_type, e.important_info,\n".
					"v.`height_ft`,
					  v.`height_in`,
					  v.`weight`".
				"FROM care_encounter e\n".
					"INNER JOIN care_person p ON p.pid=e.pid\n".
					"LEFT JOIN seg_religion r ON r.religion_nr = p.religion\n".
					"LEFT JOIN seg_encounter_vitalsigns v 
					    ON (
					      v.`encounter_nr` = e.`encounter_nr` 
					      AND vitalsign_no = 
					      (SELECT 
					        MAX(vitalsign_no) 
					      FROM
					        seg_encounter_vitalsigns 
					      WHERE encounter_nr = e.encounter_nr)
					    )".
			...
			$patientSmarty = Array(
				...
				'weight' => $row['weight'],
				'height' => $row['height_ft']." ft. and ".$row['height_in']." in.",
			...
			$smarty->assign('requestPf','<button class="button" '.$disable.' onclick="Patient_AddPf();return false;"><img src="../../gui/img/common/default/Appointment.gif"/>Add Professional Fee</button>'); /*added by mai 08-29-2014*/

			$smarty->assign('compreDischarge','<button class="button" '.$disable.' onclick="Patient_CompreDischarge();return false;"><img src="../../gui/img/common/default/documents.gif"/>Comprehensive/Discharge</button>'); /*added by mai 09-11-2014*/
			
			$smarty->assign('dentalRecord','<button class="button" '.$disable.' onclick="Patient_Dental();return false;"><img src="../../gui/img/common/default/hammer.gif"/>Dental Record</button>'); /*added by mai 09-11-2014*/
			...
			$smarty->assign('sImportantInfo','<textarea style="width:100%;" class="segInput" id="important_info_text" >'.$patientSmarty['important_info'].'</textarea>');
