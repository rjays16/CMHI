...
class PatientResultQueue extends Dashlet{
	protected static $name = 'Examination Results';
	protected static $icon = 'folder_table.png';
	...
	public function render($renderParams=null){
		...
		if($mode->is(DashletMode:: VIEW_MODE)){
			...
			return $smarty->fetch($root_path.'modules/dashboard/dashlets/PatientResultQueue/templates/ListView.tpl');
			...
			$smarty->assign('dashlet', $dashletSmarty);
			$preferencesSmarty = array(
				'pageSize' => $this->preference->get('pageSize')
			);

			$smarty->assign('settings', $preferencesSmarty);
			return $smarty->fetch($root_path.'modules/dashboard/dashlets/PatientResultQueue/templates/Config.tpl');
		}else{
