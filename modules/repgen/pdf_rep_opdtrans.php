...
"SELECT distinct cp.pid, cd.name_formal,
    ...
    (SELECT notes FROM seg_encounter_dischargeinfo WHERE encounter_nr = ce.encounter_nr) AS notes,
  	(SELECT GROUP_CONCAT(icd_description,',') FROM seg_doctors_diagnosis WHERE encounter_nr = ce.encounter_nr) AS icd, 
    ce.consulting_dr AS diagnosing_clinician 
