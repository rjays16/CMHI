...
//class RepGen_OPD_Trans extends RepGen {
	...
	function RepGen_OPD_Trans($from, $to, $dept_nr) {
		...
		$this->ColumnWidth = array(8,22,16,17,33,20,10,7,21,34,22,25,40);
		$this->Columns = 13;
	...
	function Header() {
		...
		$this->LogoX = 50;
		$this->LogoY = 3;
		$this->Image('../../gui/img/logos/cmhi_logo.jpg',$this->LogoX,$this->LogoY,20,20);
	...
	$sql =
				"SELECT ce.encounter_nr, cp.pid,
					...
					ce.er_opd_diagnosis,
					 addr_str, cd.id, IFNULL(sei.`hcare_id`, 0) AS membership
				FROM (care_encounter AS ce
					...
					LEFT JOIN seg_encounter_insurance sei 
				      ON (sei.`encounter_nr` = ce.`encounter_nr` 
				      AND sei.`hcare_id` = '18'
				    ) 
		...
		if ($result) {
			...
			if ($row["current_att_dr_nr"])
				...
				$this->Data[]=array(
					...
					mb_strtoupper($row['membership'] ? 'PHIC' : 'NN'),
