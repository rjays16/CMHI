...
*/
	...
	function Header() {
				...
				if ($row = $objInfo->getAllHospitalInfo()) {
					...
					$row['hosp_addr2']   = "2081 NATIONAL HI-WAY SALVACION, PANABO CITY";
				}

				$this->Image($root_path.'gui/img/logos/cmhi_logo.jpg',50,10,15);
				$this->SetXY(0,5);
				$this->SetFont("Arial","I","8");
				$width = 0;
				$height = 4;
				$this->SetXY(30,8);
				$this->Cell($width,$height,$row['hosp_agency'], $borderNo,$newLineYes,'C');
				$this->Ln(2);
				$this->SetFont("Arial","B","9");
				$this->SetXY(23,13);

				$this->Cell($width,$height,$row['hosp_name'],$borderNo,$newLineYes,'C');
				$this->SetFont("Arial","","9");
				$this->SetXY(23,16);
				$this->Cell($width,$height,$row['hosp_addr2'],$borderNo,$newLineYes,'C');
				$this->Ln(4);
				$this->SetFont("Arial","B","9");
				...
				$this->SetXY(140,29);
				$this->SetFont('Arial','B',8);
				$this->Cell(10,5,"WARD: ",0,0,'L');
				$this->SetXY(150,29.5);
				$this->SetFont('Arial','',7);
				$this->MultiCell(50,4,$ward,"0","L","0");
				...
				if($result!=NULL && $resdata = $result->FetchRow())
				{
						...
						$this->SetXY(10,32);
						$this->SetFont('Arial','B',8);
						$this->Cell(140,5,"PHYSICIAN:",0,0,'L');
						$this->SetXY(10,35);
						$this->SetFont('Arial','B',8);
						$this->Cell(10, 5, 'HRN:', 0, 0, 'L');
						$this->SetFont('Arial', '', 7);
	...
	function LabResult_body(){
				...
				/*added by mai 08/04/2014*/
					$this->SetFont('Arial','U',9);
					$this->SetXY(10,50);
					$this->Cell(45,3, 'E X A M I N A T I O N S', 0,'C');
					$this->Cell(25, 4, " ", "", 0, '');
					$this->Cell(45,3, 'R E S U L T', 0,'C');
					$this->Cell(25, 4, " ", "", 0, '');
					$this->Cell(45,3, 'N O R M A L   V A L U E S', 0,'C');
				/*end added by mai*/
				...
				if($result!=NULL)
				{
						...
						while($result!=NULL && $value = $result->FetchRow()){
								...
								elseif($value["is_numeric"]=="1"){
										...
										$this->Line($tb_x+75, $tb_y+5.8, $tb_x+112, $tb_y+5.8);
								}
								else{ //here-mai
										$this->SetXY($tb_x+$str,$tb_y+3);
										$this->MultiCell(45,3,$td .$value["name"],"0","L","0");
										if($fld_value!=""){
												$this->SetXY($tb_x+50,$tb_y+3);
												$this->Cell(45,5,$fld_value,'B',0,'C');
										}
										/*$this->Line($tb_x+47, $tb_y+5.8, $tb_x+112, $tb_y+5.8);*/

										/*added by mai 08/04/2014*/
										$this->SetXY($tb_x+137,$tb_y+3);
										$this->Cell(45,5,$value["SI_lo_normal"].' - '.$value["SI_hi_normal"]." ".$value["SI_unit"],0,0,'L');
										/*end added by mai*/
								}

								$tb_y = $tb_y + $tb_rowheight+3;
						}
						if($this->group_id==13){
								$this->Ln(5);
								$x = $this->getX();
		...
		function Footer()
		{
				...
				if($result)
				{
					 if($resdata = $result->FetchRow())
					 {
							 ...
							 $sql = "SELECT CONCAT(IF(ISNULL(name_first), '', CONCAT(name_first, ' ')), IF(ISNULL(name_middle), '', name_middle), '. ', IF(ISNULL(name_last), '', name_last), ', ', IF(ISNULL(care_personell.other_title), '', care_personell.other_title)) as name from care_person left join care_personell on care_personell.pid = care_person.pid WHERE care_person.pid = '".$pathologist."'";
							 ...
							 if($result!=NULL && $x = $result->FetchRow())
									 $pathologist=$x["name"];
										...
										$pathologist="";
							 $sql = "SELECT CONCAT(fn_get_pid_name('$med_tech_pid'), (SELECT IF(ISNULL(other_title),'', CONCAT(',',other_title)) FROM care_personell WHERE pid= '$med_tech_pid')) as name";
				...
				if($this->group_id=="14"){
						...
						$this->SetXY(20,253);
						$this->Cell(65,3,strtoupper("Pathologist"),0,0,"C");

						$this->SetXY(130,250);
						$this->MultiCell(65,3,strtoupper($med_tech),"B","C","0");
						#$this->Line(67,201,123,201);
						$this->SetXY(130,253);
						$this->Cell(65,3,strtoupper("Examiner"),0,0,"C");
				}

				$this->SetFont("Arial", "", "6");
				$this->SetXY(80,260);
				$this->Cell(50,5,"Encoded by: ".$encoder,0,0,'C');
				$this->SetXY(80,262);
				$this->Cell(50,5,"Encoded on: ".$encode_date,0,0,'C');
		}
}
