...
//class RepGen_OPD_Trans extends RepGen {
	...
	function RepGen_OPD_Trans($from, $to, $dept_nr) {
		...
		$this->ColumnWidth = array(8,22,16,17,19,33,20,12,7,12,35,18,25,28);
		$this->Columns = 14;
	...
	function Header() {
		...
		$this->SetFont("Arial","B","12");
				$this->Cell(0,4,'WEEKLY MORBIDITY/NOTIFIABLE LINE LISTING OF PATIENTS',$border2,1,'C');
		#$this->Ln(0);
	...
	$sql =
				...
				WHERE (DATE(ce.admission_dt) >= '$start_date'
					AND CONCAT(CAST(DATE(ce.admission_dt) AS date), ' 00:00:00') < DATE_ADD('$end_date', INTERVAL 1 DAY))
		...
		if ($result) {
			...
			if ($row["current_att_dr_nr"])
				...
				elseif ($row['civil_status']=='separated')
					$cstatus = "S";
				elseif ($row['civil_status']=='lived-in')
					$cstatus = "LI";
				elseif ($row['civil_status']=='infant')
					$cstatus = "I";
				...
				$this->Data[]=array(
					...
					($row['discharge_date'] ? date("m/d/y",strtotime($row['discharge_date']))." ".date("h:iA",strtotime($row['discharge_time'])) : ''),
