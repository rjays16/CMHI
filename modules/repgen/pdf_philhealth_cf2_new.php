...
$this-> setXY($xsave+135, $ysave);
    ...
    function setAllData($data){
        ...
        $admits_date_ = ($this->encounter_type == OUT_PATIENT) ? $encInfo['er_opd_datetime'] : $encInfo['admission_dt'];
...
//
					...
					$this->Rect($x-28, $y+7, $this->boxwidth+1, $this->boxheight+1);
	 				$this->SetFont($this->fontfamily_label, "", $this->fontsize_label3);
					$this->SetX($x-29);
 	...
 	function addAdmissionDiagnosis()
 	{
		...
		$this->Cell(35, $this->rheight2, $adm_diagnosis,$this->withoutborder, $this->continueline, $this->alignLeft);
	 	...
	 	$this->SetFont($this->fontfamily_label."bd", $this->fontstyle_label_bold, $this->fontsize_label5+3);
...
#added by Nick 2/14/2014
	...
	function addDischargeDiagnosis($icd_desc, $icd_code, $rvs_desc, $rvs_code, $rvs_date, $rvs_late) {
        ...
        for ($i = 0; $i <= 15; $i++) {
            $x[0] = $this->GetX() + $w[0];
            $this->Cell($w[0], 4, strtoupper($icd_desc[$i]), 'B', 1, 'L');//edited by genz change to upper case
        }
        ...
        for ($i = 0; $i <= 15; $i++) {
            $this->SetX($x[1] + 2);
            $x[2] = $this->GetX() + $w[2];
            $this->Cell($w[2], 4, strtoupper($rvs_desc[$i]), 'B', 1, 'L');//edited by genz change to upper case
        }
...
$icdData = $objICD_ICP->setCodesData($icd,30,true);//updated by genz
$rvsData = $objICD_ICP->setCodesData($rvs,30,true);//updated by genz
