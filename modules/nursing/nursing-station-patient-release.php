...
if($patient_ok){
	...
	if($released){
		...
		$smarty->assign('sResults',false);
		$smarty->assign('sDisp',false);
	}else{
		$init=1;
		while($dis_type=$discharge_types->FetchRow()){
		...
		//commented by alex feb. 2 , 2016
		// added jason
		// if ($enc_obj->loadEncounterData($pn)) {
		// 	$enc_type = $enc_obj->EncounterType();
		// 		if ($enc_type == 1) {
		...
		// 				$enc_death_info = $person_obj->getPatientInfo($enc_obj->PID());
		// 				while ($result_death_info=$enc_death_info->FetchRow()) {
		// 					$death_date = $result_death_info['death_date'];
		// 					$death_time = $result_death_info['death_time'];
		// 				}
		...
		// 						</select>&nbsp;<font size=1>[hh:mm]</font><br>';

		// 				$sTmp = $sTmp.'<span id="death_date_span" style="display:none"><input type="text" id="death_date" value="'.$death_date.'" name="date_death" size=12 maxlength=10 value=""  onBlur="IsValidDate(this,\''.$date_format.'\')"  onKeyUp="setDate(this,\''.$date_format.'\',\''. $lang.'\')">
		// 									<img ' . createComIcon($root_path,'show-calendar.gif','0') . ' id="x_date_trigger2" style="cursor:pointer"> <font size=1>[mm/dd/yyyy]</font>
		// 									'.$deathtime.'
		// 								</span>';
		// 			}
		// 			$counter ++;
		// 		} 
		// 		$showResults = $sTmp;
		// 		$smarty->assign('sShowResults',$showResults);
		// 		$smarty->assign('sResults',true);
		// 		$jsCalScript = "<script type=\"text/javascript\">
		...
		// 		// end result row
		// 		$counter = 0;
		// 		while($result=$rowDisp->FetchRow()){ // disposition row
		// 			if ($result['disp_desc']!='Admitted'){
		// 				$sTmp2 = $sTmp2.'&nbsp;<input name="disp_code" id="disp_code" type="radio" value="'.$result['disp_code'].'" onclick="disposition_settings('.$result['disp_code'].');"';
		// 				if ($counter == 0) {$sTmp2=$sTmp2.'checked';}
		// 				$sTmp2 = $sTmp2.'>';
		// 				$sTmp2 = $sTmp2.$result['disp_desc']."<br>";

		// 				if($result['disp_code'] == '8'){
		// 					$sTmp2 = $sTmp2.'<div id="disp" style="display:none">
		...
		// 									 &nbsp;&nbsp;<label>Address: </label><input type="text" name="hosp_add" placeholder="Hospital Address" size="18px" value="'.$patient_enc_disp['hosp_add'].'"/></div>' ;
		// 				}
		// 			}
		// 			$counter ++;
		// 		}
		// 		$showDisp = $sTmp2; 
		// 		$smarty->assign('sShowDisp',$showDisp);
		// 		$smarty->assign('sDisp',true);
		// 		// end disposition row
