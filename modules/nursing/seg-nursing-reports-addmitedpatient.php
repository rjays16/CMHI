...
* RESUSE By Genesis D. Ortiz (05-28-2014)
*/

	class RepGen_Admitted_Patient extends RepGen {
	var $area;
	var $date;
	var $encoder;
	var $shift_start;
	...
	function RepGen_Admitted_Patient ($area, $date, $encoder, $shift_start, $shift_end, $full) {
		global $db;
		$this->RepGen("LIST OF ADMITTED PATIENT");
		#$this->ColumnWidth = array(25,60,20,18,18,20,100,21);
		# 165
		$this->colored = FALSE;
		$this->ColumnWidth = array(20,50,40,40,30,15,10);
		$this->RowHeight = 6;
		$this->Alignment = array('L','L','L','L','L','C','L');
  ...
  // 	$this->Cell($total_w,4,'Republic of the Philippines',$border2,1,'C');
	 ...
	 //  $this->Cell($total_w,4,'DEPARTMENT OF HEALTH',$border2,1,'C');
	  ...
	  $this->SetFont('Arial','B',12);
		$this->Cell(17,5);
  	$this->Cell($total_w,4,'LIST OF ADMITTED PATIENTS',$border2,1,'C');
...
#			exit;
    ...
    $this->SetFont('Arial','B',9);
		...
		$this->Cell($this->ColumnWidth[1],$row,'Last Name',1,0,'C',1);
		$this->Cell($this->ColumnWidth[2],$row,'First Name',1,0,'C',1);
		$this->Cell($this->ColumnWidth[3],$row,"Middle",1,0,'C',1);
		$this->Cell($this->ColumnWidth[4],$row,"Ward",1,0,'C',1);
		$this->Cell($this->ColumnWidth[5],$row,"Room #",1,0,'C',1);
		$this->Ln();
	...
	function FetchData() {		
		...
		$sql = "SELECT ce.pid, ce.encounter_nr, name_first, name_middle, name_last,  cw.name AS ward, current_room_nr 
				FROM (care_person AS cp INNER JOIN care_encounter AS ce ON ce.pid = cp.pid) 
				LEFT JOIN care_ward cw ON cw.nr = ce.current_ward_nr";

		if($this->date){
			$date = date("Y-m-d",strtotime($this->date));
			
				$sql .= " WHERE (encounter_date <= '$date') AND ce.is_discharged != 1 AND ce.encounter_type = 3";
		}
		else{
			$sql .= " WHERE ce.is_discharged != 1 AND ce.encounter_type = 3";
		}
		...
		if ($result) {
			...
			while ($row=$result->FetchRow()) {
				$row['is_maygohome'] = 'Admitted';
				$this->Data[]=array($row['pid'],$row['name_last'],$row['name_first'],$row['name_middle'],$row['ward'], $row['current_room_nr']);
			}	
		}
		else {
			print_r($sql);
...
$rep = new RepGen_Admitted_Patient($_GET['area'],$_GET['date'],$_GET['encoder'],$_GET['shiftstart'], $_GET['shiftend']);
$rep->AliasNbPages();
$rep->FetchData();
$rep->Report();
?>