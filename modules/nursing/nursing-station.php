...
define('PHIC',18);  # Define to 18 if PHIC
...
//added by EJ 09/17/2014
$quick_informer_date = $ward_obj->getQuickInformerDetails();

while($qid = $quick_informer_date->FetchRow()) {
	$total_inpatient++;
...
#echo "name = ".$LDLastName;
	...
	$smarty->assign('LDMgh', 'MGH');
	$smarty->assign('LDMembership', 'Membership');
...
*/
	while ($perRoom = $wardRoom->FetchRow()){
		...
		for($j=1;$j<=$room_info['nr_of_beds'];$j++){
			...
			$smarty->assign('sMgh',''); /*added by mai 07-04-2014*/
			$smarty->assign('sMembership',''); /*added by mai 07-04-2014*/
			...
			#added by Genz (05-27-2014)
			$smarty->assign('sMoveIcon','');
...
#echo $bed['date_birth'];
			...
			if ($bed['is_maygohome']) $smarty->assign('sMgh','<span style="color:red">MGH<span/>'); /*added by mai 07-04-2014*/
			if ($bed['hcare_id'] == 18) $smarty->assign('sMembership','<span style="color:green">PHIC<span/>'); /*added by mai 07-04-2014*/
	...
	*/
			if($edit){
				if(($is_patient)&&!empty($bed['encounter_nr'])){
					...
					$smarty->assign('sMoveIcon','<a href="javascript:void(0);" onClick="moveToWaitingList(\''.$bed['encounter_nr'].'\')" title="Move patient to waiting list"><img '.createComIcon($root_path,'user_go.png','0','',TRUE).' alt="Move patient to waiting list"></a>');
				}
			}
