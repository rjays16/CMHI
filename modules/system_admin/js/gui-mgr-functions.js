...
function search()
{
	var searchname = $('service_name').value;
	xajax_searchcostcenter(searchname);
}
function gethistofcenter(listID,cid,id,wardid,phic,nonphic,date,created)
{
	var list=$(listID), dRows, dBody, rowSrc;
	var i;
	var classified, mode, editlink;
			...
			if (id) {
				...
				if (dRows.length%2 != 0) alt = "alt";
				 mode="edit";
					...
					rowSrc = 
						'<td width="2%" align="center" nowrap="nowrap">'+id+'</td>'+
						'<td width="10%" align="center" nowrap="nowrap">'+wardid+'</td>'+
						'<td width="10%" align="center" nowrap="nowrap">'+phic+'%</td>'+
						'<td width="10%" align="center" nowrap="nowrap">'+nonphic+'%</td>'+
						'<td width="10%" align="center" nowrap="nowrap">'+date+'</td>'+
						'<td width="10%" align="center" nowrap="nowrap">'+created+'</td>'+
					'</tr>';
			}
			else {
					rowSrc = '<tr><td colspan="4" style="">No GUI in the list...</td></tr>';
...
function cosCenter(listID,cid,id,wardid,phic,nonphic){
				...
				if (dRows.length%2 != 0) alt = "alt";
				 mode="edit";
					...
					rowSrc = 
						'<td width="2%" align="center" nowrap="nowrap">'+id+'</td>'+
						'<td width="10%" align="center" nowrap="nowrap">'+wardid+'</td>'+
						'<td width="10%" align="center" nowrap="nowrap"><input id="cphic'+cid+'" style="width:30px; text-align:center" type=text value='+phic+'></td>'+
						'<td width="10%" align="center" nowrap="nowrap"><input id="cnonphic'+cid+'" style="width:30px; text-align:center" type=text value='+nonphic+'></td>'+
						'<td width="1%" align="center">'+
						'<img src="../../../images/cashier_check.png" onclick="updateData(\''+cid+'\',\''+id+'\');"/> '+
						'</td>'+
					'</tr>';
			}
			else {
...
function updateData(id,nameWard)
{
	var Cphic = $('cphic'+id).value;
	var Cnonphic = $('cnonphic'+id).value;
	if(Cphic > 99 || Cnonphic > 99 )
	{
			alert('Please, Dont Exceed to 99 percent');
	}
	else
	{
			var reply = confirm("Are you sure you want to update ward  "+nameWard+"?");
			if(reply)
			{
				xajax_updatecostcenter(id,Cphic,Cnonphic);
