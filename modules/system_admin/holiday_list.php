...
$breakfile='edv-system_manage.php'.URL_APPEND.'&target=holiday';
...
$refCode=$holiday_obj->getLimitHoliday($GLOBAL_CONFIG['pagin_icpm_list_max_block_rows'], $pgx, $oitem,$odir);
#echo "sql = ".$person_obj->sql;
$linecount=$holiday_obj->LastRecordCount();
...
if(isset($totalcount)&&$totalcount){
	...
	$totalcount=$holiday_obj->countAllHoliday();
	$pagen->setTotalDataCount($totalcount);
}
...
<script type="text/javascript">
	function deleteHoliday(id, holiday_name){
		var answer = confirm("Are you sure you want to delete the holiday "+(holiday_name.toUpperCase())+"?");
		if (answer){
			xajax_deleteHoliday(id);
		}
	}
...
<table border=0 cellpadding=2 cellspacing=1 id="holiday_list" width="50%">
  <tr class="wardlisttitlerow">
      <td><b>Delete</b></td>
			<td><b>
	  	<?php echo $pagen->makeSortLink('Holiday','holiday',$oitem,$odir);  ?></b>
	  </td>
	  <td><?php echo $pagen->makeSortLink('Date','month',$oitem,$odir);  ?></b></td>
  </tr> 
<?php
	$toggle=0;
	$listID=0;
...
?>
  ...
  <tr  bgcolor="<?php echo $bgc ?>" id="row<?=$refcode['nr'];?>" >
  	 <td class=pblock  bgColor="#eeeeee" align="center" valign="middle" width="5%">
 			<img name="delete<?=$refcode['nr'];?>" id="delete<?=$refcode['id'];?>" src="../../images/btn_delitem.gif" style="cursor:pointer" border="0" onClick="deleteHoliday('<?=$refcode['id'];?>','<?=$refcode['holiday'];?>');"/>
		 </td>
	  <td><a href="holiday_info.php<?php echo URL_APPEND.'&retpath=list&holiday_nr='.$refcode['id']; ?>"><?php echo $refcode['holiday']; ?></a></td>
	  <td><?php 
	  $month_options = array('','Jan', 'Feb', 'Mar', 'April', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec');
	  echo $month_options[$refcode['month']]." ".$refcode["day"]?></td>
  </tr> 
<?php $listID++;
	}
	echo '
