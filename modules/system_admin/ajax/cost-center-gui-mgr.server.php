...
function cosCenter(){
	...
	$objResponse = new xajaxResponse();
	$guiObj = new CostCenterGuiMgr();
	$result = $guiObj->getCosCenter();

    $objResponse->call("clearList","gcosCenter");
	while($row=$result->FetchRow()){
	  
		$objResponse->call("cosCenter","gcosCenter",trim($row["ccprice_id"]),trim($row["ward_name"]),trim($row["ward_id"]),trim($row["PHIC"]),trim($row["NONPHIC"]));

	}
	return $objResponse;
}
function gethistofcenter()
{
	global $db;
	$objResponse = new xajaxResponse();
	$guiObj = new CostCenterGuiMgr();
    ...
    $objResponse->call("clearList","historyCcenter");
	while($row=$result->FetchRow()){
	  
		$objResponse->call("gethistofcenter","historyCcenter",trim($row["ccprice_id"]),trim($row["ward_name"]),trim($row["ward_id"]),trim($row["PHIC"]),trim($row["NONPHIC"]),trim($row["date_created"]),trim($row["history"]));

	}
	return $objResponse;
}
function searchcostcenter($name){
	
	global $db;
	$objResponse = new xajaxResponse();
	$guiObj = new CostCenterGuiMgr();
	$result = $guiObj->searchCenter($name);
...
function updatecostcenter($id,$phic,$nonphic)
{
	...
	$objResponse = new xajaxResponse();
	$guiObj = new CostCenterGuiMgr();
	$result = $guiObj->updatecostcenter($id,$phic,$nonphic);
	if($result)
	{
		$objResponse->call("refreshFrame","Successfully Update!");
	}
	else
	{
		$objResponse->call("refreshFrame","Failed to Update!");
	}	
