...
/*added by mai 07/29/2014 Commented By Jarel
...
// 				return false;
// 			}
// 			else if(parseFloatEx($('show-net-total').innerHTML) < price){
// 				alert('partial amount is greater than net total!')
// 				return false;
// 			}
// 		}

// 		$('partial_amount').value=price;
// 		$("show-partial").setAttribute('value',price)
// 		$("show-partial").innerHTML = formatNumber(price,2)
// 	}
...
//added by maimai 11-20-2014
function flagSeniorCheckbox(name, flag){
	var items = document.getElementsByName('senior'+name);
	...
	for (var i=0; i<items.length; i++)
		if (items[i].type.toLowerCase()=='checkbox') {
			if (!items[i].disabled){
				...
				if(items[i].checked != flag){
					items[i].checked = flag;
					seniorPrice(items[i], items[i].getAttribute('srcRefItem'));
				}	
				//End Jarel
			}
...
function seniorPrice(check, id){
	var total_id = $('total_'+id);
	var price_id = $('price_'+id);

	var total = parseFloatEx(total_id.value);
	var price = parseFloatEx(price_id.value);

	if(check.checked){
		total_id.value = total - (total * 0.20);
		price_id.value = price - (price * 0.20);
	}else{
		//total_id.value = price;
		total_id.value = total / 0.80;
		price_id.value = price / 0.80;
	}

	$('spantotal_'+check.id).innerHTML = formatNumber(total_id.value, 2);
	$('spanprice_'+check.id).innerHTML = formatNumber(price_id.value, 2);
	refreshTotal();
}
		...
		if (details.id) {
			var _id = details.id,
                _origID = details.id, //added by jasper 05/29/2013
					...
					_seniorCheck = ((details.senior == 0.2) ? 'checked': '');
			...
			_total = _total - (_total * details.senior); 
			_price = _price - (_price * details.senior); 
			//end added by mai
				...
				if (_flag) {
					...
					if (parseFloat(_limit) < 0) {
						...
						'<td align="right" style="color:'+_adjColor+'">'+
							'<input id="price_'+srcRefItem+'" name="price_'+srcRef+'[]" srcDept="'+src+'" refNo="'+src+'" itemID="'+_id+'" type="hidden" '+disabledTag+' value="'+_price+'"/>'+
							'<span id="spanprice_'+_id+'">'+formatNumber(_price,2)+'</span>'+
						'</td>';
						if (src=="misc") {
						...
						'</td>'+
						'<td align="center"><input type="checkbox" prevValue="'+_total+'" srcRefItem="'+srcRefItem+'" '+ _seniorCheck+' onchange="seniorPrice(this, \''+srcRefItem+'\');" refNo="'+ref+'" itemID="'+_id+'" id="'+_id+'" name="senior'+srcRef+'[]"></td>'+
						'<td align="right" style="color:'+_adjColor+'">'+
							'<input id="total_'+srcRefItem+'" name="total_'+srcRef+'[]" srcDept="'+src+'" refNo="'+ref+'" itemID="'+_id+'" type="hidden" '+disabledTag+' value="'+_total+'"/>'+
							'<span id="spantotal_'+_id+'">'+formatNumber(_total,2)+'</span>'+
						'</td>';
					}
					else {
						rowSrc += '				<td align="right">'+
						...
						rowSrc+='<td align="right">'+
							'<input id="totalorig_'+srcRefItem+'" name="totalorig_'+srcRef+'[]" srcDept="'+src+'" refNo="'+ref+'" itemID="'+_id+'" type="hidden" '+disabledTag+' value="'+_origtotal+'"/>'+formatNumber(_origtotal,2)+
						'</td>'+
						'<td align="center"><input type="checkbox" refNo="'+ref+' "onchange="seniorPrice(this, \''+srcRefItem+'\');"  itemID="'+_id+'" id="'+_id+'" name="senior'+srcRef+'[]"></td>'+
						'<td align="right" style="color:'+_adjColor+'">'+
							'<input id="total_'+srcRefItem+'" name="total_'+srcRef+'[]" srcDept="'+src+'" refNo="'+ref+'" itemID="'+_id+'" type="hidden" '+disabledTag+' value="'+_total+'"/>'+
							'<img src="../../images/charity.gif" align="absmiddle" />'+
						'</td>';
	...
	'<table id="list_'+srcRef+'" class="segList" border="0" cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:10px">'+
		'<thead>'+
			'<tr id=\"row_'+srcRef+'">'+
				...
				'<th align="right" width="9%" nowrap="nowrap">'+
					'<input type="checkbox" onchange="flagSeniorCheckbox(\''+srcRef+'[]\',this.checked); calcSubTotal(\''+src+'\',\''+ref+'\')">'+
				'Senior Disc.</th>'+
...
function refreshAmountChange() {
	...
	//var partial = parseFloatEx($('show-partial').getAttribute('value')) //added by mai 07/29/2014

	if (isNaN(total)) total=0
	var tendered = parseFloatEx($('amount_tendered').value)
	if (isNaN(tendered)) tendered=0
	// if(partial!=0){ //added by mai 07/29/2014
	// 	var change = tendered-partial
	// }else{
