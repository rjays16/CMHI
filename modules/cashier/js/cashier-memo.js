		...
		if (dBody) {
			...
			if (typeof(details) == 'object') {
				...
				var id = details.id,
					...
					price = details.price,
					refund_price = (details.refund_price ? details.refund_price : refund*price),
					name = details.name,
					desc = details.desc,
				...
				HTML+=
					...
					'</td>'+
					'<td class="centerAlign">'+
						'<input type="text" class="jedInput" type="text" name="refund[]" id="refund_'+rowid+'" value="'+refund+'" prevValue="'+refund+'" itemID="'+rowid+'" style="width:85%;text-align:center" onfocus="this.select()" onchange="adjustQty(this)" onkeyup="if (event.keyCode==13) this.blur()"/>'+
					'</td>'+
					'</td>'+
					//added by EJ 10/02/2014
					'<td class="centerAlign">'+
						'<input class="jedInput" type="text" name="refund_price[]" id="refund_price_'+rowid+'" value="'+formatNumber(refund_price,2)+'" prevValue="'+formatNumber(refund_price,2)+'" itemID="'+rowid+'" style="width:85%;text-align:center" onfocus="this.select()" onchange="adjustPrice(this)" onkeyup="if (event.keyCode==13) this.blur()"/>'+
					'</td>'+
					//ended by EJ 10/02/2014
					'<td class="centerAlign">'+
...
function adjustQty(obj) {
	...
	if (isNaN(obj.value) || obj.value<=0) {
		alert("Invalid refund quantity entered...");
	...
	if (parseFloatEx(obj.value) != parseFloatEx(obj.getAttribute("prevValue"))) {
		...
		$('refund_price_'+id).value =  formatNumber(refund,2);
	}
	refreshTotal();
...
function adjustPrice(obj) {
	...
	if (isNaN(obj.value) || obj.value<=0) {
		alert("Invalid refund price entered...");
		obj.value = obj.getAttribute("prevValue");
		obj.focus();
		obj.select();
		return false;
	...
	if ((parseFloatEx(obj.value)) > refund) {
		alert("Price refunded exceeds the maximum refundable...");
		obj.value = $('refund_total_'+id).value;
		obj.focus();
		obj.select();
		return false;
	}
	if (parseFloatEx(obj.value) != parseFloatEx(obj.getAttribute("prevValue"))) {
		var tot_refund = parseFloatEx($('refund_price_'+id).value);

		$('refund_total_'+id).value = tot_refund;
		$('refund_total2_'+id).innerHTML = formatNumber(tot_refund,2);
	}
	refreshTotal();
	var here = obj.setAttribute("prevValue",parseFloatEx(obj.value));
	//console.log(parseFloatEx(obj.value))
	return true;
}
