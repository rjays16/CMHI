...
if ($_POST['delete']) {
	if ($cm->clearMemoItems($_POST['delete']) && $cm->deleteMemo($_POST['delete'])) {
                if(ENABLE_FIS){
		$curl = new Rest_Curl();
		$curl->creditMemoAdd($_POST['delete'], $_POST['issue_date']);
	...
	while ($row=$result->FetchRow()) {
		...
		$rows .= "		<tr class=\"$class\">
				...
				<td align=\"center\">
					...
					<input type=\"hidden\" name=\"issue_date\" value=\"".date("Y-m-d h:ia",strtotime($row["issue_date"]))."\">
				</td>
				<td>".$row["memo_name"]."</td>
