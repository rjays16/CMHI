...
function addPFOItem($src, $ref, $code, $qty, $amount) {
	...
	if (in_array(strtolower($src),array('other','pp','fb','com'))) { //updated by mai 07/24/2014
...
function addReference($sDept, $sRefNo, $checked_requests=NULL, $hide_load=0, $sOR=NULL) {
	...
	if (is_numeric($rRow['grant_amount']))
		...
		$limit = -1;
	if (in_array(strtolower($sDept), array('ph','rd','ld','misc','pf'))) {
		$dept_names = array('pf'=>'Professional Fee', 'ph'=>'Pharmacy request', 'rd'=>'Radiology request', 'ld'=>'Laboratory request', 'fb'=>'Final billing',	'pp'=>'Partial payment', 'or'=>'Operating room', 'other'=>'Misc. services', 'misc'=>'Miscellaneous request');
...
//			}
			...
			if($itemOr){
				$item->senior = $cClass->getSeniorDiscount($itemOr, $item->ref, $item->id, $item->src);
			}
