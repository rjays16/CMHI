...
//			}
			...
			if($_REQUEST['payerType']=="company"){//added mai
				...
				$amount = $person_obj->getPersonOutstandingBill($row["bill_nr"]);
				$cancelled_amount = $person_obj->getCancelledPayments($row['bill_nr']); //added by maimai 12-02-2014
				$CRMamount = $person_obj->getTotalCRMBill($row["bill_nr"]); ;
			    $total = $row['total'] + ((( $CRMamount ) ? $CRMamount : 0 ) + (( $amount ) ? $amount : 0 ) + $cancelled_amount);
			} //end added mai
...
?>
			<tr class="<?= $class ?>">
				...
				<td align="center">
					...
					<?php if(($row["request_flag"] && $_REQUEST['payerType']!="company") || ($row['request_flag'] && $_REQUEST['payerType']=="company")){ ?>
						<?= ("<img src=\"".$root_path."images/flag_{$row['request_flag']}.gif\" align=\"absmiddle\" />") ?>
					<?php } else if((($row["request_flag"]) || (!$row["request_flag"] && $row["doctor_flag"])) && $_REQUEST['payerType']!="company"){ ?>
						<b>Partially Paid</b>
					<?php } else{ ?>
						<?= "" ?>
					<?php } ?>
