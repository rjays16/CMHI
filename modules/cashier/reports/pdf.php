...
//		$this->Cell(335.75,3,'',1,1,'C');
        ...
        function totalCollection()
        {
            ...
            if ($this->_count) {
                ...
                $this->Cell(45, $this->RowHeight, "CSR", 1, 0, 'L', 1);
        ...
        function AfterData()
        {
            ...
            if (!$this->_count) {
                ...
                $this->Cell(20, $this->RowHeight, number_format($this->total_cash + $this->total_check, 2), 1, 0, 'R', 1);
        ...
        function FetchData()
        {
            ...
            if ($this->encoder) {
                $where[] = "create_id=" . $db->qstr(utf8_decode($this->encoder));
            }
                    ...
                    if (!$data[$orNo]) {
                        ...
                        if (!$row['check_no']) {
                            switch (strtolower($row['ref_source'])) {
                                case 'ph':
                                    $this->total_ph += $row['amount'];
                                    break;
                                case 'csr':
                                    $this->total_csr += $row['amount'];
                                    break;
                                case 'ld':
                                    $this->total_ld += $row['amount'];
                                    break;
                                case 'rd':
                                    $this->total_rd += $row['amount'];
                                    break;
                                case 'pf':
                                    $this->total_pf += $row['amount'];
                                    break;
                                default:
