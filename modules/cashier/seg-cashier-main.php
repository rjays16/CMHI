...
if (isset($_POST["submitted"])) {
	...
	if (is_array($requests)) {
		...
		$amt_arr = array();
		$lab_items = array();
		$senior_arr = array(); //added by maimai 11-21-2014
		...
		foreach ($requests as $req) {
			...
			if (is_array($items)) {
				foreach ($items as $i=>$item) {
					...
					$senior_arr[] = ($_POST['senior'.$req][$i] ? 0.2 : 0);
				...
				foreach ($ref_arr as $rowIndex => $dummy) {
					if($src_arr[$rowIndex]=='fb' && $cnt == 0){
						...
						$data_ledger['pid'] = $data['pid'];
					   	...
					   	$data_ledger['description'] = $ORNo;

						$saveok = $person->savePersonLedgerInfo($data_ledger);
					   	
					   	if(!$person->hasBillSaveInLedger($data_ledger['bill_nr'])){
...
<?php
	...
	if ($checked_requests) {
		foreach ($checked_requests as $i=>$item) {
			if (in_array(strtolower($item['ref_source']),array('other','fb','pp', 'com'))) { //updated by mai 07/24/2014 added com
?>
...
if($pay_info["pid"])
    $enc_nr = $person->CurrentEncounter($pay_info["pid"]);
...
// $smarty->assign('sGUIPartialPaymentBtn', '<img class="segSimulatedLink" src="'.$root_path.'images/btn_add_partial.gif" align="absmiddle" id="partialPaymentBtn"'.
// 	'onclick="partialPayment()">'); //added by mai 07/29/2014
