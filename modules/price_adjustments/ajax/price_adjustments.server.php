<?php
	 ...
	 /*function populateHospitalService($service_type,$page)
	 {
		...
		function populateLabServiceList($searchId, $keyword, $page,$can_edit)
		{
			...
			if($dataRow)
			{
				...
				$cnt=0;
				while($result=$dataRow->FetchRow()) {
					$objResponse->call("viewPriceList","PriceList",trim($result["service_code"]),trim($result["name"]),trim($result["price_cash"]),trim($result["price_charge"]),trim($can_edit));
					$serviceCodeArray[$cnt]=$result["service_code"];
					$objResponse->call("saveServiceCode", $serviceCodeArray[$cnt]);
					$cnt++;
				}#end of while
		...
		function populateRadioServiceList($searchId, $keyword, $page,$can_edit)
		{
			global $db;
			$objResponse = new xajaxResponse();
			$glob_obj = new GlobalConfig($GLOBAL_CONFIG);
		...
		function populatePharmaServiceList($searchId, $keyword, $page,$can_edit)
		{
			global $db;
			$objResponse = new xajaxResponse();
			$glob_obj = new GlobalConfig($GLOBAL_CONFIG);
			...
			if($dataRow)
			{
				...
				$cnt = 0;
				while($result=$dataRow->FetchRow()) {
					$objResponse->call("viewPriceList","PriceList",trim($result["bestellnum"]),trim($result["artikelname"]),trim($result["price_cash"]),trim($result["price_charge"]),trim($can_edit));
					$serviceCodeArray[$cnt]=$result["bestellnum"];
					$objResponse->call("saveServiceCode", $serviceCodeArray[$cnt]);
					$cnt++;
				}#end of while
		...
		function populateMiscServiceList($searchId, $keyword, $page,$can_edit)
		{
			global $db;
			$objResponse = new xajaxResponse();
			$glob_obj = new GlobalConfig($GLOBAL_CONFIG);
		...
		function populateOtherServiceList($searchId, $keyword, $page,$can_edit)
		{
			global $db;
			$objResponse = new xajaxResponse();
			$glob_obj = new GlobalConfig($GLOBAL_CONFIG);
			...
			if($dataRow)
			{
				...
				$cnt = 0;
				while($result=$dataRow->FetchRow()) {
					$objResponse->call("viewPriceList","PriceList",trim($result["code"]),trim($result["name"]),trim($result["price"]),trim($result["price"]),trim($can_edit));
