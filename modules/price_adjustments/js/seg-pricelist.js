...
function startAJAXSearch(id, page,disabler)
{
    var searchEL = $(id);
	var keyword;
    var servType;
	...
	if (searchEL) {
        ...
        if ($('is_edit').value==1) {
		    ...
		    switch(servType)
		    {
			    case "1": AJAXTimerID = setTimeout("xajax_populateLabServiceList('"+id+"','"+keyword+"',"+page+",'"+disabler+"')",50); break;
			    case "2": AJAXTimerID = setTimeout("xajax_populateRadioServiceList('"+id+"','"+keyword+"',"+page+",'"+disabler+"')",50); break;
			    case "3": AJAXTimerID = setTimeout("xajax_populatePharmaServiceList('"+id+"','"+keyword+"',"+page+",'"+disabler+"')",50); break;
			    case "4": AJAXTimerID = setTimeout("xajax_populateMiscServiceList('"+id+"','"+keyword+"',"+page+",'"+disabler+"')",50); break;
			    case "5": AJAXTimerID = setTimeout("xajax_populateOtherServiceList('"+id+"','"+keyword+"',"+page+",'"+disabler+"')",50); break;
		    }
        }
...
function viewPriceList(listID, code, name, priceCash, priceCharge, disabler)
{
		...
		if (list) {
				...
				if (code) {
						...
						rowSrc = '<tr class="wardlistrow'+alt+'" id="row'+addslashes(code)+'" value="'+code+'">'+
														...
														'<td width="15%" align="center">'+'<input type="text" size="10" id="priceCash'+code+'" value='+priceCash+' onblur="callAlert(this.value,this.id); format_number(this.value,this.id,2); save_partly(\''+code+'\');" '+disabler+'></input><input type="hidden" id="priceCash_orig'+code+'" value='+priceCash+'></input></td>'+
														'<td width="15%" align="center">'+'<input type="text" size="10" id="priceCharge'+code+'" value='+priceCharge+' onblur="callAlert(this.value,this.id); format_number(this.value,this.id,2); save_partly(\''+code+'\');" '+disabler+'></input><input type="hidden" id="priceCharge_orig'+code+'" value='+priceCharge+'></input></td>'+
