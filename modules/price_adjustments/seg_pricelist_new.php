...
$can_edit = $acl->checkPermissionRaw(array('_a_1_service_price_manager'));
$can_edit = ($can_edit || $allAccess);
$disabler = ($can_edit) ? "" : "readOnly";
...
$id="service_name";
$smarty->assign('sCostCenters', '<select class="segInput" id="inputarea" name="inputarea" onChange="startAJAXList(this.id,0);initializeTempArray();startAJAXSearch(\''.$id.'\',0,\''.$disabler.'\');ClearText();">
...
$smarty->assign('search_service', '<input type="text" class="segInput" id="service_name" name="service_name" size="50" onkeyup="if(this.value.length>=3) startAJAXSearch(this.id,0,\''.$disabler.'\');" style="background-color: rgb(226, 234, 243); border-width: thin; font: bold 13px Arial;" readonly=""/>');

$smarty->assign('searchserv_btn', '<input class="segButton" id="search_serv" type="button" value="Search" onclick="startAJAXSearch(\''.$id.'\',0,\''.$disabler.'\'); return false;" disabled=""/>');

if($can_edit){
	$smarty->assign('saveBtn', '<input class="segButton" id="save" type="button" value="Save" onclick="save_changes();"/>');
}else{
	$smarty->assign('saveBtn', '<input class="segButton" id="save" type="button" value="Save" disabled/>');
}
